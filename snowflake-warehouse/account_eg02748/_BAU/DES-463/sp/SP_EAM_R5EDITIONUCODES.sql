                ...
                create or replace procedure DATAHUB_INTEGRATION.EAM_TARGET_MERGE_R5EDITIONUCODES()
              ...
              //  Start execution - log start
                ...
                snowflake.execute( {sqlText: "begin transaction;"} );
                  try
                      ...
                      var sql_command = `      
                                merge into  "DATAHUB_TARGET"."EAM_R5EDITIONUCODES" as target using (
                    ...
                    SELECT * FROM (SELECT
                     ...
                     FROM DATAHUB_INTEGRATION.VW_STREAM_EAM_R5EDITIONUCODES as  strm
                     )   
                WHERE ROWNUMBER=1  
                    ...
                    when matched then update set
                                target.EDU_RCODE=src.EDU_RCODE,
                                target.EDU_CODE=src.EDU_CODE,
                                target.EDU_LANG=src.EDU_LANG,
                                target.EDU_DESC=src.EDU_DESC,
                                target.EDU_RENTITY=src.EDU_RENTITY,
                                target.EDU_MARKET=src.EDU_MARKET,
                                target.EDU_TEXTCODE=src.EDU_TEXTCODE,
                                target.EDU_LASTSAVED=src.EDU_LASTSAVED,
...
`
  merge into  "DATAHUB_TARGET_HISTORY"."EAM_DELETED_R5EDITIONUCODES" as target using (
