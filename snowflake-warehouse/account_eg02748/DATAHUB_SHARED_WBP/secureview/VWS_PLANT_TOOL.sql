CREATE OR REPLACE SECURE VIEW ${buildvar.env}_WBP_SHARE.DATAHUB_SHARED_WBP.VWS_PLANT_TOOL
AS
SELECT 
...
, XTOOL.SUBTYPEFEATURES                          AS "SubType Features"
, A1.WARRANTYEND           						 AS "Warranty End Date"
, A1.WARRANTYSTART         						 AS "Warranty Start Date"
, XTOOL.FUELTYPE                                 AS "Fuel Type"
, XTOOL.FUNCAREA                                 AS "Functional Area"
, XTOOL.IPRATING                                 AS "Ingress Protection Rating"
, XTOOL.PHOTO3D                                  AS "Photo/3D Model"
...
FROM
  ...
  LEFT OUTER JOIN "${buildvar.env}_WCC_DATAWAREHOUSE"."DATAHUB_TARGET".IPS_WSLASSETSCONFIGURED_XTOOL AS XTOOL ON A2.COMPKEY = XTOOL.COMPKEY 
  WHERE A2.OWN = 'WBP' and A4.COMPCODE = 'TOOL';