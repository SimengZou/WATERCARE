CREATE OR REPLACE SECURE VIEW ${buildvar.env}_WBP_SHARE.DATAHUB_SHARED_WBP.VWS_PLANT_CIVIL
AS
SELECT
...
, XCIVIL.DIAMEXT                           			AS "Diameter External (mm)"
, XCIVIL.DIAMINT                           			AS "Diameter Internal (mm)"
, XCIVIL.INVLEVEL                                	AS "Invert Level (M)"
, A1.WARRANTYEND              						AS "Warranty End Date"
, A1.WARRANTYSTART            						AS "Warranty Start Date"
, XCIVIL.DESIGNRESILIENCE                           AS "Design Resilience"
, XCIVIL.QUAKEDESIGN                                AS "Earthquake Design Level"
, XCIVIL.EXTCOAT                            		AS "External Coating"
, XCIVIL.GRDLEVEL                                	AS "Ground Level"
, XCIVIL.HAZARDRATING                               AS "Hazardous Rating"
, XCIVIL.LOADRATING                                 AS "Load Rating (kN)"
, XCIVIL.LOCALITY                                   AS "Locality"
, XCIVIL.PHOTO3D                                    AS "Photo/3D Model"
...
FROM
  ...
  LEFT OUTER JOIN "${buildvar.env}_WCC_DATAWAREHOUSE"."DATAHUB_TARGET".IPS_WSLASSETSCONFIGURED_XCIVIL AS XCIVIL ON A2.COMPKEY = XCIVIL.COMPKEY 
  WHERE A2.OWN = 'WBP' and A4.COMPCODE = 'CIVL';
