...
create or replace procedure datahub_logging.sp_etl_log_process(log_type VARCHAR, process_name VARCHAR, process_status VARCHAR, log_message VARCHAR, process_id FLOAT, insert_rows FLOAT, update_rows FLOAT)
    returns string
    language javascript
    execute as caller
    as
    ...
    if( LOG_TYPE === "Insert")
        ...
        try
            {
            var log_sql_command;
            log_sql_command = "insert into datahub_logging.etl_log_process (process_name, process_execution_id, process_status, execution_message) ";
    ...
    if( LOG_TYPE === "Update")
        ...
        try
            {
            var log_sql_command;
            log_sql_command = "update datahub_logging.etl_log_process ";
