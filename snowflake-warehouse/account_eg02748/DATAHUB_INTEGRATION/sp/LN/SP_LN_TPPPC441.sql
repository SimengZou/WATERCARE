create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPPC441()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPPC441_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPPC441 as target using (SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
            strm.amac_4, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPPC441 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPPC441_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
