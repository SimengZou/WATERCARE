create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPSS220()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPSS220_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPSS220 as target using (SELECT * FROM (SELECT 
            strm.actv_cpla, 
            strm.basn, 
            strm.bfdt, 
            strm.bsdt, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPSS220 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.basn=src.basn) OR (target.basn IS NULL AND src.basn IS NULL)) AND
            ((target.cact=src.cact) OR (target.cact IS NULL AND src.cact IS NULL)) 
                when matched and src.ETL_DELETED=TRUE THEN DELETE
                when matched then update set
                    target.actv_cpla=src.actv_cpla, 
                    target.basn=src.basn, 
                    target.bfdt=src.bfdt, 
                    target.bsdt=src.bsdt, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPSS220_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.actv_cpla, 
            strm.basn, 
            strm.bfdt, 
        ...
        when not matched   then insert ( actv_cpla, basn, bfdt, bsdt, cact, compnr, cprj, cprj_basn_ref_compnr, cprj_ref_compnr, deleted, pact, sequencenumber, tact, tact_kw, timestamp, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
