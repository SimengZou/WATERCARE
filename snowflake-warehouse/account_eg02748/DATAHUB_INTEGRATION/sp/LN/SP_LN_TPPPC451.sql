create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPPC451()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPPC451_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPPC451 as target using (SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
            strm.amac_4, 
            ...
            strm.cpla,
            strm.cact,
            strm.ccts ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPPC451 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.ccts=src.ccts) OR (target.ccts IS NULL AND src.ccts IS NULL)) 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPPC451_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
