create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPDM007()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPDM007_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPDM007 as target using (SELECT * FROM (SELECT 
            strm.compnr, 
            strm.cwar, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPDM007 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPDM007_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.compnr, 
            strm.cwar, 
            strm.cwar_ref_compnr, 
        ...
        when not matched   then insert ( compnr, cwar, cwar_ref_compnr, deleted, item, item_ref_compnr, sequencenumber, site, site_ref_compnr, timestamp, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
