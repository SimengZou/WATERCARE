create or replace procedure DATAHUB_INTEGRATION.SP_LN_TFGLD202()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TFGLD202_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TFGLD202 as target using (SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TFGLD202 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.dimx=src.dimx) OR (target.dimx IS NULL AND src.dimx IS NULL)) AND
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TFGLD202_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
            strm.ccur_ref_compnr, 
        ...
        when not matched   then insert ( bpid, ccur, ccur_ref_compnr, compnr, cono, deleted, dimx, dtyp, duac, duac_kw, fcah_1, fcah_2, fcah_3, fcam, fdah_1, fdah_2, fdah_3, fdam, fqt1, fqt2, leac, ncah_1, ncah_2, ncah_3, ncam, ndah_1, ndah_2, ndah_3, ndam, nqt1, nqt2, prno, ptyp, ptyp_kw, sequencenumber, timestamp, username, year, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
