create or replace procedure DATAHUB_INTEGRATION.SP_IPS_CDR_BUILDING_BLDGPLCOND()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_CDR_BUILDING_BLDGPLCOND_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_CDR_BUILDING_BLDGPLCOND as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.APBLDGKEY, 
            strm.APBLDGPLCONDKEY, 
            ...
            strm.APBLDGPLCONDKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_CDR_BUILDING_BLDGPLCOND as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.APBLDGPLCONDKEY=src.APBLDGPLCONDKEY) OR (target.APBLDGPLCONDKEY IS NULL AND src.APBLDGPLCONDKEY IS NULL)) 
                when matched and src.ETL_DELETED=TRUE THEN DELETE
                when matched then update set
                    target.ADDBY=src.ADDBY, 
                    target.ADDDTTM=src.ADDDTTM, 
                    target.APBLDGKEY=src.APBLDGKEY, 
                    target.APBLDGPLCONDKEY=src.APBLDGPLCONDKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_CDR_BUILDING_BLDGPLCOND as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.APBLDGKEY, 
