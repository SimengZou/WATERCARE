create or replace procedure DATAHUB_INTEGRATION.SP_IPS_METERMANAGEMENT_WATER_READINGEXPORTSCHEDULE()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_METERMANAGEMENT_WATER_READINGEXPORTSCHEDULE_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_METERMANAGEMENT_WATER_READINGEXPORTSCHEDULE as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            ...
            strm.DISABLEAUTOSCHEDULING, 
            strm.EXCLBILLEDDAYS, 
            strm.EXCLBILLEDFLAG, 
            strm.EXCLEXCEPMETERS, 
            strm.EXCLLASTREADDAYSAGO, 
            ...
            strm.READINGEXPORTSCHEDULEKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_METERMANAGEMENT_WATER_READINGEXPORTSCHEDULE as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.READINGEXPORTSCHEDULEKEY=src.READINGEXPORTSCHEDULEKEY) OR (target.READINGEXPORTSCHEDULEKEY IS NULL AND src.READINGEXPORTSCHEDULEKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.DISABLEAUTOSCHEDULING=src.DISABLEAUTOSCHEDULING, 
                    ...
                    target.EXCLLASTREADDAYSAGO=src.EXCLLASTREADDAYSAGO, 
                    ...
                    target.EXPORTDATE=src.EXPORTDATE, 
                    ...
                    target.ISRERUN=src.ISRERUN, 
                    target.MODBY=src.MODBY, 
                    target.MODDTTM=src.MODDTTM, 
                    target.NEXTREADINGDATE=src.NEXTREADINGDATE, 
        ...
        when not matched and (src.ETL_DELETED=FALSE OR src.ETL_DELETED IS NULL)  then insert ( 
            ...
            values (
                    ...
                    src.DISABLEAUTOSCHEDULING, 
                    src.EXCLBILLEDDAYS, 
                    src.EXCLBILLEDFLAG, 
                    src.EXCLEXCEPMETERS, 
                    src.EXCLLASTREADDAYSAGO, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_METERMANAGEMENT_WATER_READINGEXPORTSCHEDULE as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ALLOWARCHIVEFLAG, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ALLOWARCHIVEFLAG, ALLOWOUTSTANDINGROUTESFLAG, CONCATENATEFILEFLAG, DAYSBEFORENEXTREADINGDAY, DELETED, DELETEPREVFILEVERFLAG, DISABLEAUTOSCHEDULING, EXCLBILLEDDAYS, EXCLBILLEDFLAG, EXCLEXCEPMETERS, EXCLLASTREADDAYSAGO, EXCLLASTREADFLAG, EXCLNEWMETERS, EXCLNOACCTMETERS, EXCLSRAFTERDAYS, EXCLSRBEFOREDAYS, EXCLSRFLAG, EXCLZEROREADINGMETERS, EXCLZEROUSAGEMETERS, EXPORTDATE, EXPORTDIRPATH, EXPORTRUNNUMBER, EXPORTTEMPLATEFILE, EXPORTTYPE, ISRERUN, MODBY, MODDTTM, NEXTREADINGDATE, READINGEXPORTSCHEDULEKEY, RECEIVERCODE, RERUNNUMBER, RUNNUMBER, SCHEDULEKEY, SENDERCODE, SORTBYCOLUMNS, VARIATION_ID, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
