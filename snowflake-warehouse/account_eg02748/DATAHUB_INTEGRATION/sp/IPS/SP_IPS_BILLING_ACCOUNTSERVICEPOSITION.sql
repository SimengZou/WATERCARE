create or replace procedure DATAHUB_INTEGRATION.SP_IPS_BILLING_ACCOUNTSERVICEPOSITION()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_BILLING_ACCOUNTSERVICEPOSITION_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_BILLING_ACCOUNTSERVICEPOSITION as target using (SELECT * FROM (SELECT 
            ...
            strm.ACCOUNTSERVICEPOSITIONKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_BILLING_ACCOUNTSERVICEPOSITION as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.ACCOUNTSERVICEPOSITIONKEY=src.ACCOUNTSERVICEPOSITIONKEY) OR (target.ACCOUNTSERVICEPOSITIONKEY IS NULL AND src.ACCOUNTSERVICEPOSITIONKEY IS NULL)) 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_BILLING_ACCOUNTSERVICEPOSITION as target using (
                SELECT * FROM (SELECT 
            strm.ACCOUNTSERVICEKEY, 
            strm.ACCOUNTSERVICEPOSITIONKEY, 
            strm.ADDBY, 
        ...
        when not matched   then insert ( ACCOUNTSERVICEKEY, ACCOUNTSERVICEPOSITIONKEY, ADDBY, ADDDTTM, ADDRKEY, ASSETTYPE, CONSUMPTIONPERCENTAGE, DATALAKE_DELETED, GRAPHUSAGEFLAG, HIDEREADINGSONBILL, METERREGISTERUSE, MODBY, MODDTTM, MOVEINREADINGKEY, MOVEOUTREADINGKEY, POSITION, STARTDTTM, STOPDTTM, SUBTRACTIVEFLAG, USEDAYSRDS, USEUOM, USGHISTINBILLOUTPUT, VARIATION_ID, WINTERAVG, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
