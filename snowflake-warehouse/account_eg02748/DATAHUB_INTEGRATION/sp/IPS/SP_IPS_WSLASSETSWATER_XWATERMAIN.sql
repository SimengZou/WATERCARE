create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSWATER_XWATERMAIN()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSWATER_XWATERMAIN_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSWATER_XWATERMAIN as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XWATERMAINKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSWATER_XWATERMAIN as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XWATERMAINKEY=src.XWATERMAINKEY) OR (target.XWATERMAINKEY IS NULL AND src.XWATERMAINKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XWATERMAINKEY=src.XWATERMAINKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSWATER_XWATERMAIN as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
