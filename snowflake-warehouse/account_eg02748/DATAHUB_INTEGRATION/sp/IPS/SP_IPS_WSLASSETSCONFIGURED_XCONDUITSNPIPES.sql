create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSCONFIGURED_XCONDUITSNPIPES()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSCONFIGURED_XCONDUITSNPIPES_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSCONFIGURED_XCONDUITSNPIPES as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XCONDUITSNPIPESKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSCONFIGURED_XCONDUITSNPIPES as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XCONDUITSNPIPESKEY=src.XCONDUITSNPIPESKEY) OR (target.XCONDUITSNPIPESKEY IS NULL AND src.XCONDUITSNPIPESKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XCONDUITSNPIPESKEY=src.XCONDUITSNPIPESKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSCONFIGURED_XCONDUITSNPIPES as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
