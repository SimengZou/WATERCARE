create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSSEWER_XSEWERMAIN()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSSEWER_XSEWERMAIN_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSSEWER_XSEWERMAIN as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XSEWERMAINKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSSEWER_XSEWERMAIN as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XSEWERMAINKEY=src.XSEWERMAINKEY) OR (target.XSEWERMAINKEY IS NULL AND src.XSEWERMAINKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XSEWERMAINKEY=src.XSEWERMAINKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSSEWER_XSEWERMAIN as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
