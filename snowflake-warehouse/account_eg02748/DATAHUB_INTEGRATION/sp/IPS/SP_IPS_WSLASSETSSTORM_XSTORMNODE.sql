create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSSTORM_XSTORMNODE()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSSTORM_XSTORMNODE_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSSTORM_XSTORMNODE as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XSTORMNODEKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSSTORM_XSTORMNODE as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XSTORMNODEKEY=src.XSTORMNODEKEY) OR (target.XSTORMNODEKEY IS NULL AND src.XSTORMNODEKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XSTORMNODEKEY=src.XSTORMNODEKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSSTORM_XSTORMNODE as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
        ...
        when not matched   then insert ( ACCELDEPREC, ACCELDEPRECDESC, ACQTYPE, ADDBY, ADDDTTM, BUSINESSAREA, COMPKEY, CONFINEDSPACE, COSTCENTRE, CRITICALITY, DATALAKE_DELETED, DISPOSALDESC, DRAWINGNO, DRAWINGNOREF, FORDISPOSAL, LINKEDASSET, MAKE, MODBY, MODDTTM, MODELNO, PROJECTNO, PROJECTWISE, RELDATE, RELDEPTH, RELDIA, RELGEOSP, RELMATL, RELOWN, RELSTATUS, SAFETYCRITICAL, VARIATION_ID, WARRANTYEND, WARRANTYSTART, XSTORMNODEKEY, ZONECATCHMENT, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
