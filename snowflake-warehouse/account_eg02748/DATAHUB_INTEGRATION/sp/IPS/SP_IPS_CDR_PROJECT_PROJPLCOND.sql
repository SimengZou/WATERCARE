create or replace procedure DATAHUB_INTEGRATION.SP_IPS_CDR_PROJECT_PROJPLCOND()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_CDR_PROJECT_PROJPLCOND_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_CDR_PROJECT_PROJPLCOND as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            ...
            strm.APPROJPLCONDKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_CDR_PROJECT_PROJPLCOND as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.APPROJPLCONDKEY=src.APPROJPLCONDKEY) OR (target.APPROJPLCONDKEY IS NULL AND src.APPROJPLCONDKEY IS NULL)) 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_CDR_PROJECT_PROJPLCOND as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.APPLANSTATUSKEY, 
