create or replace procedure DATAHUB_INTEGRATION.SP_IPS_BILLING_CLASS()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_BILLING_CLASS_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_BILLING_CLASS as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.BUSINESSFLAG, 
            strm.CODE, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_BILLING_CLASS as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.CODE=src.CODE) OR (target.CODE IS NULL AND src.CODE IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.BUSINESSFLAG=src.BUSINESSFLAG, 
                    ...
                    target.SURETYBONDFLAG=src.SURETYBONDFLAG, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_BILLING_CLASS as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.BUSINESSFLAG, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, BUSINESSFLAG, CODE, DELETED, DELINQUENTDATEDAYS, DELINQUENTDATEFORMULAKEY, DESCRIPT, DUEDATEDAYS, DUEDATEFORMULAKEY, EFFDATE, EXPDATE, MODBY, MODDTTM, SURETYBONDFLAG, VARIATION_ID, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
