create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSSEWER_XSEWERSERVICELINE()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSSEWER_XSEWERSERVICELINE_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSSEWER_XSEWERSERVICELINE as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XSEWERSERVICELINEKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSSEWER_XSEWERSERVICELINE as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XSEWERSERVICELINEKEY=src.XSEWERSERVICELINEKEY) OR (target.XSEWERSERVICELINEKEY IS NULL AND src.XSEWERSERVICELINEKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XSEWERSERVICELINEKEY=src.XSEWERSERVICELINEKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSSEWER_XSEWERSERVICELINE as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
