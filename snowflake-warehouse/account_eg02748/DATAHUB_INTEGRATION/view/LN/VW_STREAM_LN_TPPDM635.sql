CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_LN_TPPDM635 AS SELECT
                        ...
                        src:dfpp::integer AS dfpp, 
                        src:dfpp_kw::varchar AS dfpp_kw, 
                        src:dfps::integer AS dfps, 
                        src:dfps_kw::varchar AS dfps_kw, 
                        src:dfsb::varchar AS dfsb, 
                        src:dfsb_ref_compnr::integer AS dfsb_ref_compnr, 
                        ...
                        src:uset_ref_compnr::integer AS uset_ref_compnr, src:sequencenumber::integer as ETL_SEQUENCE_NUMBER,
            CASE
                WHEN 'LN' = 'LN'
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:dfpp), '0'), 38, 10) is not null and 
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:dfps), '0'), 38, 10) is not null and 
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:dfsb_ref_compnr), '0'), 38, 10) is not null and 
