CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_LN_TFACP001 AS SELECT
                        src:affi::integer AS affi, 
                        src:affi_kw::varchar AS affi_kw, 
                        src:compnr::integer AS compnr, 
                        src:defp::varchar AS defp, 
                        src:defp_ref_compnr::integer AS defp_ref_compnr, 
                        src:deleted::boolean AS deleted, 
                        src:desc::object AS desc, 
                        src:fisu::varchar AS fisu, 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:defp_ref_compnr), '0'), 38, 10) is not null and 
