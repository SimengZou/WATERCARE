CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_IPS_CDR_CODEENFORCEMENT_CASESTATUSCODE AS SELECT
                        ...
                        src:ISPORTALCLOSEDSTATUS::varchar AS ISPORTALCLOSEDSTATUS, 
                        src:MODBY::varchar AS MODBY, 
                        src:MODDTTM::datetime AS MODDTTM, 
                        src:SYSTEMLICENSE::varchar AS SYSTEMLICENSE, 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_CODEENFORCEMENT_CASESTATUSCODE as strm)
                WHERE
                ROWNUMBER=1) where 
