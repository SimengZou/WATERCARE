CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_EAM_U5ASTMAPPING AS SELECT
                        src:AST_ATTRBDESC::varchar AS AST_ATTRBDESC, 
                        src:AST_ATTRIBUTE::varchar AS AST_ATTRIBUTE, 
                        src:AST_CLASS::varchar AS AST_CLASS, 
                        src:AST_CLASSDESC::varchar AS AST_CLASSDESC, 
                        src:AST_ID::varchar AS AST_ID, 
                        src:AUTOID::numeric(38, 10) AS AUTOID, 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_EAM_U5ASTMAPPING as strm)
                WHERE
                ROWNUMBER=1) where 
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:AUTOID), '0'), 38, 10) is not null and 
