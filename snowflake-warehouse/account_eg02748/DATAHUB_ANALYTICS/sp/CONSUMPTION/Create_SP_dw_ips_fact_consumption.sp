//  Name:       sp_dw_ips_fact_consumption
//
//  Purpose:    Load consumption data - staging table dw_ips_fact_consumption loaded from staging table dw_ips_stage_consump_stg9 by job sp_dw_ips_fact_consumption
...
create or replace procedure datahub_analytics.sp_dw_ips_fact_consumption()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_ANALYTICS.DW_IPS_FACT_CONSUMPTION;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `	INSERT INTO DATAHUB_ANALYTICS.DW_IPS_FACT_CONSUMPTION
		 (ACCOUNT_KEY, ACCOUNT_SERVICE_KEY, ADDRESS_KEY, COMP_KEY, month, ACT_DAILY_AVG, ACTUAL_DAYS, 
		 ACTUAL_VOLUME, FULL_MONTH_FLAG, EST_DAILY_AVG, EST_DAYS, EST_VOLUME, TOTAL_VOLUME, ESTIMATE_FLAG, 
		 ESTIMATE_METHOD, CONSUMPTION_PERCENTAGE, ETL_LOAD_DATETIME, ETL_SOURCE)
		 SELECT 
		        account_key, account_service_key, address_key, comp_key,
		        month, act_daily_avg, actual_days, actual_volume, full_month_flag,
		        est_daily_avg, est_days, est_volume, total_volume, estimate_flag, estimate_method,
		        consumption_percentage, GETDATE() as etl_load_datetime,  'IPS' as etl_source
		 FROM datahub_staging.dw_ips_stage_consump_stg9
		 ORDER BY account_key, account_service_key, comp_key, MONTH;`} );
