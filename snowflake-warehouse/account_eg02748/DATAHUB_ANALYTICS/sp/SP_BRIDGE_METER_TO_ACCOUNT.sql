CREATE OR REPLACE PROCEDURE "SP_BRIDGE_METER_TO_ACCOUNT"()
...
//  Run merge statement
        ...
        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT target USING (SELECT  ";
				...
				sql_command += "   FROM DATAHUB_STAGING.STG2_BRIDGE_METER_TO_ACCOUNT";
				sql_command += "   WHERE ACCOUNT_SERVICE_POSITION_KEY IS NOT NULL";
				sql_command += "   ) AS b ON coalesce(target.ACCOUNT_SERVICE_POSITION_KEY,-1) =coalesce(b.ACCOUNT_SERVICE_POSITION_KEY ,-1)";
				sql_command += "    and target.WATER_METER_SID=b.WATER_METER_SID ";
				sql_command += "    and target.ACCOUNT_SERVICE_SID=b.ACCOUNT_SERVICE_SID ";
				sql_command += "    and target.ACCOUNT_SID=b.ACCOUNT_SID ";
				sql_command += "   WHEN MATCHED THEN UPDATE";
				sql_command += "   SET target.WATER_METER_SID=b.WATER_METER_SID,";
				sql_command += "   target.ACCOUNT_SID=b.ACCOUNT_SID,";
				sql_command += "   target.ACCOUNT_SERVICE_SID=b.ACCOUNT_SERVICE_SID,";
				sql_command += "   target.SUBTRACTIVE_FLAG=b.SUBTRACTIVE_FLAG,";
				sql_command += "   target.CONSUMPTION_PERCENTAGE=b.CONSUMPTION_PERCENTAGE,";
				sql_command += "   target.CONSUMPTION_PERCENTAGE_FLAG=b.CONSUMPTION_PERCENTAGE_FLAG,";
				...
				sql_command += "   target.RECORD_END_DATE=b.RECORD_END_DATE";
				...
				sql_command += "   CURRENT_RECORD)";
				sql_command += "   VALUES (  ";
				sql_command += "   b.WATER_METER_SID,";
				sql_command += "   b.ACCOUNT_SID,";
				sql_command += "   b.ACCOUNT_SERVICE_SID,";
				sql_command += "   b.ACCOUNT_SERVICE_POSITION_KEY,";
				sql_command += "   b.WATER_METER_KEY, ";
				sql_command += "   b.ACCOUNT_KEY, ";
				sql_command += "   b.ACCOUNT_SERVICE_KEY,";
				sql_command += "   b.SUBTRACTIVE_FLAG,";
				sql_command += "   b.CONSUMPTION_PERCENTAGE,";
				sql_command += "   b.CONSUMPTION_PERCENTAGE_FLAG,";
				sql_command += "    b.ETL_SOURCE_DATETIME, ";
				sql_command += "    b.ETL_RECORD_CREATED, ";
				sql_command += "    b.ETL_RECORD_UPDATED,";
				sql_command += "   coalesce(b.current_record,''Y''));";
