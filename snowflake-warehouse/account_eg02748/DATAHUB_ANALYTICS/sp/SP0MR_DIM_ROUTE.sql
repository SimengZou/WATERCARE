CREATE OR REPLACE PROCEDURE "SP0MR_DIM_ROUTE"()
...
//  Run merge statement
			...
			sql_command += "  METER.ROUTE_KEY,";
			sql_command += "  ''Not Available'' AS ROUTE_DESCRIPTION,";
			sql_command += "  max(METER.ETL_LOAD_DATETIME) AS ETL_SOURCE_DATETIME, ";
			sql_command += "  min(METER.ETL_RECORD_CREATED) AS ETL_RECORD_CREATED, ";
			sql_command += "  max(METER.ETL_RECORD_UPDATED) AS ETL_RECORD_UPDATED";
			sql_command += " FROM DATAHUB_STAGING.STG_DIM_WATER_METER METER";
			sql_command += "   LEFT JOIN DATAHUB_ANALYTICS.DIM_ROUTE ROUTE ON METER.ROUTE_KEY = ROUTE.ROUTE_KEY";
			sql_command += " WHERE ROUTE.ROUTE_KEY IS NULL AND METER.ROUTE_KEY IS NOT null";
