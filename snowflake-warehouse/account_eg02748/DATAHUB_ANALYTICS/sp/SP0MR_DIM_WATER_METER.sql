CREATE OR REPLACE PROCEDURE "SP0MR_DIM_WATER_METER"()
...
//  Run merge statement
			...
			sql_command += "  READING.COMP_KEY,";
			sql_command += "  0 AS ROUTE_SID,";
			sql_command += "  ''Not Available'' AS METER_DESCRIPTION,";
			sql_command += "  max(READING.ETL_SOURCE_DATETIME) AS ETL_LOAD_DATETIME, ";
			...
			sql_command += "   LEFT JOIN DATAHUB_ANALYTICS.DIM_WATER_METER METER ON READING.COMP_KEY = METER.COMP_KEY";
			sql_command += " WHERE METER.COMP_KEY IS NULL AND READING.COMP_KEY is not null ";
			sql_command += " GROUP BY 1";
			sql_command += "  ) AS b ON target.COMP_KEY =b.COMP_KEY";
			sql_command += "  WHEN NOT MATCHED THEN INSERT (COMP_KEY, ";
