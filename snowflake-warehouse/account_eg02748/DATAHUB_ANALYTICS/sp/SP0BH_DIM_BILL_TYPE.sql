CREATE OR REPLACE PROCEDURE "SP0BH_DIM_BILL_TYPE"()
...
//  Run merge statement
        ...
        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.DIM_BILL_TYPE target USING (SELECT  ";
			sql_command += "  BILLHEADER.BILL_TYPE_KEY,";
			sql_command += "  ''N/A'' AS BILL_TYPE, ";
			sql_command += "  ''Not Available'' AS BILL_TYPE_DESCRIPTION,";
			sql_command += "  max(BILLHEADER.ETL_SOURCE_DATETIME) AS ETL_SOURCE_DATETIME, ";
			sql_command += "  min(BILLHEADER.ETL_RECORD_CREATED) AS ETL_RECORD_CREATED, ";
			sql_command += "  max(BILLHEADER.ETL_RECORD_UPDATED) AS ETL_RECORD_UPDATED";
			sql_command += " FROM DATAHUB_STAGING.STG_FACT_BILL_HEADER BILLHEADER";
			sql_command += "   LEFT JOIN DATAHUB_ANALYTICS.DIM_BILL_TYPE BILLTYPE ON BILLHEADER.BILL_TYPE_KEY =BILLTYPE.BILL_TYPE_KEY";
			sql_command += "    WHERE BILLTYPE.BILL_TYPE_KEY IS NULL AND BILLHEADER.BILL_TYPE_KEY IS NOT NULL";
			sql_command += " GROUP BY 1";
			sql_command += "  ) AS b ON target.BILL_TYPE_key =b.BILL_TYPE_key";
			sql_command += "  WHEN NOT MATCHED THEN INSERT (BILL_TYPE_KEY, ";
			...
			sql_command += "  VALUES (b.BILL_TYPE_KEY, ";
			sql_command += "  b.BILL_TYPE,";
			sql_command += "  b.BILL_TYPE_DESCRIPTION ,";
