CREATE OR REPLACE PROCEDURE "SP_DAILY_BILL_HEADER_LOAD"()
...
// /* Load Bill Header data model */

//  define array of stored procedure calls 
        const call_sp = [];
			
		call_sp[0] = " CALL DATAHUB_STAGING.SP_STG_DIM_BILL_TYPE(); ";
		call_sp[1] = " CALL DATAHUB_ANALYTICS.SP_DIM_BILL_TYPE(); ";

		call_sp[2] = " CALL DATAHUB_STAGING.SP_STG_DIM_BILL_RUN(); ";
		call_sp[3] = " CALL DATAHUB_ANALYTICS.SP_DIM_BILL_RUN(); ";


	//	/* get deltas */
		call_sp[4] = " CALL DATAHUB_STAGING.SP_STG_FACT_BILL_HEADER(); ";

//  Initialise dimension records if needed
        call_sp[5] = " CALL DATAHUB_ANALYTICS.sp0BH_DIM_BILL_TYPE(); ";
		call_sp[6] = " CALL DATAHUB_ANALYTICS.sp0BH_DIM_BILL_RUN(); ";
		call_sp[7] = " CALL DATAHUB_ANALYTICS.sp0BH_DIM_BILL(); ";
		call_sp[8] = " CALL DATAHUB_ANALYTICS.sp0BH_DIM_CUSTOMER_ACCOUNT(); ";
       
//	/* extract SIDs from dimensions */
		call_sp[9] = " CALL DATAHUB_STAGING.SP_STG2_FACT_BILL_HEADER(); ";

	//	/* merge new data to fact table */
		call_sp[10] = " CALL DATAHUB_ANALYTICS.SP_FACT_BILL_HEADER(); ";
     ...
     if (result === "Success") 
	   {
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Completed Bill Header Model Load'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
       }
     else 	{
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Bill Header Model Load  Failed'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
