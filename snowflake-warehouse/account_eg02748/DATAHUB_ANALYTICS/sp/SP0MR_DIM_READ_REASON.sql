CREATE OR REPLACE PROCEDURE "SP0MR_DIM_READ_REASON"()
...
//  Run merge statement
	        ...
	        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.DIM_READ_REASON target USING (SELECT  ";
			sql_command += "  READING.READ_REASON AS READ_REASON_CODE,";
			sql_command += "  ''Not Available'' AS READ_REASON_DESCRIPTION,";
			sql_command += "  max(READING.ETL_SOURCE_DATETIME) AS ETL_SOURCE_DATETIME, ";
			sql_command += "  min(READING.ETL_RECORD_CREATED) AS ETL_RECORD_CREATED, ";
			sql_command += "  max(READING.ETL_RECORD_UPDATED) AS ETL_RECORD_UPDATED";
			sql_command += " FROM DATAHUB_STAGING.STG_FACT_METER_READING READING";
			sql_command += "   LEFT JOIN DATAHUB_ANALYTICS.DIM_READ_REASON READREASON ON upper(READING.READ_REASON)= upper(READREASON.READ_REASON_CODE)";
			sql_command += " WHERE READREASON.READ_REASON_CODE IS NULL AND READING.READ_REASON IS NOT NULL";
			sql_command += " GROUP BY 1";
			sql_command += "  ) AS b ON upper(target.READ_REASON_CODE) = upper(b.READ_REASON_CODE)";
			sql_command += "  WHEN NOT MATCHED THEN INSERT (READ_REASON_CODE, ";
			...
			sql_command += "  VALUES (b.READ_REASON_CODE, ";
			sql_command += "  b.READ_REASON_DESCRIPTION ,";
