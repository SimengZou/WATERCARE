CREATE OR REPLACE PROCEDURE "SP0_BRIDGE_METER_TO_ACCOUNT"()
...
//  Run merge statement
        ...
        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT target USING ( ";
				sql_command += " (SELECT  WATER_METER_SID, ";
				...
				sql_command += "    WHERE ACCOUNT_SERVICE_POSITION_KEY IS NULL AND ACCOUNT_SID=0";
				sql_command += "    AND WATER_METER_SID NOT IN (SELECT DISTINCT WATER_METER_SID FROM DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT ))";
				...
				sql_command += "    CURRENT_RECORD";
				sql_command += "    FROM DATAHUB_STAGING.STG2_BRIDGE_METER_TO_ACCOUNT";
				sql_command += "    WHERE ACCOUNT_SERVICE_POSITION_KEY IS NULL AND ";
				sql_command += "    WATER_METER_SID=0";
				sql_command += "    AND ACCOUNT_SID NOT IN (SELECT DISTINCT ACCOUNT_SID FROM DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT ))";
				sql_command += "   ) AS b ON coalesce(target.ACCOUNT_SERVICE_POSITION_KEY,-1) =coalesce(b.ACCOUNT_SERVICE_POSITION_KEY ,-1)";
				sql_command += "   and coalesce(target.WATER_METER_key,-1)=coalesce(b.WATER_METER_key,-1)";
				sql_command += "   and coalesce(target.ACCOUNT_SERVICE_key,-1)=coalesce(b.ACCOUNT_SERVICE_key,-1)";
				sql_command += "   and coalesce(target.ACCOUNT_key,-1)=coalesce(b.ACCOUNT_key,-1)";
