CREATE OR REPLACE PROCEDURE "SP_DAILY_BILLING_OPTIMISATION_LOAD"()
...
// /* Load Billing Optimisation data models */

//  define array of stored procedure calls 
        const call_sp = [];
			
		call_sp[0] = " CALL DATAHUB_ANALYTICS.SP_DAILY_METER_READINGS_LOAD(); ";
		
		call_sp[1] = " CALL DATAHUB_ANALYTICS.SP_DAILY_BILL_LINE_ITEM_LOAD(); ";
		
		call_sp[2] = " CALL DATAHUB_ANALYTICS.SP_DAILY_BRIDGE_METER_TO_ACCOUNT_LOAD(); ";

		call_sp[3] = " CALL DATAHUB_ANALYTICS.SP_DAILY_BILL_HEADER_LOAD(); ";
		
		call_sp[4] = " CALL DATAHUB_ANALYTICS.SP_DAILY_ACCOUNT_TRANSACTION_LOAD(); ";
     ...
     if (result === "Success") 
	   {
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Completed Billing Optimisations Models Load'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
       }
     else 	{
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Billing Optimisations Models Load Failed'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
