//  Name:       sp_stg30_ips_sm_billing_meter
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//  stg30_ips_sm_billing_meter loaded from staging table 
//  stg20_ips_sm_billing_route and source target tables:
//   IPS_ASSETMANAGEMENT_WATER_COMPWMTR
//   IPS_METERMANAGEMENT_WATER_RTADDR 
//   IPS_METERMANAGEMENT_WATER_ROUTE 
//    by job sp_stg30_ips_sm_billing_meter
//  Get list of meters and routes for selected day 
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_stg30_ips_sm_billing_meter()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG30_IPS_SM_billing_meter;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.STG30_IPS_SM_BILLING_METER (UNITID, COMPKEY, ROUTEID, SERNO, UNITID_CLEAN) 
			SELECT m.UNITID, M.COMPKEY, ROUTEID, m.serno, 
			CASE WHEN position('(' IN m.UNITID)>0 THEN left(m.UNITID, position('(' IN m.UNITID)-1)
                 ELSE m.UNITID END AS UNITID_clean
			FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_COMPWMTR m
			...
			WHERE r.ROUTEKEY IN (Select ROUTEKEY 
								 FROM datahub_staging.STG20_IPS_SM_billing_route r
								 )`} );
