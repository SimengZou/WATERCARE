//  Name:       sp_stg50_ips_sm_billing_smart_read
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//   stg50_ips_sm_billing_smart_read loaded from staging table: stg40_ips_sm_billing_smart_read
//  by job sp_stg50_ips_sm_billing_smart_read
...
create or replace procedure datahub_staging.sp_stg50_ips_sm_billing_smart_read()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG50_IPS_SM_billing_smart_read;"});
...
//  Run insert statement  
        ...
        try
            {            
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_STAGING.STG50_IPS_SM_BILLING_SMART_READ 
			(UNITID, COMPKEY, ROUTEID, SM_READING, M_UNIT, READING_TYPE, READ_DATE, SM_READING_DATE, CURRENT_READ_DATE) 
			SELECT unitid, compkey, routeid, sm_reading, m_unit, reading_type, 
			read_date, sm_reading_date, current_read_date
			FROM (
				SELECT abs(sm_reading_date-read_date) AS distance, unitid, compkey, routeid, sm_reading, m_unit, reading_type, read_date, sm_reading_date, current_read_date,
				row_number() OVER (PARTITION BY unitid ORDER BY distance,sm_reading_date desc) AS most_recent_read
				FROM datahub_staging.STG40_IPS_SM_billing_smart_read)
			WHERE most_recent_read=1 ;`} );
