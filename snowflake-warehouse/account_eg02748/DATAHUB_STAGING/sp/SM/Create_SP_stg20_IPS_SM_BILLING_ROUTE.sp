//  Name:       sp_stg20_ips_sm_billing_route
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table stg20_ips_sm_billing_route loaded from staging table 
//  stg10_ips_sm_billing_readday by job sp_stg20_ips_sm_billing_readday
//  Get list of routes for the day  
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_stg20_ips_sm_billing_route()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG20_IPS_SM_billing_route;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.STG20_IPS_SM_BILLING_ROUTE (ROUTEKEY) 
					SELECT ROUTEKEY 
					  FROM DATAHUB_TARGET.IPS_METERMANAGEMENT_WATER_ROUTE r
					  WHERE RIGHT(ROUTEID,2) in 
						(SELECT NEW_READ_DAY FROM datahub_staging.STG10_IPS_SM_billing_readday)
						 AND routeid LIKE 'SMT%'
						; `} );
