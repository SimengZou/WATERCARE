CREATE OR REPLACE PROCEDURE "SP_STG_DIM_TRANSACTION_COMMENTS"()
...
//update the latest time included in extract for COMMENTS table
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for TRANSACTION_COMMENTS, TRANSACTION_COMMENTSREASON, LINEITEMSETUP  and COMMENTS tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	           
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_TRANSACTION_COMMENTS"});
...
//  Run insert statement
		...
		var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_TRANSACTION_COMMENTS ( ";		
		sql_command += " TRANSACTION_COMMENTS_KEY, ";
		sql_command += " TRANSACTION_COMMENTS, ";
		sql_command += " ETL_SOURCE_DATETIME, ";
		sql_command += " ETL_RECORD_CREATED, ";
		sql_command += " ETL_RECORD_UPDATED) ";
		sql_command += " SELECT ACCTTRAN.COMMENTSKEY AS TRANSACTION_COMMENTS_KEY, ";
        // changed from: sql_command += "   substring(COMMENTS.TEXT_SEARCH,1,2000) AS TRANSACTION_COMMENTS,";
		sql_command += "   '''' AS TRANSACTION_COMMENTS,";
		sql_command += "   max(greatest(ACCTTRAN.etl_load_datetime , coalesce(ACCTTRAN.etl_load_datetime, COMMENTS.etl_load_datetime))) AS ETL_SOURCE_DATETIME, ";
		...
		sql_command += "   INNER JOIN DATAHUB_TARGET.IPS_BILLING_COMMENTS COMMENTS ";
		sql_command += "   ON ACCTTRAN.COMMENTSKEY =COMMENTS.COMMENTSKEY";
	
	    sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB ACCTTRAN_CTRL ";
	    sql_command += "  ON ACCTTRAN_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_TRANSACTION_COMMENTS'' AND ACCTTRAN_CTRL.TABLE_NAME=''IPS_BILLING_ACCTTRAN'' AND ACCTTRAN_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB COMMENTS_CTRL ";
	    sql_command += "  ON COMMENTS_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_TRANSACTION_COMMENTS'' AND COMMENTS_CTRL.TABLE_NAME=''IPS_BILLING_COMMENTS'' AND COMMENTS_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
