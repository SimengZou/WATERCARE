CREATE OR REPLACE PROCEDURE "SP_STG_FACT_BILL_HEADER"()
...
//update the latest time included in extract for BILLRUN table	
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for BILLRUN and BILL tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_FACT_BILL_HEADER"});
...
//  Run insert statement
      ...
      var sql_command =  " INSERT INTO DATAHUB_STAGING.STG_FACT_BILL_HEADER (  ";
			...
			sql_command += " BILL.BILLRUNKEY AS BILL_RUN_KEY, ";
			sql_command += " BILL.BILLTYPEKEY AS BILL_TYPE_KEY, ";
			sql_command += " BILL.BILLDATE AS BILL_DATE, ";
			sql_command += " BILL.MODDTTM AS BILL_MODIFIED_DATE, ";
			sql_command += " BILL.ADDDTTM AS BILL_ADDED_DATE, ";
			...
			sql_command += " BILL.PRINCIPALTOTALAMOUNT AS PRINCIPAL_TOTAL_AMOUNT, ";
			sql_command += " BILL.ACTUALAMT AS BILL_ACTUAL_AMOUNT, ";
			sql_command += " BILL.BILLAMT AS BILL_AMOUNT, ";
			sql_command += " BILL.PRIORBILLAMT AS PRIOR_BILL_AMOUNT, ";
			sql_command += " GREATEST( BILL.etl_load_datetime, COALESCE(BILLRUN.etl_load_datetime, BILL.etl_load_datetime))  AS ETL_SOURCE_DATETIME, ";
			...
			sql_command += " LEFT JOIN  DATAHUB_TARGET.IPS_BILLING_BILLRUN BILLRUN";
			sql_command += "   ON BILL.BILLRUNKEY= BILLRUN.BILLRUNKEY";
			sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB BILL_CTRL";
			sql_command += "   ON BILL_CTRL.STG_PROCESS_NAME=''SP_STG_FACT_BILL_HEADER'' AND BILL_CTRL.TABLE_NAME=''IPS_BILLING_BILL'' AND BILL_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
			sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB BILLRUN_CTRL ";
			sql_command += "   ON BILLRUN_CTRL.STG_PROCESS_NAME=''SP_STG_FACT_BILL_HEADER'' AND BILLRUN_CTRL.TABLE_NAME=''IPS_BILLING_BILLRUN'' AND BILLRUN_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
			sql_command += " WHERE  BILL.ETL_LOAD_DATETIME>BILL_CTRL.INC_ETL_LOAD_DATETIME";
			sql_command += " OR  BILLRUN.ETL_LOAD_DATETIME>BILLRUN_CTRL.INC_ETL_LOAD_DATETIME;";
