//  Name:       sp_dw_ips_stage_consump_stg6a
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg6a loaded from staging table dw_ips_stage_consump_stg6 by job sp_dw_ips_stage_consump_stg6a
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg6a()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG6a;"});
...
//  Run insert statement  				
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG6a	
		 (ACCOUNT_KEY, ACCOUNT_SERVICE_KEY, ADDRESS_KEY, START_DATETIME, STOP_DATETIME, CONSUMPTION_PERCENTAGE, SUBTRACTIVE_FLAG, COMP_KEY, COMP_READING_KEY, 
		 ROWNUM, PREV_READ_DATE, READ_DATE, CAL_MONTH, MONTH_START_DATE, MONTH_END_DATE, AVG_DAILY_USAGE, ACTUAL_DAYS, FULL_MONTH_FLAG)
		 select distinct
		   account_key, account_service_key, address_key, start_datetime, stop_datetime, consumption_percentage, subtractive_flag,
		   comp_key, comp_reading_key, rownum, prev_read_date, read_date, 
		   cal_month, calendar_date as month_start_date, dateadd(day, -1, dateadd(mon, 1, calendar_date)) as month_end_date, 
		   avg_daily_usage, datediff(day, start_dt, stop_dt) as actual_days, full_month_flag
		 from datahub_staging.dw_ips_stage_consump_stg6;
`} );
