//  Name:       sp_dw_ips_stage_consump_stg4a
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg4a loaded from staging table dw_ips_stage_consump_stg4 by job sp_dw_ips_stage_consump_stg4a
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg4a()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG4a;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG4a
		 (COMP_KEY, COMP_READING_KEY, ROWNUM, PREV_READ_DATE, READ_DATE, READING, usage, READING_DAYS, REPORTING_USAGE)
		 SELECT DISTINCT
		   a.comp_key, a.comp_reading_key, a.rownum, a.prev_read_date, a.read_date, a.reading, a.usage, a.reading_days,
		   case when a.usage >=0 and PrevReading.usage is null then a.usage
		        when a.usage >=0 and PrevReading.usage >=0 then a.usage
		       When a.usage <0 and PrevReading.usage >0 then a.usage + cast(PrevReading.usage as int)
		        when a.usage >0 and PrevReading.usage <0 then a.usage + cast(PrevReading.usage as int)
		        when a.reading_days = 2 then 0
		        ELSE 0
		        END as reporting_usage
		 FROM datahub_staging.dw_ips_stage_consump_stg4 a
		 LEFT OUTER JOIN datahub_staging.dw_ips_stage_consump_stg4 PrevReading 
		   ON a.comp_key = PrevReading.comp_key 
		      AND (a.rownum -1) = PrevReading.rownum ;`} );
            var rs = sqlStmt.execute();
            var number_of_rows_inserted = sqlStmt.getNumRowsInserted();
            var number_of_rows_updated =  sqlStmt.getNumRowsUpdated(); 
			snowflake.execute({sqlText: `update datahub_staging.dw_ips_stage_consump_stg4a  set reporting_usage=0 
            where reporting_usage < 0 ;`});
