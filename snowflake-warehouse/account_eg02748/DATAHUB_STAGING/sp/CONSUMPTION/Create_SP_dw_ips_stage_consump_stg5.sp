//  Name:       sp_dw_ips_stage_consump_stg5
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg5 loaded from staging table dw_ips_stage_consump_stg4a by job sp_dw_ips_stage_consump_stg5
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg5()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG5;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG5
		 (COMP_KEY, COMP_READING_KEY, ROWNUM, PREV_READ_DATE, READ_DATE, READING, USAGE, READING_DAYS, REPORTING_USAGE, AVG_DAILY_USAGE)
		 SELECT DISTINCT
		   comp_key, comp_reading_key, rownum, prev_read_date, read_date, 
		   reading, usage, reading_days, reporting_usage,
		   CASE WHEN reading_days IS NULL OR reading_days = 0 THEN NULL
		         ELSE cast( reporting_usage as decimal (19,9))/reading_days END as avg_daily_usage
		 FROM datahub_staging.dw_ips_stage_consump_stg4a;`} );
