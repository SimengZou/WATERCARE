//  Name:       sp_dw_ips_stage_consump_stg5a
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg5a loaded from staging table dw_ips_stage_consump_stg5 by job sp_dw_ips_stage_consump_stg5a
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg5a()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG5a;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG5a
		 (ACCOUNT_KEY, ACCOUNT_SERVICE_KEY, ADDRESS_KEY, START_DATETIME, STOP_DATETIME, SERVICE_STATUS, CONSUMPTION_PERCENTAGE, 
		 SUBTRACTIVE_FLAG, COMP_KEY, COMP_READING_KEY, ROWNUM, PREV_READ_DATE, READ_DATE, READING, USAGE, READING_DAYS, REPORTING_USAGE, AVG_DAILY_USAGE)
		 SELECT  
		   a.account_key, s.account_service_key, asl.address_key, s.start_datetime, s.stop_datetime, 
		   case when s.start_datetime is null then null      
		           when s.stop_datetime is not null then 'INACTV' else 'ACTIVE' end service_status,
		   p.consumption_percentage, p.subtractive_flag,
		   stg5.comp_key, stg5.comp_reading_key, stg5.rownum, stg5.prev_read_date, stg5.read_date, stg5.reading, stg5.usage, 
		   stg5.reading_days, stg5.reporting_usage, avg_daily_usage   
		 FROM datahub_staging.dw_ips_stage_consump_stg5 stg5 
		 INNER JOIN datahub_analytics.dw_ips_dim_assloc asl ON asl.comp_key = stg5.comp_key 
		 INNER JOIN datahub_analytics.dw_ips_dim_accountserviceposition p  ON p.address_key = asl.address_key AND p.position = asl.position
		 INNER JOIN datahub_analytics.dw_ips_dim_accountservices s  ON s.account_service_key = p.account_service_key
		 INNER JOIN datahub_analytics.dw_ips_dim_account a  ON a.account_key = s.account_key
		 INNER JOIN datahub_analytics.dw_ips_dim_serviceoptions so  ON s.service_options_key = so.service_options_key
		 WHERE
		 CAST(stg5.read_date AS DATE) > CAST(s.start_datetime AS DATE)   AND CAST(stg5.read_date AS DATE) <= CAST(coalesce(s.stop_datetime, to_date('31-Dec-9999','DD-Mon-YYYY')) AS DATE)
		 AND trim(upper(so.code)) in ('WATER', 'TANKER');`} );
