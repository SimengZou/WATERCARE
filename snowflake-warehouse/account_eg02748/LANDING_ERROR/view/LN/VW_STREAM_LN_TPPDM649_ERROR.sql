CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPDM649_ERROR AS SELECT src, 'LN_TPPDM649' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:vldf), '1900-01-01')) is null and 
                    src:vldf is not null) THEN
                    'vldf contains a non-timestamp value : \'' || AS_VARCHAR(src:vldf) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:vldt), '1900-01-01')) is null and 
                    src:vldt is not null) THEN
                    'vldt contains a non-timestamp value : \'' || AS_VARCHAR(src:vldt) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:vldf), '1900-01-01')) is null and 
                    src:vldf is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:vldt), '1900-01-01')) is null and 
                    src:vldt is not null) or 
                (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:sequencenumber), '1900-01-01')) is null and 
