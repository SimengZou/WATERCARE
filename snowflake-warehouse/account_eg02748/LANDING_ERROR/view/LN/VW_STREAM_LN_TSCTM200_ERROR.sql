CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TSCTM200_ERROR AS SELECT src, 'LN_TSCTM200' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:psrc_ref_compnr), '0'), 38, 10) is null and 
                    src:psrc_ref_compnr is not null) THEN
                    'psrc_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:psrc_ref_compnr) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:qudt), '1900-01-01')) is null and 
                    src:qudt is not null) THEN
                    'qudt contains a non-timestamp value : \'' || AS_VARCHAR(src:qudt) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:qxdt), '1900-01-01')) is null and 
                    src:qxdt is not null) THEN
                    'qxdt contains a non-timestamp value : \'' || AS_VARCHAR(src:qxdt) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:squo), '0'), 38, 10) is null and 
                    src:squo is not null) THEN
                    'squo contains a non-numeric value : \'' || AS_VARCHAR(src:squo) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:supe), '0'), 38, 10) is null and 
                    src:supe is not null) THEN
                    'supe contains a non-numeric value : \'' || AS_VARCHAR(src:supe) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:psrc_ref_compnr), '0'), 38, 10) is null and 
                    src:psrc_ref_compnr is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:qudt), '1900-01-01')) is null and 
                    src:qudt is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:qxdt), '1900-01-01')) is null and 
                    src:qxdt is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ratc_1), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:squo), '0'), 38, 10) is null and 
                    src:squo is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:supe), '0'), 38, 10) is null and 
                    src:supe is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:term), '0'), 38, 10) is null and 
