CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TCPPL040_ERROR AS SELECT src, 'LN_TCPPL040' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spvr_ref_compnr), '0'), 38, 10) is null and 
                    src:spvr_ref_compnr is not null) THEN
                    'spvr_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:spvr_ref_compnr) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spvr_ref_compnr), '0'), 38, 10) is null and 
                    src:spvr_ref_compnr is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:timestamp), '1900-01-01')) is null and 
