CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TFGLD005_ERROR AS SELECT src, 'LN_TFGLD005' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:compnr), '0'), 38, 10) is null and 
                    src:compnr is not null) THEN
                    'compnr contains a non-numeric value : \'' || AS_VARCHAR(src:compnr) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:corr), '0'), 38, 10) is null and 
                    src:corr is not null) THEN
                    'corr contains a non-numeric value : \'' || AS_VARCHAR(src:corr) || '\'' WHEN 
