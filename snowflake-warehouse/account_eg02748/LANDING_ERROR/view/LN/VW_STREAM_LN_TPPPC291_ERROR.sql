CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPPC291_ERROR AS SELECT src, 'LN_TPPPC291' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hemn_ref_compnr), '0'), 38, 10) is null and 
                    src:hemn_ref_compnr is not null) THEN
                    'hemn_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:hemn_ref_compnr) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hsrn), '0'), 38, 10) is null and 
                    src:hsrn is not null) THEN
                    'hsrn contains a non-numeric value : \'' || AS_VARCHAR(src:hsrn) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:teti), '0'), 38, 10) is null and 
                    src:teti is not null) THEN
                    'teti contains a non-numeric value : \'' || AS_VARCHAR(src:teti) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPPC291 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:amoc), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hemn_ref_compnr), '0'), 38, 10) is null and 
                    src:hemn_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hper), '0'), 38, 10) is null and 
                    src:hper is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hsrn), '0'), 38, 10) is null and 
                    src:hsrn is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hyea), '0'), 38, 10) is null and 
