CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPTC147_ERROR AS SELECT src, 'LN_TPPTC147' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrt), '0'), 38, 10) is null and 
                    src:dfrt is not null) THEN
                    'dfrt contains a non-numeric value : \'' || AS_VARCHAR(src:dfrt) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eqan), '0'), 38, 10) is null and 
                    src:eqan is not null) THEN
                    'eqan contains a non-numeric value : \'' || AS_VARCHAR(src:eqan) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eqty), '0'), 38, 10) is null and 
                    src:eqty is not null) THEN
                    'eqty contains a non-numeric value : \'' || AS_VARCHAR(src:eqty) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrt), '0'), 38, 10) is null and 
                    src:dfrt is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eqan), '0'), 38, 10) is null and 
                    src:eqan is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eqty), '0'), 38, 10) is null and 
                    src:eqty is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:exep), '0'), 38, 10) is null and 
