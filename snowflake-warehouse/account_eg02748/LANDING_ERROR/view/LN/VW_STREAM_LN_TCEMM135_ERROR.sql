CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TCEMM135_ERROR AS SELECT src, 'LN_TCEMM135' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:extl), '0'), 38, 10) is null and 
                    src:extl is not null) THEN
                    'extl contains a non-numeric value : \'' || AS_VARCHAR(src:extl) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hsub), '0'), 38, 10) is null and 
                    src:hsub is not null) THEN
                    'hsub contains a non-numeric value : \'' || AS_VARCHAR(src:hsub) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:extl), '0'), 38, 10) is null and 
                    src:extl is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:hsub), '0'), 38, 10) is null and 
                    src:hsub is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ofbp_ref_compnr), '0'), 38, 10) is null and 
