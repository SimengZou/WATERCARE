CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5CIWORESULTS_ERROR AS SELECT src, 'EAM_U5CIWORESULTS' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIR_OBSDATE), '1900-01-01')) is null and 
                    src:CIR_OBSDATE is not null) THEN
                    'CIR_OBSDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:CIR_OBSDATE) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CIR_SEQUENCE), '0'), 38, 10) is null and 
                    src:CIR_SEQUENCE is not null) THEN
                    'CIR_SEQUENCE contains a non-numeric value : \'' || AS_VARCHAR(src:CIR_SEQUENCE) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIR_OBSDATE), '1900-01-01')) is null and 
                    src:CIR_OBSDATE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CIR_SEQUENCE), '0'), 38, 10) is null and 
                    src:CIR_SEQUENCE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CREATED), '1900-01-01')) is null and 
