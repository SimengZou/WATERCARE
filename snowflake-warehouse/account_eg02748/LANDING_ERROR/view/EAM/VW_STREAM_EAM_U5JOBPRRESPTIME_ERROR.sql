CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5JOBPRRESPTIME_ERROR AS SELECT src, 'EAM_U5JOBPRRESPTIME' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WOCOMPLETIONTIME), '0'), 38, 10) is null and 
                    src:JBP_WOCOMPLETIONTIME is not null) THEN
                    'JBP_WOCOMPLETIONTIME contains a non-numeric value : \'' || AS_VARCHAR(src:JBP_WOCOMPLETIONTIME) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WORESOLUTIONTIME), '0'), 38, 10) is null and 
                    src:JBP_WORESOLUTIONTIME is not null) THEN
                    'JBP_WORESOLUTIONTIME contains a non-numeric value : \'' || AS_VARCHAR(src:JBP_WORESOLUTIONTIME) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WORESPONSETIME), '0'), 38, 10) is null and 
                    src:JBP_WORESPONSETIME is not null) THEN
                    'JBP_WORESPONSETIME contains a non-numeric value : \'' || AS_VARCHAR(src:JBP_WORESPONSETIME) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WOCOMPLETIONTIME), '0'), 38, 10) is null and 
                    src:JBP_WOCOMPLETIONTIME is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WORESOLUTIONTIME), '0'), 38, 10) is null and 
                    src:JBP_WORESOLUTIONTIME is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:JBP_WORESPONSETIME), '0'), 38, 10) is null and 
                    src:JBP_WORESPONSETIME is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:LASTSAVED), '1900-01-01')) is null and 
