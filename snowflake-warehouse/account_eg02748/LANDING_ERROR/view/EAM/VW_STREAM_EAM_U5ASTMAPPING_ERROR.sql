CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5ASTMAPPING_ERROR AS SELECT src, 'EAM_U5ASTMAPPING' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AUTOID), '0'), 38, 10) is null and 
                    src:AUTOID is not null) THEN
                    'AUTOID contains a non-numeric value : \'' || AS_VARCHAR(src:AUTOID) || '\'' WHEN 
