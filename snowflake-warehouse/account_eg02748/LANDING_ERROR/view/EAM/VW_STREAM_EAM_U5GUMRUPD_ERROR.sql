CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5GUMRUPD_ERROR AS SELECT src, 'EAM_U5GUMRUPD' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MRU_UPDATEDATE), '1900-01-01')) is null and 
                    src:MRU_UPDATEDATE is not null) THEN
                    'MRU_UPDATEDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:MRU_UPDATEDATE) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MRU_UPDATEDATE), '1900-01-01')) is null and 
                    src:MRU_UPDATEDATE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:UPDATECOUNT), '0'), 38, 10) is null and 
