CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5SERVICEREQUEST_ERROR AS SELECT src, 'EAM_U5SERVICEREQUEST' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CRM_REPORTEDDATE), '1900-01-01')) is null and 
                    src:CRM_REPORTEDDATE is not null) THEN
                    'CRM_REPORTEDDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:CRM_REPORTEDDATE) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_EAM_U5SERVICEREQUEST as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CREATED), '1900-01-01')) is null and 
                    src:CREATED is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CRM_REPORTEDDATE), '1900-01-01')) is null and 
                    src:CRM_REPORTEDDATE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:LASTSAVED), '1900-01-01')) is null and 
