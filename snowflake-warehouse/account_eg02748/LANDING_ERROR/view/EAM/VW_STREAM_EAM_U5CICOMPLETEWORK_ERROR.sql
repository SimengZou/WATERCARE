CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5CICOMPLETEWORK_ERROR AS SELECT src, 'EAM_U5CICOMPLETEWORK' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_COMPLETEDDATE), '1900-01-01')) is null and 
                    src:CIC_COMPLETEDDATE is not null) THEN
                    'CIC_COMPLETEDDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:CIC_COMPLETEDDATE) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_INITIATEDDATE), '1900-01-01')) is null and 
                    src:CIC_INITIATEDDATE is not null) THEN
                    'CIC_INITIATEDDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:CIC_INITIATEDDATE) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_STARTDATE), '1900-01-01')) is null and 
                    src:CIC_STARTDATE is not null) THEN
                    'CIC_STARTDATE contains a non-timestamp value : \'' || AS_VARCHAR(src:CIC_STARTDATE) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_COMPLETEDDATE), '1900-01-01')) is null and 
                    src:CIC_COMPLETEDDATE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_INITIATEDDATE), '1900-01-01')) is null and 
                    src:CIC_INITIATEDDATE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CIC_STARTDATE), '1900-01-01')) is null and 
                    src:CIC_STARTDATE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CREATED), '1900-01-01')) is null and 
