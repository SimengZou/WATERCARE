CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AREALEN), '0'), 38, 10) is null and 
                    src:AREALEN is not null) THEN
                    'AREALEN contains a non-numeric value : \'' || AS_VARCHAR(src:AREALEN) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AREAWID), '0'), 38, 10) is null and 
                    src:AREAWID is not null) THEN
                    'AREAWID contains a non-numeric value : \'' || AS_VARCHAR(src:AREAWID) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AREALEN), '0'), 38, 10) is null and 
                    src:AREALEN is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AREASZ), '0'), 38, 10) is null and 
                    src:AREASZ is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AREAWID), '0'), 38, 10) is null and 
                    src:AREAWID is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BLDGFLOOR), '0'), 38, 10) is null and 
