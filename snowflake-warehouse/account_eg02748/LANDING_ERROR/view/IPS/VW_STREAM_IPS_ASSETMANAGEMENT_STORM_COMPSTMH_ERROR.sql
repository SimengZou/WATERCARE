CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_STORM_COMPSTMH_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_STORM_COMPSTMH' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BARLDIAM), '0'), 38, 10) is null and 
                    src:BARLDIAM is not null) THEN
                    'BARLDIAM contains a non-numeric value : \'' || AS_VARCHAR(src:BARLDIAM) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CVRDIAM), '0'), 38, 10) is null and 
                    src:CVRDIAM is not null) THEN
                    'CVRDIAM contains a non-numeric value : \'' || AS_VARCHAR(src:CVRDIAM) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HYDIST), '0'), 38, 10) is null and 
                    src:HYDIST is not null) THEN
                    'HYDIST contains a non-numeric value : \'' || AS_VARCHAR(src:HYDIST) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:MHDPTH), '0'), 38, 10) is null and 
                    src:MHDPTH is not null) THEN
                    'MHDPTH contains a non-numeric value : \'' || AS_VARCHAR(src:MHDPTH) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_STORM_COMPSTMH as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
