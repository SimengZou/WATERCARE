CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_BILLING_BILLTYPELINEITEM_ERROR AS SELECT src, 'IPS_BILLING_BILLTYPELINEITEM' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    src:ADDDTTM is not null) THEN
                    'ADDDTTM contains a non-timestamp value : \'' || AS_VARCHAR(src:ADDDTTM) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BILLTYPELINEITEMKEY), '0'), 38, 10) is null and 
                    src:BILLTYPELINEITEMKEY is not null) THEN
                    'BILLTYPELINEITEMKEY contains a non-numeric value : \'' || AS_VARCHAR(src:BILLTYPELINEITEMKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BILLTYPELINEITEMKEY), '0'), 38, 10) is null and 
                    src:BILLTYPELINEITEMKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CALCULATIONORDER), '0'), 38, 10) is null and 
