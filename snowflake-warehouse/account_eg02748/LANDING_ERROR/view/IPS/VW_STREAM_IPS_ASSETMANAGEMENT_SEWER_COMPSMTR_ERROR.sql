CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSMTR_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_SEWER_COMPSMTR' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:COMPONENT), '0'), 38, 10) is null and 
                    src:COMPONENT is not null) THEN
                    'COMPONENT contains a non-numeric value : \'' || AS_VARCHAR(src:COMPONENT) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HIGHREGISTER), '0'), 38, 10) is null and 
                    src:HIGHREGISTER is not null) THEN
                    'HIGHREGISTER contains a non-numeric value : \'' || AS_VARCHAR(src:HIGHREGISTER) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWREGISTER), '0'), 38, 10) is null and 
                    src:LOWREGISTER is not null) THEN
                    'LOWREGISTER contains a non-numeric value : \'' || AS_VARCHAR(src:LOWREGISTER) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:METERSIZE), '0'), 38, 10) is null and 
                    src:METERSIZE is not null) THEN
                    'METERSIZE contains a non-numeric value : \'' || AS_VARCHAR(src:METERSIZE) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OVFLELEV), '0'), 38, 10) is null and 
                    src:OVFLELEV is not null) THEN
                    'OVFLELEV contains a non-numeric value : \'' || AS_VARCHAR(src:OVFLELEV) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PIPESIZE), '0'), 38, 10) is null and 
                    src:PIPESIZE is not null) THEN
                    'PIPESIZE contains a non-numeric value : \'' || AS_VARCHAR(src:PIPESIZE) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WETWLOVER), '0'), 38, 10) is null and 
                    src:WETWLOVER is not null) THEN
                    'WETWLOVER contains a non-numeric value : \'' || AS_VARCHAR(src:WETWLOVER) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HIGHREGISTER), '0'), 38, 10) is null and 
                    src:HIGHREGISTER is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:INSTDATE), '1900-01-01')) is null and 
                    src:INSTDATE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:INTKEY), '0'), 38, 10) is null and 
                    src:INTKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWREGISTER), '0'), 38, 10) is null and 
                    src:LOWREGISTER is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:MAINKEY), '0'), 38, 10) is null and 
                    src:MAINKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:METERSIZE), '0'), 38, 10) is null and 
                    src:METERSIZE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MFGDATE), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OVFLELEV), '0'), 38, 10) is null and 
                    src:OVFLELEV is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PIPESIZE), '0'), 38, 10) is null and 
                    src:PIPESIZE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:POSITION), '0'), 38, 10) is null and 
