CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_WSLCDRBUILD_XBUILDFEESGD_ERROR AS SELECT src, 'IPS_WSLCDRBUILD_XBUILDFEESGD' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XBUILDFEESGDKEY), '0'), 38, 10) is null and 
                    src:XBUILDFEESGDKEY is not null) THEN
                    'XBUILDFEESGDKEY contains a non-numeric value : \'' || AS_VARCHAR(src:XBUILDFEESGDKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XBUILDFEESGDKEY), '0'), 38, 10) is null and 
                    src:XBUILDFEESGDKEY is not null) or 
                (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:VARIATION_ID), '1900-01-01')) is null and 
