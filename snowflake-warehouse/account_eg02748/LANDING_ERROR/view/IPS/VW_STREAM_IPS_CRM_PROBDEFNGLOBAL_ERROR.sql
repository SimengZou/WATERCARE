CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CRM_PROBDEFNGLOBAL_ERROR AS SELECT src, 'IPS_CRM_PROBDEFNGLOBAL' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROBDEFNGLOBALKEY), '0'), 38, 10) is null and 
                    src:PROBDEFNGLOBALKEY is not null) THEN
                    'PROBDEFNGLOBALKEY contains a non-numeric value : \'' || AS_VARCHAR(src:PROBDEFNGLOBALKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROBDEFNGLOBALKEY), '0'), 38, 10) is null and 
                    src:PROBDEFNGLOBALKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SEARCHRADIUS), '0'), 38, 10) is null and 
