CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_FLEET_COMPVEH_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_FLEET_COMPVEH' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CYLINDERS), '0'), 38, 10) is null and 
                    src:CYLINDERS is not null) THEN
                    'CYLINDERS contains a non-numeric value : \'' || AS_VARCHAR(src:CYLINDERS) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NOTIRES), '0'), 38, 10) is null and 
                    src:NOTIRES is not null) THEN
                    'NOTIRES contains a non-numeric value : \'' || AS_VARCHAR(src:NOTIRES) || '\'' WHEN 
