CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_WSLCRM_XACTIVITYAUDITGD_ERROR AS SELECT src, 'IPS_WSLCRM_XACTIVITYAUDITGD' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XACTIVITYAUDITDPKEY), '0'), 38, 10) is null and 
                    src:XACTIVITYAUDITDPKEY is not null) THEN
                    'XACTIVITYAUDITDPKEY contains a non-numeric value : \'' || AS_VARCHAR(src:XACTIVITYAUDITDPKEY) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XACTIVITYAUDITGDKEY), '0'), 38, 10) is null and 
                    src:XACTIVITYAUDITGDKEY is not null) THEN
                    'XACTIVITYAUDITGDKEY contains a non-numeric value : \'' || AS_VARCHAR(src:XACTIVITYAUDITGDKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XACTIVITYAUDITDPKEY), '0'), 38, 10) is null and 
                    src:XACTIVITYAUDITDPKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XACTIVITYAUDITGDKEY), '0'), 38, 10) is null and 
                    src:XACTIVITYAUDITGDKEY is not null) or 
                (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:VARIATION_ID), '1900-01-01')) is null and 
