CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CASHIERING_CASHREGISTER_ERROR AS SELECT src, 'IPS_CASHIERING_CASHREGISTER' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PAYMENTTERMINALFORMULAKEY), '0'), 38, 10) is null and 
                    src:PAYMENTTERMINALFORMULAKEY is not null) THEN
                    'PAYMENTTERMINALFORMULAKEY contains a non-numeric value : \'' || AS_VARCHAR(src:PAYMENTTERMINALFORMULAKEY) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:RECEIPTCOPIES), '0'), 38, 10) is null and 
                    src:RECEIPTCOPIES is not null) THEN
                    'RECEIPTCOPIES contains a non-numeric value : \'' || AS_VARCHAR(src:RECEIPTCOPIES) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SCANNERFORMULAKEY), '0'), 38, 10) is null and 
                    src:SCANNERFORMULAKEY is not null) THEN
                    'SCANNERFORMULAKEY contains a non-numeric value : \'' || AS_VARCHAR(src:SCANNERFORMULAKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PAYMENTTERMINALFORMULAKEY), '0'), 38, 10) is null and 
                    src:PAYMENTTERMINALFORMULAKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:RECEIPTCOPIES), '0'), 38, 10) is null and 
                    src:RECEIPTCOPIES is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:REGISTERGROUPSETUPKEY), '0'), 38, 10) is null and 
                    src:REGISTERGROUPSETUPKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:REGKEY), '0'), 38, 10) is null and 
                    src:REGKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SCANNERFORMULAKEY), '0'), 38, 10) is null and 
                    src:SCANNERFORMULAKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:VARIATION_ID), '0'), 38, 10) is null and 
