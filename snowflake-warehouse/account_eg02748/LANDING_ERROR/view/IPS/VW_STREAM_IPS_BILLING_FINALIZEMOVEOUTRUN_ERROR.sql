CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_BILLING_FINALIZEMOVEOUTRUN_ERROR AS SELECT src, 'IPS_BILLING_FINALIZEMOVEOUTRUN' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FINALIZEMOVEOUTRUNKEY), '0'), 38, 10) is null and 
                    src:FINALIZEMOVEOUTRUNKEY is not null) THEN
                    'FINALIZEMOVEOUTRUNKEY contains a non-numeric value : \'' || AS_VARCHAR(src:FINALIZEMOVEOUTRUNKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FINALIZEMOVEOUTRUNKEY), '0'), 38, 10) is null and 
                    src:FINALIZEMOVEOUTRUNKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LASTINVOCATIONSTATUS), '0'), 38, 10) is null and 
