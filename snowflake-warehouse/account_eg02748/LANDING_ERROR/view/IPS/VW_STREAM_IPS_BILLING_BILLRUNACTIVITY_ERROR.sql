CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_BILLING_BILLRUNACTIVITY_ERROR AS SELECT src, 'IPS_BILLING_BILLRUNACTIVITY' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AVERAGEBILLSPERMINUTE), '0'), 38, 10) is null and 
                    src:AVERAGEBILLSPERMINUTE is not null) THEN
                    'AVERAGEBILLSPERMINUTE contains a non-numeric value : \'' || AS_VARCHAR(src:AVERAGEBILLSPERMINUTE) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BILLRUNACTIVITYKEY), '0'), 38, 10) is null and 
                    src:BILLRUNACTIVITYKEY is not null) THEN
                    'BILLRUNACTIVITYKEY contains a non-numeric value : \'' || AS_VARCHAR(src:BILLRUNACTIVITYKEY) || '\'' WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:BILLRUNACTIVITYKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_BILLING_BILLRUNACTIVITY as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ACTIVITYTYPE), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:AVERAGEBILLSPERMINUTE), '0'), 38, 10) is null and 
                    src:AVERAGEBILLSPERMINUTE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BILLRUNACTIVITYKEY), '0'), 38, 10) is null and 
                    src:BILLRUNACTIVITYKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BILLRUNKEY), '0'), 38, 10) is null and 
