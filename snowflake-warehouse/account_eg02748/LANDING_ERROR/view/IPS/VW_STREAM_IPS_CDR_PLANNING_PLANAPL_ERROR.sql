CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PLANNING_PLANAPL_ERROR AS SELECT src, 'IPS_CDR_PLANNING_PLANAPL' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APPLANAPLKEY), '0'), 38, 10) is null and 
                    src:APPLANAPLKEY is not null) THEN
                    'APPLANAPLKEY contains a non-numeric value : \'' || AS_VARCHAR(src:APPLANAPLKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APPLANAPLKEY), '0'), 38, 10) is null and 
                    src:APPLANAPLKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APPLANKEY), '0'), 38, 10) is null and 
