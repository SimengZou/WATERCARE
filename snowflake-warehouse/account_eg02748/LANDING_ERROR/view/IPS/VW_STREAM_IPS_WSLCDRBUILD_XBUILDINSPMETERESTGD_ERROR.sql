CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_WSLCDRBUILD_XBUILDINSPMETERESTGD_ERROR AS SELECT src, 'IPS_WSLCDRBUILD_XBUILDINSPMETERESTGD' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XBUILDINSPMETERESTGDKEY), '0'), 38, 10) is null and 
                    src:XBUILDINSPMETERESTGDKEY is not null) THEN
                    'XBUILDINSPMETERESTGDKEY contains a non-numeric value : \'' || AS_VARCHAR(src:XBUILDINSPMETERESTGDKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XBUILDINSPMETERESTGDKEY), '0'), 38, 10) is null and 
                    src:XBUILDINSPMETERESTGDKEY is not null) or 
                (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:VARIATION_ID), '1900-01-01')) is null and 
