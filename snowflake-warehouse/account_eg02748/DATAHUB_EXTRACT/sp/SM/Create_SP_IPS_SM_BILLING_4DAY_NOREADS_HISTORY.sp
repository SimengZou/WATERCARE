//  Name:       sp_ips_sm_billing_4day_noreads_history
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - permanent table 
//   ips_sm_billing_4day_noreads loaded from staging table:  datahub_staging.stg55_ips_sm_billing_4day_noreads
//  by job sp_ips_sm_billing_4day_noreads_history
//  Stores meters where no reading was found in smart meters within 4 days of the expected read - keeps historical no reads for 180 days
//
//  Description:
//
...
//
//  4.  Delete old records as per delete_days 
//
...
create or replace procedure datahub_extract.sp_ips_sm_billing_4day_noreads_history(DELETE_DAYS FLOAT)
    returns string
    language javascript
    execute as caller
    as
...
//  Delete old records as per delete_days 

        
        snowflake.execute ({sqlText: `DELETE FROM DATAHUB_EXTRACT.IPS_SM_BILLING_4DAY_NOREADS_HISTORY
			WHERE dateadd(DAY,` + DELETE_DAYS.toString() + `, to_date(ETL_LOAD_DATETIME))<= convert_timezone('UTC', 'Pacific/Auckland', CURRENT_TIMESTAMP());`}); 
...
//  Run insert statement  
        ...
        try
            {            
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_EXTRACT.IPS_SM_BILLING_4DAY_NOREADS_HISTORY 
			(UNITID, COMPKEY, ROUTEID, SM_READING, M_UNIT, READING_TYPE, READ_DATE, SM_READING_DATE, EXTRACT_READDATE, ETL_LOAD_DATETIME) 
				SELECT UNITID, COMPKEY, ROUTEID, SM_READING, M_UNIT, READING_TYPE, READ_DATE, SM_READING_DATE, EXTRACT_READDATE, ETL_LOAD_DATETIME
				FROM DATAHUB_STAGING.STG55_IPS_SM_BILLING_4DAY_NOREADS; `} );
