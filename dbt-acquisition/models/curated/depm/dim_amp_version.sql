...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['WorkflowEnabled'], type='varchar', alias='workflowenabled') }},
        {{ trx_json_extract('src', ['StartYear'], type='varchar', alias='startyear') }},
        {{ trx_json_extract('src', ['Timestamp'], type='timestamp_ntz', alias='timestamp') }},
        {{ trx_etl_columns() }}
    from {{ source('source_landing_depm', 'depm_dim_amp_version') }}
)
