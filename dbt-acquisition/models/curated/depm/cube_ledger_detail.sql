...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['LedgerDetailLineNo'], type='varchar', alias='ledgerdetaillineno') }},
        {{ trx_json_extract('src', ['LedgerDetailMeas'], type='varchar', alias='ledgerdetailmeas') }},
        {{ trx_json_extract('src', ['Value'], type='varchar', alias='value') }},
        {{ trx_json_extract('src', ['Timestamp'], type='timestamp_ntz', alias='timestamp') }},
        {{ trx_etl_columns() }}
    from {{ source('source_landing_depm', 'depm_cube_ledger_detail') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['version', 'year', 'period', 'ldgtrxtype', 'currency', 'analysis01', 'analysis02', 'analysis03', 'analysis04', 'analysis05', 'analysis06', 'analysis07', 'analysis08', 'analysis09', 'analysis10', '_account', 'ledgerdetaillineno', 'ledgerdetailmeas']) }}
from cte_flattened
{% if is_incremental() %}
