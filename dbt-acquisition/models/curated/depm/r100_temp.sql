{{ config(
    ...
    description='Generated from anySQL Connector dEPM_R100_Temp',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_depm', 'depm_r100_temp') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['version', 'project', 'wbs', 'riskid', 'riskactionid']) }}
from cte_flattened
{% if is_incremental() %}
