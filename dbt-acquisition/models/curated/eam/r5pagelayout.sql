...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_eam', 'eam_r5pagelayout') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['plo_usergroup', 'plo_pagename', 'plo_elementid', 'plo_meflag']) }}
from cte_flattened
{% if is_incremental() %}
