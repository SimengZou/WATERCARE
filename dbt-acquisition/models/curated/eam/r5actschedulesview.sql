{{ config(
    ...
    description='eam_R5ACTSCHEDULESVIEW',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['ACS_STARTTIME'], type='varchar', alias='acs_starttime') }},
        {{ trx_json_extract('src', ['ACS_ENDTIME'], type='varchar', alias='acs_endtime') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_eam', 'eam_r5actschedulesview') }}
)
