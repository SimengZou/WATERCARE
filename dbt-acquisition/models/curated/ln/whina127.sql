{{ config(
    ...
    description='LN whina127 Inventory Receipt Transaction Variance Cost Details table, Generated 2026-04-10T19:42:47Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['item_cwar_trdt_seqn_inwp_vpdt_vseq_ref_compnr'], type='int', alias='item_cwar_trdt_seqn_inwp_vpdt_vseq_ref_compnr') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_whina127') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'item', 'cwar', 'trdt', 'seqn', 'inwp', 'vpdt', 'vseq', 'cpcp']) }}
from cte_flattened
{% if is_incremental() %}
