{{ config(
    ...
    description='LN tppdm035 Subcontracting table, Generated 2026-04-10T19:41:55Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['cdf_sctg', 'en_US'], type='varchar', alias='cdf_sctg__en_us') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tppdm035') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'csub']) }}
from cte_flattened
{% if is_incremental() %}
