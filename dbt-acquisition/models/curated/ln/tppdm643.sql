{{ config(
    ...
    description='LN tppdm643 Project Revenues table, Generated 2026-04-10T19:42:01Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tppdm643') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'cpro']) }}
from cte_flattened
{% if is_incremental() %}
