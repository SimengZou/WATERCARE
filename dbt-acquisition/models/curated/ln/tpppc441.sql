{{ config(
    ...
    description='LN tpppc441 Cost Control by Project / Activity / Cost Type table, Generated 2026-04-10T19:42:15Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc441') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'mpto', 'cpla', 'cact', 'cotp']) }}
from cte_flattened
{% if is_incremental() %}
