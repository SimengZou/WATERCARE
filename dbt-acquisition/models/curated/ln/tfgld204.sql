{{ config(
    ...
    description='LN tfgld204 Opening Balances - Dimension/Ledger/Currency table, Generated 2026-04-10T19:41:44Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tfgld204') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cono', 'year', 'dtyp', 'dimx', 'leac', 'ccur', 'duac', 'bpid']) }}
from cte_flattened
{% if is_incremental() %}
