{{ config(
    ...
    description='LN tppdm625 Project Equipment table, Generated 2026-04-10T19:42:00Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['dfeq'], type='varchar', alias='dfeq') }},
        ...
        {{ trx_json_extract('src', ['dfrp'], type='int', alias='dfrp') }},
        {{ trx_json_extract('src', ['dfrp_kw'], type='varchar', alias='dfrp_kw') }},
        ...
        {{ trx_json_extract('src', ['dfeq_ref_compnr'], type='int', alias='dfeq_ref_compnr') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tppdm625') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'cequ']) }}
from cte_flattened
{% if is_incremental() %}
