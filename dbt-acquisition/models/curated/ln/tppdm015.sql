{{ config(
    ...
    description='LN tppdm015 Labor table, Generated 2026-04-10T19:41:53Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tppdm015') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'task']) }}
from cte_flattened
{% if is_incremental() %}
