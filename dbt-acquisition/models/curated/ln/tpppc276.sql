{{ config(
    ...
    description='LN tpppc276 Subcontracting Cost Forecast table, Generated 2026-04-10T19:42:10Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc276') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'rgdt', 'csub', 'cspa', 'cact']) }}
from cte_flattened
{% if is_incremental() %}
