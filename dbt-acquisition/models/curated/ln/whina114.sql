{{ config(
    ...
    description='LN whina114 Inventory Receipt Transaction Consumptions table, Generated 2026-04-10T19:42:44Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_whina114') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'item', 'cwar', 'trdt', 'seqn', 'inwp', 'sern']) }}
from cte_flattened
{% if is_incremental() %}
