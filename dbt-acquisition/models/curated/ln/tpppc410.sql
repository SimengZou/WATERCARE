{{ config(
    ...
    description='LN tpppc410 Cost Control by Project / Material Control Code table, Generated 2026-04-10T19:42:13Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc410') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'ccit']) }}
from cte_flattened
{% if is_incremental() %}
