{{ config(
    ...
    description='LN tpptc352 Actual Budget by Equipment Control Code table, Generated 2026-04-10T19:42:27Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpptc352') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'ccal', 'cceq']) }}
from cte_flattened
{% if is_incremental() %}
