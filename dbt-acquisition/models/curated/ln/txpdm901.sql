{{ config(
    ...
    description='LN txpdm901 Local Board Master table, Generated 2026-04-10T19:42:40Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_txpdm901') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'locb']) }}
from cte_flattened
{% if is_incremental() %}
