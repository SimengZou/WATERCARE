{{ config(
    ...
    description='LN tpptc351 Actual Budget by Labor Control Code table, Generated 2026-04-10T19:42:26Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpptc351') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'ccal', 'ccts']) }}
from cte_flattened
{% if is_incremental() %}
