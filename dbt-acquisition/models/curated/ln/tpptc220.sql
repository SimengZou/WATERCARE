{{ config(
    ...
    description='LN tpptc220 Material Budget Lines by Activity table, Generated 2026-04-10T19:42:24Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['snsb'], type='int', alias='snsb') }},
        ...
        {{ trx_json_extract('src', ['espp'], type='varchar', alias='espp') }},
        {{ trx_json_extract('src', ['slnk'], type='int', alias='slnk') }},
        {{ trx_json_extract('src', ['slnk_kw'], type='varchar', alias='slnk_kw') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpptc220') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'cpla', 'cact', 'sern']) }}
from cte_flattened
{% if is_incremental() %}
