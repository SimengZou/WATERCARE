{{ config(
    ...
    description='LN tppss020 Baselines table, Generated 2026-04-10T19:42:18Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['bsta'], type='int', alias='bsta') }},
        {{ trx_json_extract('src', ['bsta_kw'], type='varchar', alias='bsta_kw') }},
        {{ trx_json_extract('src', ['crdt'], type='timestamp_ntz', alias='crdt') }},
        {{ trx_json_extract('src', ['apdt'], type='timestamp_ntz', alias='apdt') }},
        {{ trx_json_extract('src', ['lupd'], type='timestamp_ntz', alias='lupd') }},
        {{ trx_json_extract('src', ['dbsn'], type='int', alias='dbsn') }},
        {{ trx_json_extract('src', ['lmus'], type='varchar', alias='lmus') }},
        {{ trx_json_extract('src', ['txta'], type='int', alias='txta') }},
        {{ trx_json_extract('src', ['cprj_dbsn_ref_compnr'], type='int', alias='cprj_dbsn_ref_compnr') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tppss020') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'basn']) }}
from cte_flattened
{% if is_incremental() %}
