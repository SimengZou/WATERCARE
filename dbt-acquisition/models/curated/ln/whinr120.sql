{{ config(
    ...
    description='LN whinr120 Item Issue by Period table, Generated 2025-06-12T01:48:43Z from package combination ce01090',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['ddcl'], type='float', alias='ddcl') }},
        {{ trx_json_extract('src', ['ddqt'], type='float', alias='ddqt') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_whinr120') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'item', 'year', 'peri']) }}
from cte_flattened
{% if is_incremental() %}
