{{ config(
    ...
    description='LN tpppc216 Material Cost Forecast table, Generated 2026-04-10T19:42:08Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc216') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'rgdt', 'item', 'cspa', 'cact']) }}
from cte_flattened
{% if is_incremental() %}
