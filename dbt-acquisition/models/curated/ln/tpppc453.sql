{{ config(
    ...
    description='LN tpppc453 Cost Control by Project / Activity / Subcontracting Control Code table, Generated 2026-04-10T19:42:16Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc453') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'cpla', 'cact', 'ccsb']) }}
from cte_flattened
{% if is_incremental() %}
