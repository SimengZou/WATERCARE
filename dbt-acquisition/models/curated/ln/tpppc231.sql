{{ config(
    ...
    description='LN tpppc231 Labor Costs table, Generated 2026-04-10T19:42:08Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['tetc'], type='int', alias='tetc') }},
        {{ trx_json_extract('src', ['tetc_kw'], type='varchar', alias='tetc_kw') }},
        ...
        {{ trx_json_extract('src', ['serc'], type='int', alias='serc') }},
        ...
        {{ trx_json_extract('src', ['cdf_quan'], type='float', alias='cdf_quan') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc231') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cpth', 'year', 'peri', 'emno', 'sern']) }}
from cte_flattened
{% if is_incremental() %}
