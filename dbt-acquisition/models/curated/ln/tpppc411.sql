{{ config(
    ...
    description='LN tpppc411 Cost Control by Project / Labor Control Code table, Generated 2026-04-10T19:42:14Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc411') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'ccts']) }}
from cte_flattened
{% if is_incremental() %}
