{{ config(
    ...
    description='LN tpptc127 Control Data Material Lines table, Generated 2026-04-10T19:42:21Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpptc127') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'item', 'sern']) }}
from cte_flattened
{% if is_incremental() %}
