{{ config(
    ...
    description='LN tpppc256 Equipment Cost Forecast table, Generated 2026-04-10T19:42:10Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc256') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'rgdt', 'cequ', 'cspa', 'cact']) }}
from cte_flattened
{% if is_incremental() %}
