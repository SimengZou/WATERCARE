{{ config(
    ...
    description='LN tpppc402 Cost Control by Project / Cost Component table, Generated 2026-04-10T19:42:13Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['cott'], type='int', alias='cott') }},
        {{ trx_json_extract('src', ['cott_kw'], type='varchar', alias='cott_kw') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc402') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'mpto', 'ccco', 'cott']) }}
from cte_flattened
{% if is_incremental() %}
