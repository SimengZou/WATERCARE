{{ config(
    ...
    description='LN tpppc440 Cost Control by Project / Activity table, Generated 2026-04-10T19:42:15Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tpppc440') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cprj', 'mpto', 'cpla', 'cact']) }}
from cte_flattened
{% if is_incremental() %}
