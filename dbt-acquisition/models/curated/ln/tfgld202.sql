{{ config(
    ...
    description='LN tfgld202 History - Dimension/Ledger Account Totals table, Generated 2026-04-10T19:41:43Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_tfgld202') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'cono', 'ptyp', 'year', 'prno', 'dtyp', 'dimx', 'leac', 'ccur', 'duac', 'bpid']) }}
from cte_flattened
{% if is_incremental() %}
