{{ config(
    ...
    description='LN whinh521 Adjustment Order Lines table, Generated 2026-04-10T19:42:52Z from package combination ce01101',
...
with cte_flattened as (
    select
        ...
        {{ trx_json_extract('src', ['qadj'], type='float', alias='qadj') }},
        {{ trx_json_extract('src', ['qadr'], type='float', alias='qadr') }},
        ...
        {{ trx_json_extract('src', ['rjin'], type='int', alias='rjin') }},
        {{ trx_json_extract('src', ['rjin_kw'], type='varchar', alias='rjin_kw') }},
        ...
        {{ trx_etl_columns() }}
    from {{ source('source_landing_ln', 'ln_whinh521') }}
)

select
    *,
    {{ trx_audit_columns() }},
    {{ trx_audit_surrogate(columns=['compnr', 'orno', 'pono']) }}
from cte_flattened
{% if is_incremental() %}
