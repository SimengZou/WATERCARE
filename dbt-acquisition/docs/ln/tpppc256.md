...
LN tpppc256 Equipment Cost Forecast table, Generated 2026-04-10T19:42:10Z from package combination ce01101
...
|---|---|
| **Source system** | `ln` |
| **Source table** | `ln_tpppc256` |
| **Materialization** | `incremental` |
| **Primary keys** | `compnr`, `cprj`, `rgdt`, `cequ`, `cspa`, `cact` |
| **Column count** | 73 |
...
| `pric_prjc` | `float` | `float` |  |  | CC: Equipment Cost Price in Project Currency |
| `pric_cntc` | `float` | `float` |  |  | CC: Equipment Cost Price in Contract Currency |
| `pric_dwhc` | `float` | `float` |  |  | CC: Equipment Cost Price in Data Warehouse Currency |
| `pric_refc` | `float` | `float` |  |  | CC: Equipment Cost Price in Reference Currency |
| `pric_homc` | `float` | `float` |  |  | CC: Equipment Cost Price in Local Currency |
| `pric_rpc1` | `float` | `float` |  |  | CC: Equipment Cost Price in Reference Currency 1 |
| `pric_rpc2` | `float` | `float` |  |  | CC: Equipment Cost Price in Reference Currency 2 |
| `amoc_prjc` | `float` | `float` |  |  | CC: Equipment Cost Amount in Project Currency |
| `amoc_cntc` | `float` | `float` |  |  | CC: Equipment Cost Amount in Contract Currency |
| `amoc_dwhc` | `float` | `float` |  |  | CC: Equipment Cost Amount in Data Warehouse Currency |
| `amoc_refc` | `float` | `float` |  |  | CC: Equipment Cost Amount in Reference Currency |
| `amoc_homc` | `float` | `float` |  |  | CC: Equipment Cost Amount in Local Currency |
| `amoc_rpc1` | `float` | `float` |  |  | CC: Equipment Cost Amount in Reporting Currency 1 |
| `amoc_rpc2` | `float` | `float` |  |  | CC: Equipment Cost Amount in Reporting Currency 2 |
