CREATE OR REPLACE PROCEDURE "SP_DAILY_BRIDGE_METER_TO_ACCOUNT_LOAD"()
...
//  define array of stored procedure calls 
	...
	//	/* get deltas */
		call_sp[0] = " CALL DATAHUB_STAGING.SP_STG_BRIDGE_METER_TO_ACCOUNT(); ";

//	/* extract SIDs from dimensions */
		call_sp[1] = " CALL DATAHUB_STAGING.SP_STG2_BRIDGE_METER_TO_ACCOUNT(); ";

	//	/* merge new data to fact table */
		call_sp[2] = " CALL DATAHUB_ANALYTICS.SP_BRIDGE_METER_TO_ACCOUNT(); ";
		
	//	/* insert meters and accounts with no related account positions */
		call_sp[3] = " CALL DATAHUB_ANALYTICS.SP0_BRIDGE_METER_TO_ACCOUNT(); ";
		
	//	/* expire water meter sid records with new account position */
		call_sp[4] = " CALL DATAHUB_ANALYTICS.SP2_BRIDGE_METER_TO_ACCOUNT(); ";
		
	//	/* expire account sid records with new account position */
		call_sp[5] = " CALL DATAHUB_ANALYTICS.SP3_BRIDGE_METER_TO_ACCOUNT(); ";
     ...
     if (result === "Success") 
	   {
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Bridge Table Load  Failed'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
