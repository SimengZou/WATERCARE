CREATE OR REPLACE PROCEDURE "SP_DAILY_BILL_LINE_ITEM_LOAD"()
...
// /* Load Bill Line Item data model */

//  define array of stored procedure calls 
        const call_sp = [];
			
		call_sp[0] = " CALL DATAHUB_STAGING.SP_STG1_DIM_CUSTOMER_ACCOUNT(); ";
		call_sp[1] = " CALL DATAHUB_STAGING.SP_STG_DIM_CUSTOMER_ACCOUNT(); ";

		call_sp[2] = " CALL DATAHUB_ANALYTICS.SP_DIM_CUSTOMER_ACCOUNT(); ";

		call_sp[3]= " CALL DATAHUB_STAGING.SP_STG_DIM_ACCOUNT_SERVICE(); ";

		call_sp[4] = " CALL DATAHUB_ANALYTICS.SP_DIM_ACCOUNT_SERVICE(); ";

		call_sp[5] = " CALL DATAHUB_STAGING.SP_STG_DIM_BILL(); ";
		call_sp[6] = " CALL DATAHUB_ANALYTICS.SP_DIM_BILL(); ";


		call_sp[7] = " CALL DATAHUB_STAGING.SP_STG_DIM_GL_MAPPING_INFORMATION(); ";

		call_sp[8] = " CALL DATAHUB_ANALYTICS.SP_DIM_GL_MAPPING_INFORMATION(); ";

		call_sp[9] = " CALL DATAHUB_STAGING.SP_STG_DIM_BILL_LINE_ITEM_SETUP(); ";

		call_sp[10] = " CALL DATAHUB_ANALYTICS.SP_DIM_BILL_LINE_ITEM_SETUP(); ";

	//	/* get deltas */
		call_sp[11] = " CALL DATAHUB_STAGING.SP_STG_FACT_BILL_LINE_ITEM(); ";
	
    /* Initialise late arriving dimension records */	
		call_sp[12] = " CALL datahub_analytics.sp0BLI_DIM_CUSTOMER_ACCOUNT(); ";
		call_sp[13] = " CALL datahub_analytics.sp0BLI_DIM_BILL(); ";
		call_sp[14] = " CALL datahub_analytics.sp0BLI_DIM_ACCOUNT_SERVICE(); ";
		call_sp[15] = " CALL datahub_analytics.sp0BLI_DIM_GL_MAPPING_INFORMATION(); ";
		
		call_sp[16] = " CALL datahub_analytics.sp0BLI_DIM_BILL_LINE_ITEM_SETUP(); ";
		

//	/* extract SIDs from dimensions */
		call_sp[17] = " CALL DATAHUB_STAGING.SP_STG2_FACT_BILL_LINE_ITEM(); ";

	//	/* merge new data to fact table */
		call_sp[18] = " CALL DATAHUB_ANALYTICS.SP_FACT_BILL_LINE_ITEM(); ";
     ...
     if (result === "Success") 
	   {
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Completed Bill Line Item Model Load'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
       }
     else 	{
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Bill Line Item Model Load Failed'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
