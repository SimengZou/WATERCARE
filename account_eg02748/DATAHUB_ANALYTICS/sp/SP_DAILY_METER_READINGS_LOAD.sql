CREATE OR REPLACE PROCEDURE "SP_DAILY_METER_READINGS_LOAD"()
...
///* Load Meter Readings data model */

//  define array of stored procedure calls 

        const call_sp = [];
		
		call_sp[0] = " CALL DATAHUB_STAGING.SP_STG_DIM_ROUTE(); ";

		call_sp[1] = " CALL DATAHUB_ANALYTICS.SP_DIM_ROUTE(); ";
		
		call_sp[2] = " CALL DATAHUB_STAGING.SP_STG_DIM_WATER_METER(); ";
		
		call_sp[3] = " CALL DATAHUB_STAGING.SP_STG2_DIM_WATER_METER(); ";
		
		call_sp[4] = " CALL DATAHUB_ANALYTICS.SP_DIM_WATER_METER(); ";

		call_sp[5] = " CALL DATAHUB_STAGING.SP_STG_DIM_CUSTOMER_PROBLEM(); ";

		call_sp[6] = " CALL DATAHUB_ANALYTICS.SP_DIM_CUSTOMER_PROBLEM(); ";

		call_sp[7] = " CALL DATAHUB_STAGING.SP_STG_DIM_READ_REASON(); ";

		call_sp[8] = " CALL DATAHUB_ANALYTICS.SP_DIM_READ_REASON(); ";


		call_sp[9] = " CALL DATAHUB_STAGING.SP_STG_DIM_READ_SOURCE(); ";

		call_sp[10] = " CALL DATAHUB_ANALYTICS.SP_DIM_READ_SOURCE(); ";

		call_sp[11] = " CALL DATAHUB_STAGING.SP_STG_DIM_READER_CODE(); ";

		call_sp[12] = " CALL DATAHUB_ANALYTICS.SP_DIM_READER_CODE(); ";


		call_sp[13] = " CALL DATAHUB_STAGING.SP_STG_DIM_EMPLOYEE(); ";

		call_sp[14] = " CALL DATAHUB_ANALYTICS.SP_DIM_EMPLOYEE(); ";

	//	/* get deltas */
		call_sp[15] = " CALL DATAHUB_STAGING.SP_STG_FACT_METER_READING();  ";
		
	/* initialise late arriving dimension records  DE-705 */	
		call_sp[16] = " CALL datahub_analytics.sp0mr_dim_water_meter();  ";
		call_sp[17] = " CALL datahub_analytics.sp0mr_dim_route();  ";
		call_sp[18] = " CALL datahub_analytics.sp0mr_dim_read_source();  ";
		call_sp[19] = " CALL datahub_analytics.sp0mr_dim_customer_problem();  ";
		call_sp[20] = " CALL datahub_analytics.sp0mr_dim_reader_code();  ";
		call_sp[21] = " CALL datahub_analytics.sp0mr_dim_read_reason();  ";
		call_sp[22] = " CALL datahub_analytics.sp0mr_dim_employee();  ";

	//	/* extract SIDs from dimensions */
		call_sp[23] = " CALL DATAHUB_STAGING.SP_STG2_FACT_METER_READING(); ";

	//	/* merge new data to fact table */
		call_sp[24] = " CALL DATAHUB_ANALYTICS.SP_FACT_METER_READING();  ";
    
	
	// Loop all stored procedures required for bill line item load - if one fails stop processing 
     var i=0;
     ...
     if (result === "Success") 
	   {
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Completed Meter Readings Model Load'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
       }
     else 	{
        ...
        if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Meter Readings Model Load  Failed'');";
            snowflake.execute({sqlText: debug_statement}); 
			}
