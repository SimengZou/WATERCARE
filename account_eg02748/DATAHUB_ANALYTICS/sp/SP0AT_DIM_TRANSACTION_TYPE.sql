CREATE OR REPLACE PROCEDURE "SP0AT_DIM_TRANSACTION_TYPE"()
...
//  Run merge statement
	        ...
	        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.DIM_TRANSACTION_TYPE target USING ( ";
				
			sql_command += " SELECT  ";
			sql_command += "   ACCTTRAN.TRANSACTION_TYPE_CODE,";
			sql_command += "   ACCTTRAN.TRANSACTION_DESIGNATOR_CODE,";
			sql_command += "   ''Not Available'' AS TRANSACTION_TYPE_DESCRIPTION,";
			sql_command += "   ''Not Available'' AS TRANSACTION_DESIGNATOR_DESCRIPTION,";
			sql_command += "   max(ACCTTRAN.ETL_SOURCE_DATETIME) AS ETL_SOURCE_DATETIME, ";
			sql_command += "   min(ACCTTRAN.ETL_RECORD_CREATED) AS ETL_RECORD_CREATED, ";
			sql_command += "   max(ACCTTRAN.ETL_RECORD_UPDATED) AS ETL_RECORD_UPDATED";
			sql_command += "  FROM DATAHUB_STAGING.STG_FACT_ACCOUNT_TRANSACTION ACCTTRAN";
			sql_command += "    LEFT JOIN DATAHUB_ANALYTICS.DIM_TRANSACTION_TYPE TRANSACTION_TYPE ";
			sql_command += "      ON COALESCE(ACCTTRAN.TRANSACTION_TYPE_CODE,-1) =COALESCE(TRANSACTION_TYPE.TRANSACTION_TYPE_CODE,-1)";
			sql_command += "      AND COALESCE(ACCTTRAN.TRANSACTION_DESIGNATOR_CODE,-1)= COALESCE(TRANSACTION_TYPE.TRANSACTION_DESIGNATOR_CODE,-1)";
			sql_command += "     WHERE TRANSACTION_TYPE.TRANSACTION_TYPE_CODE IS NULL AND ACCTTRAN.TRANSACTION_TYPE_CODE IS NOT NULL";
			sql_command += "  GROUP BY 1,2";
			sql_command += "   ) AS b ON target.TRANSACTION_TYPE_CODE =b.TRANSACTION_TYPE_CODE ";
			sql_command += "     AND COALESCE(target.TRANSACTION_DESIGNATOR_CODE,-1)= COALESCE(b.TRANSACTION_DESIGNATOR_CODE,-1)";
			sql_command += " WHEN NOT MATCHED THEN INSERT (TRANSACTION_TYPE_CODE, ";
			...
			sql_command += " VALUES (b.TRANSACTION_TYPE_CODE, ";
			sql_command += "   b.TRANSACTION_TYPE_DESCRIPTION ,";
			sql_command += "   b.TRANSACTION_DESIGNATOR_CODE, ";
			sql_command += "   b.TRANSACTION_DESIGNATOR_DESCRIPTION,";
