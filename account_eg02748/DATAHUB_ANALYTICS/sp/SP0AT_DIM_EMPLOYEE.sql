CREATE OR REPLACE PROCEDURE "SP0AT_DIM_EMPLOYEE"()
...
//  Run merge statement
	        ...
	        var sql_command = "MERGE INTO DATAHUB_ANALYTICS.DIM_EMPLOYEE target USING ( ";		
			sql_command += " SELECT  ";
			sql_command += "   ACCTTRAN.TRANSACTION_BY_EMPLOYEE_KEY AS EMPLOYEE_CODE,";
			sql_command += "   ''Not Available'' AS EMPLOYEE_NAME,";
			...
			sql_command += "    LEFT JOIN DATAHUB_ANALYTICS.DIM_EMPLOYEE EMPLOYEE ON upper(ACCTTRAN.TRANSACTION_BY_EMPLOYEE_KEY)= upper(EMPLOYEE.EMPLOYEE_CODE)";
			sql_command += "  WHERE EMPLOYEE.EMPLOYEE_CODE IS NULL AND ACCTTRAN.TRANSACTION_BY_EMPLOYEE_KEY IS NOT NULL";
			sql_command += "  GROUP BY 1";
			sql_command += "  ) AS b ON upper(target.EMPLOYEE_CODE) = upper(b.EMPLOYEE_CODE)";
			sql_command += "  WHEN NOT MATCHED THEN INSERT (EMPLOYEE_CODE, ";
			sql_command += "  EMPLOYEE_NAME, ";
			sql_command += "  ETL_SOURCE_DATETIME, ";
			sql_command += "  ETL_RECORD_CREATED, ";
			sql_command += "  ETL_RECORD_UPDATED)";
			sql_command += "  VALUES (b.EMPLOYEE_CODE, ";
			sql_command += "  b.EMPLOYEE_NAME ,";
