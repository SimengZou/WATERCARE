CREATE OR REPLACE PROCEDURE "SP2_BRIDGE_METER_TO_ACCOUNT"()
...
//  Run merge statement
		...
		var sql_command = " UPDATE DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT b";
			sql_command += " SET b.Current_record=''N'', b.etl_record_deleted=current_timestamp(), b.etl_record_updated=current_timestamp()";
			sql_command += " WHERE b.ACCOUNT_SERVICE_POSITION_KEY IS NULL AND ";
			sql_command += " b.water_meter_sid<>0 AND b.current_record=''Y''";
			sql_command += " AND b.water_meter_sid IN (SELECT DISTINCT WATER_METER_SID FROM DATAHUB_ANALYTICS.BRIDGE_METER_TO_ACCOUNT ";
