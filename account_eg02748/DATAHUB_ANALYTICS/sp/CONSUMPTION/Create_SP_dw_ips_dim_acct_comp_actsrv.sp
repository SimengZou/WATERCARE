//  Name:       sp_dw_ips_dim_acct_comp_actsrv
//
//  Purpose:    Load consumption data - staging table dw_ips_dim_acct_comp_actsrv loaded from staging table dw_ips_stage_consump_stg5 and 5 dimension tables by job sp_dw_ips_dim_acct_comp_actsrv
...
create or replace procedure datahub_analytics.sp_dw_ips_dim_acct_comp_actsrv()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_ANALYTICS.DW_IPS_DIM_ACCT_COMP_ACTSRV;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText:  ` INSERT INTO DATAHUB_ANALYTICS.DW_IPS_DIM_ACCT_COMP_ACTSRV
		 (ACCOUNT_KEY, COMP_KEY, ACCOUNT_SERVICE_KEY)
		 SELECT  DISTINCT
		   a.account_key, stg5.comp_key, s.account_service_key      
		 ...
		 WHERE CAST(stg5.read_date AS DATE) > CAST(s.start_datetime AS DATE) 
		 AND CAST(stg5.read_date AS DATE) <= CAST(coalesce(s.stop_datetime,  to_date('31-Dec-9999','DD-Mon-YYYY')) AS DATE);`} );
