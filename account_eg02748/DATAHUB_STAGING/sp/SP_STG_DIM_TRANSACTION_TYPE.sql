CREATE OR REPLACE PROCEDURE "SP_STG_DIM_TRANSACTION_TYPE"()
...
//update the latest time included in extract for ACCTTRAN table
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for ACCTTRAN table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	           
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_TRANSACTION_TYPE"});
...
//  Run insert statement
		...
		var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_TRANSACTION_TYPE ";
		sql_command += " (TRANSACTION_TYPE_CODE, ";
		sql_command += " TRANSACTION_TYPE_DESCRIPTION, ";
		sql_command += " TRANSACTION_DESIGNATOR_CODE, ";
		sql_command += " TRANSACTION_DESIGNATOR_DESCRIPTION, ";
		...
		sql_command += " CASE ACCTTRAN.TRANSACTIONTYPE ";
		sql_command += "   WHEN 1 THEN ''Adjustment''";
		sql_command += "   WHEN 2 THEN ''Apply''";
		sql_command += "   WHEN 3 THEN ''Charge''";
		sql_command += "   WHEN 4 THEN ''Credit''";
		sql_command += "   WHEN 5 THEN ''Drop Balance''";
		sql_command += "   WHEN 6 THEN ''Refund''";
		sql_command += "   WHEN 7 THEN ''Resolution''";
		sql_command += "   WHEN 8 THEN ''Round''";
		sql_command += "   WHEN 9 THEN ''Payment''";
		sql_command += "   WHEN 10 THEN ''Payment Adjustment''";
		sql_command += "   WHEN 11 THEN ''Transfer''";
		sql_command += " END AS TRANSACTION_TYPE_DESCRIPTION,";
		sql_command += " ACCTTRAN.TRANSACTIONDESIGNATOR AS TRANSACTION_DESIGNATOR_CODE,";
		sql_command += " CASE ACCTTRAN.TRANSACTIONDESIGNATOR ";
		sql_command += "   WHEN 1 THEN ''Account''";
		sql_command += "   WHEN 2 THEN ''Budget Billing Settle''";
		sql_command += "   WHEN 3 THEN ''Deposit''";
		sql_command += "   WHEN 4 THEN ''Discount'' ";
		sql_command += "   WHEN 5 THEN ''Interest''";
		sql_command += "   WHEN 6 THEN ''Overpay''";
		sql_command += "   WHEN 7 THEN ''Payment Resolution''";
		sql_command += "   WHEN 8 THEN ''Penalty''";
		sql_command += "   WHEN 9 THEN ''Penalty Interest''";
		sql_command += "   WHEN 10 THEN ''Principal''";
		sql_command += "   WHEN 11 THEN ''Surcharge''";
		sql_command += "  END AS TRANSACTION_DESIGNATOR_DESCRIPTION,";
		sql_command += "  max(ACCTTRAN.ETL_LOAD_DATETIME) AS ETL_SOURCE_DATETIME, ";
	    ...
	    sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB TRANSACTION_TYPE_CTRL ";
	    sql_command += "  ON TRANSACTION_TYPE_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_TRANSACTION_TYPE'' AND TRANSACTION_TYPE_CTRL.TABLE_NAME=''IPS_BILLING_ACCTTRAN'' AND TRANSACTION_TYPE_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
			
		sql_command += " WHERE  ACCTTRAN.ETL_LOAD_DATETIME>TRANSACTION_TYPE_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += "  GROUP BY 1,2,3,4;";
