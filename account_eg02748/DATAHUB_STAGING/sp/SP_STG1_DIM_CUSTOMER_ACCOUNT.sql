CREATE OR REPLACE PROCEDURE "SP_STG1_DIM_CUSTOMER_ACCOUNT"()
...
// not required - extract all as its required for the next join
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.stg1_dim_CUSTOMER_ACCOUNT"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.stg1_dim_CUSTOMER_ACCOUNT (  ";
		sql_command += "ACCOUNTKEY,  ";
		sql_command += "IGC_FLAG, ";
		sql_command += "COMPLIANCE_FLAG, ";
		sql_command += "ETL_SOURCE_DATETIME, ";
		sql_command += "ETL_RECORD_CREATED, ";
		sql_command += "ETL_RECORD_UPDATED) ";
		sql_command += "SELECT ";
		sql_command += "ACCTTRAN.accountkey, ";
		sql_command += "CASE WHEN UPPER(LINEITEM.PRINTTEXT) LIKE ''%IGC%'' THEN 1 ELSE 0 END AS IGC_Flag, ";
		sql_command += "CASE WHEN UPPER(LINEITEM.PRINTTEXT) LIKE ''%COMPLIANCE%'' THEN 1 ELSE 0 END AS Compliance_Flag, ";
		sql_command += "greatest(ACCTTRAN.ETL_LOAD_DATETIME,lineitem.ETL_LOAD_DATETIME ) AS ETL_SOURCE_DATETIME, ";
		...
		sql_command += "LEFT OUTER JOIN DATAHUB_TARGET.IPS_BILLING_LINEITEM LINEITEM ";
		sql_command += "ON ACCTTRAN.LINEITEMKEY = LINEITEM.LINEITEMKEY ";
	    sql_command += "WHERE UPPER(LINEITEM.PRINTTEXT) LIKE ''%IGC%'' OR UPPER(LINEITEM.PRINTTEXT) LIKE ''%COMPLIANCE%'';";
