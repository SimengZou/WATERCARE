CREATE OR REPLACE PROCEDURE "SP_STG_DIM_BILL_LINE_ITEM_SETUP"()
...
//update the latest time included in extract for BILL_LINE_ITEM_SETUP table
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for BILL_LINE_ITEM_SETUP source table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_BILL_LINE_ITEM_SETUP"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_BILL_LINE_ITEM_SETUP (  ";
		sql_command += " BILL_LINE_ITEM_SETUP_KEY, ";
		sql_command += " BILL_LINE_ITEM_CODE, ";
		sql_command += " BILL_LINE_ITEM_DESCRIPTION, ";
		sql_command += " BILL_LINE_ITEM_DESIGNATION, ";
		sql_command += " BILL_LINE_ITEM_DESIGNATION_DESCRIPTION, ";
		sql_command += " BILL_LINE_ITEM_EFFECTIVE_DATE, ";
		sql_command += " BILL_LINE_ITEM_GROUP, ";
		sql_command += " FEE_TYPE_KEY,";
		sql_command += " PRINT_TEXT,";
		sql_command += " ETL_SOURCE_DATETIME, ";
		sql_command += " ETL_RECORD_CREATED, ";
		sql_command += " ETL_RECORD_UPDATED)";
		sql_command += " SELECT ";
		sql_command += " LINEITEMSETUP.LINEITEMSETUPKEY AS BILL_LINE_ITEM_SETUP_KEY, ";
		sql_command += " LINEITEMSETUP.LINEITEM AS BILL_LINE_ITEM_CODE, ";
		sql_command += " LINEITEMSETUP.DESCRIPTION AS BILL_LINE_ITEM_DESCRIPTION, ";
		sql_command += " LINEITEMSETUP.DESIGNATION AS BILL_LINE_ITEM_DESIGNATION, ";
		sql_command += " CASE WHEN LINEITEMSETUP.DESIGNATION=''DR'' THEN ''Debit''";
		sql_command += "       WHEN LINEITEMSETUP.DESIGNATION=''CR'' THEN ''Credit''";
		sql_command += "       ELSE ''Not applicable (only for adjustments)'' END AS BILL_LINE_ITEM_DESIGNATION_DESCRIPTION, ";
		sql_command += " LINEITEMSETUP.EFFDATE AS BILL_LINE_ITEM_EFFECTIVE_DATE, ";
		sql_command += " LINEITEMSETUP.lineitemgroup AS BILL_LINE_ITEM_GROUP, ";
		sql_command += " LINEITEMSETUP.FeeTypeKey AS FEE_TYPE_KEY,";
		sql_command += " LINEITEMSETUP.PRINTTEXT AS PRINT_TEXT,";
		sql_command += " LINEITEMSETUP.etl_load_datetime AS ETL_SOURCE_DATETIME, ";
		sql_command += " current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += " current_timestamp() AS ETL_RECORD_UPDATED";
		sql_command += "  FROM DATAHUB_TARGET.IPS_BILLING_LINEITEMSETUP LINEITEMSETUP";
		sql_command += "  INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB LINEITEMSETUP_CTRL ";
		sql_command += "    ON LINEITEMSETUP_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_BILL_LINE_ITEM_SETUP'' AND LINEITEMSETUP_CTRL.TABLE_NAME=''IPS_BILLING_LINEITEMSETUP'' AND LINEITEMSETUP_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
		sql_command += "  WHERE  LINEITEMSETUP.ETL_LOAD_DATETIME>LINEITEMSETUP_CTRL.INC_ETL_LOAD_DATETIME;";
