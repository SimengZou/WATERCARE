//  Name:       sp_dw_ips_stage_consump_stg2
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg2 loaded from staging table dw_ips_stage_consump_stg1 by job sp_dw_ips_stage_consump_stg2
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg2()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG2;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG2
		 (COMPKEY, READINGKEY, READDTTM, READING, USAGE, READYFLAG, BILLED)
		 select distinct
		   w.comp_key, mr.reading_key, mr.read_datetime, mr.reading, coalesce(t.consump, mr.usage) as usage,  
		     mr.ready_flag, 'Y' as billed 
		 from datahub_analytics.dw_ips_dim_compwmtr w 
		   inner join datahub_analytics.dw_ips_dim_assloc asl  on w.comp_key=asl.comp_key
		   inner join datahub_analytics.dw_ips_dim_meterreading mr  on w.comp_key=mr.comp_key
		   inner join datahub_staging.dw_ips_stage_consump_stg1 t on mr.reading_key=t.readingkey
		   inner join datahub_analytics.dw_ips_dim_watertrackedreading wtr on mr.reading_key=wtr.reading_key
		   left outer join
		      (select comp_key, description from datahub_analytics.dw_ips_dim_xwatermeterdetail xwmd 
		       inner join datahub_analytics.dw_ips_dim_meterzerocodes mzc  on xwmd.meter_zero_code = mzc.code) z 
		     on w.comp_key=z.comp_key
		 where mr.read_datetime >= asl.installed_datetime and mr.read_datetime <= coalesce(asl.removed_datetime, getdate());`} );
