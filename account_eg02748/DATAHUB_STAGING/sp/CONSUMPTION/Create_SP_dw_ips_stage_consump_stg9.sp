//  Name:       sp_dw_ips_stage_consump_stg9
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg9 loaded from staging table dw_ips_stage_consump_stg8a by job sp_dw_ips_stage_consump_stg9
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg9()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG9;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `	INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG9
		 (ACCOUNT_KEY, ACCOUNT_SERVICE_KEY, ADDRESS_KEY, COMP_KEY, MONTH, CAL_MONTH_NUM, CAL_YEAR, ACT_DAILY_AVG, 
		 ACTUAL_DAYS, ACTUAL_VOLUME, FULL_MONTH_FLAG, EST_DAILY_AVG, EST_DAYS, EST_VOLUME, TOTAL_VOLUME, ESTIMATE_FLAG, ESTIMATE_METHOD, CONSUMPTION_PERCENTAGE, RATIO)
		 SELECT distinct
		   account_key, account_service_key, address_key,
		   comp_key, month, cal_month_num, cal_year,
		   avg_daily_usage * consumption_percentage/ 100.0 as act_daily_avg,
		   actual_days,
		   actual_volume * consumption_percentage/ 100.0 as actual_volume,
		   full_month_flag,
		   est_daily_avg * consumption_percentage/ 100.0 as est_daily_avg,
		   est_days,
		   est_volume * consumption_percentage/ 100.0 as est_volume,
		   (coalesce(actual_volume, 0) * consumption_percentage / 100.0) + 
		   (coalesce(est_volume, 0) * consumption_percentage / 100.0) as total_volume,       
		   estimate_flag,
		   estimate_method,
		   consumption_percentage,
		   ratio
		 FROM datahub_staging.dw_ips_stage_consump_stg8a
		 where month > 201908;`} );
