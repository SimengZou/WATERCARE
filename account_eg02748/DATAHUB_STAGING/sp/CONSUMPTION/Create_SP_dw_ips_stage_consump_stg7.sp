//  Name:       sp_dw_ips_stage_consump_stg7
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg7 loaded from staging table dw_ips_stage_consump_stg6a by job sp_dw_ips_stage_consump_stg7
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg7()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG7;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText:  `	 INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG7	
		 (ACCOUNT_KEY, ACCOUNT_SERVICE_KEY, ADDRESS_KEY, CONSUMPTION_PERCENTAGE, SUBTRACTIVE_FLAG, COMP_KEY, CAL_MONTH, FIRST_READING_DATE, 
		 PREV_READING_DATE, MONTH_START_DATE, MONTH_END_DATE, AVG_DAILY_USAGE, ACTUAL_DAYS, ACTUAL_VOLUME, FULL_MONTH_FLAG)
		 SELECT 
		   account_key, account_service_key, address_key, consumption_percentage, subtractive_flag,
		   comp_key, cal_month, 
		   MIN(read_date) as first_reading_date,
		   MAX(prev_read_date) as prev_reading_date,
		   MIN(month_start_date) as month_start_date,
		   MAX(month_end_date) as month_end_date,
		   SUM(avg_daily_usage * actual_days) / SUM(actual_days) as avg_daily_usage,
		   SUM(actual_days) as actual_days,
		   SUM(avg_daily_usage * actual_days) as actual_volume,        
		    CASE WHEN SUM(actual_days) = Datediff(day, to_date( Cast(cal_month as varchar)||'01','YYYYMMDD'), 
		        Dateadd(day, -1, Dateadd(mon, 1, to_date( Cast(cal_month as varchar)||'01', 'YYYYMMDD'))))+1 THEN 'Y'
		          ELSE 'N'  END AS  full_month_flag         
		 FROM datahub_staging.dw_ips_stage_consump_stg6a
		 WHERE Actual_Days <> 0 
		 GROUP BY  account_key, account_service_key, address_key, consumption_percentage, subtractive_flag, comp_key, cal_month;
`} );
