//  Name:       sp_dw_ips_stage_consump_stg3
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg3 loaded from staging table dw_ips_stage_consump_stg2 by job sp_dw_ips_stage_consump_stg3
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg3()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG3;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG3
		 (COMP_KEY, COMP_READING_KEY, ROWNUM, READ_DATE, READING, USAGE)
		 SELECT DISTINCT
		   compkey, readingkey, ROW_NUMBER() OVER (PARTITION BY compkey ORDER BY readdttm, readingkey) as rownum,
		   readdttm, reading, usage
		 FROM datahub_staging.dw_ips_stage_consump_stg2;`} );
