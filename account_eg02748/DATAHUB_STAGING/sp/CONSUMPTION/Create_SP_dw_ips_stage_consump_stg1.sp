//  Name:       sp_dw_ips_stage_consump_stg1
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg1 loaded by job sp_dw_ips_stage_consump_stg1
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg1()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG1;"});
...
//  Run insert statement
        ...
        //    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
		    ...
		    var sqlStmt = snowflake.createStatement( {sqlText:  `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG1
		 (SEQ, COMPKEY, READINGKEY, READING, MAXREAD)
		 SELECT 
		 ROW_NUMBER() OVER (PARTITION BY comp_key ORDER BY reading_key) as seq, 
		 t1.comp_key, t1.reading_key, t1.reading, t1.maxread
		 from (
		  select distinct 
		  mr.comp_key, mr.reading_key, mr.reading , 
		  case when mt.lowregnumberdials > 9 then 1000000000 else power(10,mt.lowregnumberdials) end as maxread
		  from datahub_analytics.dw_ips_dim_meterreading mr  
		   inner join datahub_analytics.dw_ips_dim_compwmtr w  on mr.comp_key=w.comp_key
		   inner join datahub_analytics.dw_ips_dim_watermetertype mt  on w.unit_type = mt.code
		   inner join datahub_analytics.dw_ips_dim_watertrackedreading wtr  on mr.reading_key=wtr.reading_key
		    where mr.no_read_flag='N' and mr.estmate_flag='N'
		 ) t1;`} );
