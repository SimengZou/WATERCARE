//  Name:       sp_dw_ips_stage_consump_stg4
//
//  Purpose:    Load consumption data - staging table dw_ips_stage_consump_stg4 loaded from staging table dw_ips_stage_consump_stg3 by job sp_dw_ips_stage_consump_stg4
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_dw_ips_stage_consump_stg4()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG4;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.DW_IPS_STAGE_CONSUMP_STG4
		 (COMP_KEY, COMP_READING_KEY, ROWNUM, PREV_READ_DATE, READ_DATE, READING, USAGE, READING_DAYS)
		 SELECT DISTINCT
		   dw_ips_stage_consump_stg3.comp_key, dw_ips_stage_consump_stg3.comp_reading_key, 
		   dw_ips_stage_consump_stg3.rownum, PrevReading.read_date as prev_read_date, dw_ips_stage_consump_stg3.read_date,
		   dw_ips_stage_consump_stg3.reading, dw_ips_stage_consump_stg3.usage, 
		   DATEDIFF ( day , PrevReading.read_date,  dw_ips_stage_consump_stg3.read_date) as reading_days
		 FROM datahub_staging.dw_ips_stage_consump_stg3
		 LEFT OUTER JOIN datahub_staging.dw_ips_stage_consump_stg3 PrevReading ON dw_ips_stage_consump_stg3.comp_key = PrevReading.comp_key 
		      AND (dw_ips_stage_consump_stg3.rownum - 1) = PrevReading.rownum ;`} );
