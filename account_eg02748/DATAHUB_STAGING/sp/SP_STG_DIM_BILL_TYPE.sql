CREATE OR REPLACE PROCEDURE "SP_STG_DIM_BILL_TYPE"()
...
//update the latest time included in extract for BILLTYPE table
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for BILLTYPE source table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_BILL_TYPE"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_BILL_TYPE (  ";
		sql_command += " BILL_TYPE_KEY, ";
		sql_command += " BILL_TYPE, ";
		sql_command += " BILL_TYPE_DESCRIPTION, ";
		sql_command += " ETL_SOURCE_DATETIME, ";
		sql_command += " ETL_RECORD_CREATED, ";
		sql_command += " ETL_RECORD_UPDATED)";
		sql_command += " SELECT ";
		sql_command += " BILLTYPE.BILLTYPEKEY AS BILL_TYPE_KEY, ";
		sql_command += " BILLTYPE.BILLTYPE AS BILL_TYPE, ";
		sql_command += " BILLTYPE.BILLDESC AS BILL_TYPE_DESCRIPTION, ";
		sql_command += " BILLTYPE.etl_load_datetime  AS ETL_SOURCE_DATETIME, ";
		sql_command += " current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += " current_timestamp() AS ETL_RECORD_UPDATED";
		sql_command += " FROM DATAHUB_TARGET.IPS_BILLING_BILLTYPE BILLTYPE ";
		sql_command += "   INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB BILLTYPE_CTRL ";
		sql_command += " ON BILLTYPE_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_BILL_TYPE'' AND BILLTYPE_CTRL.TABLE_NAME=''IPS_BILLING_BILLTYPE'' AND BILLTYPE_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
		sql_command += " WHERE  BILLTYPE.ETL_LOAD_DATETIME>BILLTYPE_CTRL.INC_ETL_LOAD_DATETIME;";
