CREATE OR REPLACE PROCEDURE "SP_STG_DIM_CUSTOMER_PROBLEM"()
...
//update the latest time included in extract for PROBDEFN table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_CRM_PROBDEFN ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_CRM_PROBDEFN'' AND schema_name=''DATAHUB_TARGET'';";
	    snowflake.execute({sqlText: update_datetime_sql_command}); 
		
//update the latest time included in extract for CUSTPROB table	
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_CRM_CUSTPROB) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_CRM_CUSTPROB'' AND schema_name=''DATAHUB_TARGET'';";
	    snowflake.execute({sqlText: update_datetime_sql_command}); 
		
	        
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for PROBDEFN and CUSTPROB tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_CUSTOMER_PROBLEM"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_CUSTOMER_PROBLEM (  ";
		...
		sql_command += "SELECT ";
		sql_command += "CUSTPROB.ADDDTTM AS Customer_Problem_Added_Date, ";
		sql_command += "PROBDEFN.PROBDESC AS Customer_Problem_Description, ";
		sql_command += "PROBDEFN.PROBCODE AS Customer_Problem_Code, ";
		sql_command += "CUSTPROB.PROB AS Customer_Problem_KEY, ";
		sql_command += "CUSTPROB.RESFLAG AS Resolved_Flag, ";
		sql_command += "CUSTPROB.SERVNO AS Customer_Problem_Number, ";
		sql_command += "CUSTPROB.ACCTKEY AS Account_KEY, ";
		sql_command += "CUSTPROB.COMPKEY AS Comp_Key, ";
		sql_command += "greatest(CUSTPROB.etl_load_datetime,PROBDEFN.ETL_LOAD_DATETIME) AS  ETL_LOAD_DATETIME, ";
		sql_command += "current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += "current_timestamp() AS ETL_RECORD_UPDATED ";
		sql_command += "FROM DATAHUB_TARGET.IPS_CRM_CUSTPROB CUSTPROB ";
		sql_command += "LEFT JOIN DATAHUB_TARGET.IPS_CRM_PROBDEFN PROBDEFN ";
		sql_command += "ON CUSTPROB.PROB = PROBDEFN.PROBKEY ";
	    sql_command += "LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB PROBDEFN_CTRL ";
	    sql_command += " ON PROBDEFN_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_CUSTOMER_PROBLEM'' AND PROBDEFN_CTRL.TABLE_NAME=''IPS_CRM_PROBDEFN'' AND PROBDEFN_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB CUSTPROB_CTRL ";
	    sql_command += "  ON CUSTPROB_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_CUSTOMER_PROBLEM'' AND CUSTPROB_CTRL.TABLE_NAME=''IPS_CRM_CUSTPROB'' AND CUSTPROB_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " WHERE  CUSTPROB.ETL_LOAD_DATETIME>CUSTPROB_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += " OR PROBDEFN.ETL_LOAD_DATETIME> PROBDEFN_CTRL.INC_ETL_LOAD_DATETIME  ;";
