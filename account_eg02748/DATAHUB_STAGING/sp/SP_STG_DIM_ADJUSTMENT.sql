CREATE OR REPLACE PROCEDURE "SP_STG_DIM_ADJUSTMENT"()
...
//update the latest time included in extract for ADJUSTMENTREASON table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_BILLING_ADJUSTMENTREASON ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_BILLING_ADJUSTMENTREASON'' AND schema_name=''DATAHUB_TARGET'';";
	    snowflake.execute({sqlText: update_datetime_sql_command}); 	
...
//update the latest time included in extract for COMMENTS table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_BILLING_COMMENTS ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_BILLING_COMMENTS'' AND schema_name=''DATAHUB_TARGET'';";
	    snowflake.execute({sqlText: update_datetime_sql_command}); 		
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for ADJUSTMENT, ADJUSTMENTREASON, LINEITEMSETUP  and COMMENTS tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	           
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_ADJUSTMENT"});
...
//  Run insert statement
		...
		var sql_command = " INSERT INTO DATAHUB_STAGING.STG_DIM_ADJUSTMENT";
		...
		sql_command += " SELECT";
		sql_command += " ADJUSTMENT.ADJUSTMENTKEY  AS Adjustment_Key,";
		sql_command += " ADJUSTMENT.ADJUSTMENTREASON AS Adjustment_Reason_Code,";
		sql_command += " ADJREASON.DESCRIPT AS Adjustment_Reason_Description,";
		sql_command += " LINEITEMSETUP.LINEITEM AS Adjustment_Type,";
		sql_command += " LINEITEMSETUP.DESCRIPTION AS Adjustment_Type_Description,";
		sql_command += " ADJUSTMENT.STATUS  AS Adjustment_Status_Code,";
		sql_command += " CASE ADJUSTMENT.STATUS ";
		sql_command += "   WHEN ''A'' THEN ''Approved''";
		sql_command += "   WHEN ''S'' THEN ''Submitted''";
		sql_command += "   WHEN ''R'' THEN ''Rejected''";
		sql_command += "   WHEN ''C'' THEN ''Canceled''";
		sql_command += "   WHEN ''E'' THEN ''Pre-Approved''";
		sql_command += "   END AS Adjustment_Status_Description,";
		sql_command += " ADJUSTMENT.STATUSDATE  AS Adjustment_Status_Date,";
		// changed from: sql_command += " substring(COMMENTS.TEXT_SEARCH,1,2000) AS Adjustment_Comments,";
		sql_command += " '''' AS Adjustment_Comments,";
		sql_command += " ADJUSTMENT.PARENTADJUSTMENTKEY  AS Parent_Adjustment,";
		sql_command += " ADJUSTMENT.PRIORITY  AS Adjustment_Priority,";
		sql_command += " ADJUSTMENT.ADJUSTMENTDTTM  AS Adjustment_Datetime,";
		sql_command += " ADJUSTMENT.ADDDTTM  AS Adjustment_Added_Date,";
		sql_command += " ADJUSTMENT.MODDTTM  AS Adjustment_Modified_Date,";
		sql_command += " ADJUSTMENT.ADJUSTMENTSCOUNT  AS Adjustment_Count,";
		sql_command += " ADJUSTMENT.FULLYPAIDSTAT  AS Is_Adjustment_Fully_Paid,";
		sql_command += " ADJUSTMENT.PROPERTYBASEDFLAG  AS Is_Adjustment_Property_Based,";
		sql_command += " ADJUSTMENT.ADJUSTEDBY  AS Adjusted_By,";
		sql_command += " ADJUSTMENT.STATUSBY  AS Adjustment_Status_By,";
		sql_command += " ADJUSTMENT.ADDBY  AS Adjustment_Added_By,";
		sql_command += " ADJUSTMENT.MODBY  AS Adjustment_Modified_By,";
		sql_command += " GREATEST(ADJUSTMENT.etl_load_datetime, COALESCE(ADJREASON.etl_load_datetime, ADJUSTMENT.etl_load_datetime), ";
		sql_command += " COALESCE(COMMENTS.etl_load_datetime,ADJUSTMENT.etl_load_datetime), COALESCE(LINEITEMSETUP.etl_load_datetime,ADJUSTMENT.etl_load_datetime) ) AS ETL_SOURCE_DATETIME,";
		sql_command += " current_timestamp() AS ETL_RECORD_CREATED,";
		sql_command += " current_timestamp() AS ETL_RECORD_UPDATED";
		sql_command += " FROM DATAHUB_TARGET.IPS_BILLING_ADJUSTMENT ADJUSTMENT";
		sql_command += " LEFT JOIN DATAHUB_TARGET.IPS_BILLING_ADJUSTMENTREASON ADJREASON";
		sql_command += "   ON upper(ADJUSTMENT.ADJUSTMENTREASON) = upper(ADJREASON.code)";
		sql_command += " LEFT JOIN DATAHUB_TARGET.IPS_BILLING_LINEITEMSETUP LINEITEMSETUP";
		sql_command += "   ON ADJUSTMENT.ADJTYPESUKEY =lineitemsetup.lineitemsetupkey";
		sql_command += " LEFT JOIN DATAHUB_TARGET.IPS_BILLING_COMMENTS COMMENTS  ";
		sql_command += "   ON ADJUSTMENT.COMMENTSKEY = COMMENTS.commentskey";
		
	    sql_command += " LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB ADJUSTMENT_CTRL ";
	    sql_command += "  ON ADJUSTMENT_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_ADJUSTMENT'' AND ADJUSTMENT_CTRL.TABLE_NAME=''IPS_BILLING_ADJUSTMENT'' AND ADJUSTMENT_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB ADJUSTMENTREASON_CTRL ";
	    sql_command += "  ON ADJUSTMENTREASON_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_ADJUSTMENT'' AND ADJUSTMENTREASON_CTRL.TABLE_NAME=''IPS_BILLING_ADJUSTMENTREASON'' AND ADJUSTMENTREASON_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB LINEITEMSETUP_CTRL ";
	    sql_command += "  ON LINEITEMSETUP_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_LINEITEMSETUP'' AND LINEITEMSETUP_CTRL.TABLE_NAME=''IPS_BILLING_LINEITEMSETUP'' AND LINEITEMSETUP_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		sql_command += " LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB COMMENTS_CTRL ";
	    sql_command += " ON COMMENTS_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_COMMENTS'' AND COMMENTS_CTRL.TABLE_NAME=''IPS_BILLING_COMMENTS'' AND COMMENTS_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
	
		
		sql_command += " WHERE  ADJUSTMENT.ETL_LOAD_DATETIME>ADJUSTMENT_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += " OR  ADJREASON.ETL_LOAD_DATETIME>ADJUSTMENTREASON_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += " OR  LINEITEMSETUP.ETL_LOAD_DATETIME>LINEITEMSETUP_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += " OR COMMENTS.ETL_LOAD_DATETIME> COMMENTS_CTRL.INC_ETL_LOAD_DATETIME  ;";
