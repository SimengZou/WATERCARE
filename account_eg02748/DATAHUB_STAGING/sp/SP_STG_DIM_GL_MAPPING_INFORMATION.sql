CREATE OR REPLACE PROCEDURE "SP_STG_DIM_GL_MAPPING_INFORMATION"()
...
//update the latest time included in extract for GL_MAPPING_INFORMATION table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_WSLBILLING_GLMAPPING ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_WSLBILLING_GLMAPPING'' AND schema_name=''DATAHUB_TARGET'';";
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for GL_MAPPING_INFORMATION source tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_GL_MAPPING_INFORMATION"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_GL_MAPPING_INFORMATION (  ";
		sql_command += " GL_MAPPING_CODE, ";
		sql_command += " GL_MAPPING_DESCRIPTION, ";
		sql_command += " COST_CENTRE_CODE, ";
		sql_command += " GL_ACCOUNT_CODE, ";
		sql_command += " ETL_SOURCE_DATETIME, ";
		sql_command += " ETL_RECORD_CREATED, ";
		sql_command += " ETL_RECORD_UPDATED)";
		sql_command += " SELECT ";
		sql_command += " GLMAPPING.CODE AS GL_MAPPING_CODE,";
		sql_command += " GLMAPPING.DESCRIPT AS GL_MAPPING_DESCRIPTION,";
		sql_command += " CASE WHEN GLMAPPING.GL1 IS NULL ";
		sql_command += "      THEN ''NA'' ";
		sql_command += "      ELSE SUBSTRING(GLMAPPING.GL1,0,4) END AS COST_CENTRE_CODE, ";
		sql_command += " CASE WHEN GLMAPPING.GL1 IS NULL ";
		sql_command += "      THEN ''NA'' ";
		sql_command += "      ELSE SUBSTRING(GLMAPPING.GL1,5) END AS GL_ACCOUNT_CODE, ";
		sql_command += " etl_load_datetime AS ETL_SOURCE_DATETIME, ";
		sql_command += " current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += " current_timestamp() AS ETL_RECORD_UPDATED";
		sql_command += " FROM DATAHUB_TARGET.IPS_WSLBILLING_GLMAPPING GLMAPPING";
		sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB GLMAPPING_CTRL ";
		sql_command += "    ON GLMAPPING_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_GL_MAPPING_INFORMATION'' AND GLMAPPING_CTRL.TABLE_NAME=''IPS_WSLBILLING_GLMAPPING'' AND GLMAPPING_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
		sql_command += " WHERE  GLMAPPING.ETL_LOAD_DATETIME>GLMAPPING_CTRL.INC_ETL_LOAD_DATETIME;";
