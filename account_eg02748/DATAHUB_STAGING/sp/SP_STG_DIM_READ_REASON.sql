CREATE OR REPLACE PROCEDURE "SP_STG_DIM_READ_REASON"()
...
//update the latest time included in extract for READINGREASON table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_METERREADINGREASON ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_ASSETMANAGEMENT_WATER_METERREADINGREASON'' AND schema_name=''DATAHUB_TARGET'';";
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for READINGREASON table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_READ_REASON"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_READ_REASON (  ";
		sql_command += "READ_REASON_CODE, ";
		sql_command += "READ_REASON_DESCRIPTION, ";
		sql_command += "ETL_SOURCE_DATETIME, ";
		sql_command += "ETL_RECORD_CREATED, ";
		sql_command += "ETL_RECORD_UPDATED) ";
		sql_command += "SELECT ";
		sql_command += "METERREADINGREASON.CODE AS Read_Reason_code, ";
		sql_command += "METERREADINGREASON.DESCRIPT AS Read_Reason_Description, ";
		sql_command += "METERREADINGREASON.etl_load_datetime AS  ETL_SOURCE_DATETIME, ";
		sql_command += "current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += "current_timestamp() AS ETL_RECORD_UPDATED ";
		sql_command += "FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_METERREADINGREASON METERREADINGREASON ";
		
		sql_command += "LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB READINGREASON_CTRL ";
	    sql_command += " ON READINGREASON_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_READ_REASON'' AND READINGREASON_CTRL.TABLE_NAME=''IPS_ASSETMANAGEMENT_WATER_METERREADINGREASON'' AND READINGREASON_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		
	    sql_command += "  WHERE  METERREADINGREASON.ETL_LOAD_DATETIME> READINGREASON_CTRL.INC_ETL_LOAD_DATETIME ; ";
