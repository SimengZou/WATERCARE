CREATE OR REPLACE PROCEDURE "SP_STG_DIM_BILL"()
...
//update the latest time included in extract for BILLTYPE table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_BILLING_BILLTYPE ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_BILLING_BILLTYPE'' AND schema_name=''DATAHUB_TARGET'';";
	    snowflake.execute({sqlText: update_datetime_sql_command}); 
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for BILL and BILLTYPE source tables.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_BILL"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_BILL (  ";
		sql_command += " BILL_KEY, ";
		sql_command += " BILL_NUMBER, ";
		sql_command += " BILL_ESTIMATE_FLAG, ";
		sql_command += " BILL_PAID_STATUS, ";
		sql_command += " BILL_TYPE, ";
		sql_command += " BILL_DESCRIPTION, ";
		sql_command += " ADDRESS_LINE1, ";
		sql_command += " ADDRESS_LINE2, ";
		sql_command += " BILL_TO, ";
		sql_command += " BILL_TO_LINE2, ";
		sql_command += " CITY, ";
		sql_command += " COUNTRY, ";
		sql_command += " STATE, ";
		sql_command += " ZIP, ";
		sql_command += " CREDIT_FLAG, ";
		sql_command += " DELINQUENT_DATE, ";
		sql_command += " PREVIOUS_BILL_KEY,";
		sql_command += " ETL_SOURCE_DATETIME, ";
		sql_command += " ETL_RECORD_CREATED, ";
		sql_command += " ETL_RECORD_UPDATED)";
		sql_command += " SELECT ";
		sql_command += " BILL.BILLKEY AS  Bill_Key,";
		sql_command += " BILL.BILLNO AS  Bill_Number,";
		sql_command += " BILL.ESTIMATEFLAG AS  Bill_Estimate_Flag,";
		sql_command += " BILL.PAIDSTAT AS  Bill_Paid_Status,";
		sql_command += " BILLTYPE.BILLTYPE AS  Bill_Type,";
		sql_command += " BILLTYPE.BILLDESC AS  Bill_Description,";
		sql_command += " BILL.ADDR1 AS ADDRESS_LINE1, ";
		sql_command += " BILL.ADDR2 AS ADDRESS_LINE2, ";
		sql_command += " BILL.BILLTO AS BILL_TO, ";
		sql_command += " BILL.BILLTOLINE2 AS BILL_TO_LINE2, ";
		sql_command += " BILL.CITY AS CITY, ";
		sql_command += " BILL.COUNTRY AS COUNTRY, ";
		sql_command += " BILL.STATE AS STATE, ";
		sql_command += " BILL.ZIP AS ZIP, ";
		sql_command += " BILL.CREDITFLAG AS CREDIT_FLAG, ";
		sql_command += " BILL.DELINQUENTDATE AS DELINQUENT_DATE, ";
		sql_command += " BILL.PREVIOUSBILLKEY  AS PREVIOUS_BILL_KEY, ";
		sql_command += " GREATEST(BILL.etl_load_datetime, BILLTYPE.etl_load_datetime)  AS ETL_SOURCE_DATETIME, ";
		sql_command += " current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += " current_timestamp() AS ETL_RECORD_UPDATED";
		sql_command += " FROM DATAHUB_TARGET.IPS_BILLING_BILL BILL";
		sql_command += " LEFT JOIN DATAHUB_TARGET.IPS_BILLING_BILLTYPE BILLTYPE";
		sql_command += "   ON BILL.BILLTYPEKEY =BILLTYPE.BILLTYPEKEY";
		sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB BILL_CTRL ";
		sql_command += "     ON BILL_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_BILL'' AND BILL_CTRL.TABLE_NAME=''IPS_BILLING_BILL'' AND BILL_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
		sql_command += " INNER JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB BILLTYPE_CTRL ";
		sql_command += "     ON BILLTYPE_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_BILL'' AND BILLTYPE_CTRL.TABLE_NAME=''IPS_BILLING_BILLTYPE'' AND BILLTYPE_CTRL.SCHEMA_NAME=''DATAHUB_TARGET''";
		sql_command += " WHERE  BILL.ETL_LOAD_DATETIME>BILL_CTRL.INC_ETL_LOAD_DATETIME ";
		sql_command += "     OR BILLTYPE.ETL_LOAD_DATETIME>BILLTYPE_CTRL.INC_ETL_LOAD_DATETIME;";
