CREATE OR REPLACE PROCEDURE "SP_STG_DIM_READER_CODE"()
...
//update the latest time included in extract for METERREADERCODE table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_METERREADERCODE ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_ASSETMANAGEMENT_WATER_METERREADERCODE'' AND schema_name=''DATAHUB_TARGET'';";
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for METERREADERCODE table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_READER_CODE"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_READER_CODE (  ";
		sql_command += "READER_CODE, ";
		sql_command += "READER_CODE_DESCRIPTION, ";
		sql_command += "ETL_SOURCE_DATETIME, ";
		sql_command += "ETL_RECORD_CREATED, ";
		sql_command += "ETL_RECORD_UPDATED) ";
		sql_command += "SELECT ";
		sql_command += "METERREADERCODE.CODE AS READER_code, ";
		sql_command += "METERREADERCODE.DESCRIPT AS READER_CODE_Description, ";
		sql_command += "METERREADERCODE.etl_load_datetime AS  ETL_SOURCE_DATETIME, ";
		sql_command += "current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += "current_timestamp() AS ETL_RECORD_UPDATED ";
		sql_command += "FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_METERREADERCODE METERREADERCODE ";
		
		sql_command += "LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB READERCODE_CTRL ";
	    sql_command += " ON READERCODE_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_READER_CODE'' AND READERCODE_CTRL.TABLE_NAME=''IPS_ASSETMANAGEMENT_WATER_METERREADERCODE'' AND READERCODE_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		
	    sql_command += "  WHERE  METERREADERCODE.ETL_LOAD_DATETIME> READERCODE_CTRL.INC_ETL_LOAD_DATETIME; ";
