//  Name:       sp_stg37_ips_sm_billing_meter
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//  stg37_ips_sm_billing_meter loaded from staging tables:
//     datahub_staging.STG32_IPS_SM_billing_meter_old_routes 
//     DATAHUB_STAGING.STG35_IPS_SM_BILLING_OUT_OF_CYCLE
//
//    by job sp_stg37_ips_sm_billing_meter
//  Merge all meters and routes required for the day - including the Final and Special readings scheduled 
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_stg37_ips_sm_billing_meter()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG37_IPS_SM_billing_meter;"});
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_STAGING.STG37_IPS_SM_BILLING_METER (UNITID, UNITID_CLEAN, SERNO, COMPKEY, ROUTEID, READING_TYPE, CURRENT_READ_DATE) 
				SELECT ips.UNITID, ips.UNITID_CLEAN, ips.SERNO, ips.COMPKEY, ips.ROUTEID, 'ROUTINE' AS READING_TYPE, rday.READ_DATE as CURRENT_READ_DATE
				FROM datahub_staging.STG32_IPS_SM_billing_meter_old_routes ips,
                    datahub_staging.stg10_ips_sm_billing_readday rday
				    WHERE ips.UNITID_CLEAN NOT IN (SELECT UNITID_CLEAN FROM  DATAHUB_STAGING.STG35_IPS_SM_BILLING_OUT_OF_CYCLE )
				UNION 
				SELECT UNITID, UNITID_CLEAN, SERNO, COMPKEY, ROUTEID, 
				CASE WHEN REQUEST_TYPE='Final' THEN 'FINAL' ELSE 'SPR' END AS READING_TYPE,
				SCHEDULED_READDATE AS CURRENT_READ_DATE
				FROM  DATAHUB_STAGING.STG35_IPS_SM_BILLING_OUT_OF_CYCLE; `} );
