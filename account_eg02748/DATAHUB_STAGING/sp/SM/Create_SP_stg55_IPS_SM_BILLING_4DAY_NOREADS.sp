//  Name:       sp_stg55_ips_sm_billing_4day_noreads
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//   stg55_ips_sm_billing_4day_noreads loaded from staging table: datahub_staging.stg50_ips_sm_billing_smart_read
//  by job sp_stg55_ips_sm_billing_4day_noreads
//  Stores meters where no reading was found in smartmeters within 4 days of the expected read
//
//  Description:
//
//  Stored procedure for IPS billing for smart meters from datalake
//
//  Parameters: LAGDAYS FLOAT- number of days to allow for teh lag - should be 4, but can be modeified - fro testing for example to 2
//
//
...
create or replace procedure datahub_staging.sp_stg55_ips_sm_billing_4day_noreads(LAGDAYS FLOAT)
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG55_IPS_SM_BILLING_4DAY_NOREADS;"});
...
//  Run insert statement  
        ...
        try
            {            
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_STAGING.STG55_IPS_SM_BILLING_4DAY_NOREADS 
			(UNITID, COMPKEY, ROUTEID, SM_READING, M_UNIT, READING_TYPE, READ_DATE, SM_READING_DATE, EXTRACT_READDATE, ETL_LOAD_DATETIME) 
			SELECT rd.UNITID, rd.COMPKEY, rd.ROUTEID, rd.SM_READING, rd.M_UNIT, rd.READING_TYPE, rd.READ_DATE , rd.SM_READING_DATE, current_READ_DATE AS extract_readdate,
			current_timestamp() as ETL_LOAD_DATETIME
			FROM  datahub_staging.stg50_ips_sm_billing_smart_read rd 
			WHERE (dateadd(DAY,` + LAGDAYS.toString() +`-1,rd.READ_DATE) >= current_READ_DATE AND SM_READING IS NULL) /* less than 4 days have passed with NO reading, store in the table for the next day check*/ ;`} );
