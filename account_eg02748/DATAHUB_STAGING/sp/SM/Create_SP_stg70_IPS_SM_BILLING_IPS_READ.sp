//  Name:       sp_stg70_ips_sm_billing_ips_read
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//   stg70_ips_sm_billing_ips_read loaded from staging tables: 
//     STG38_IPS_SM_billing_all_meters
//     stg60_ips_sm_billing_readkey
//     stg50_ips_sm_billing_smart_read
// and IPS source table in target schema:
//     IPS_ASSETMANAGEMENT_WATER_METERREADING  
//  by job sp_stg70_ips_sm_billing_ips_read
//
//      Get the most recent reading from IPS  
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_stg70_ips_sm_billing_ips_read()
    returns string
    language javascript
    execute as caller
    as
...
//  Truncate target table 
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG70_IPS_SM_billing_ips_read;"});
...
//  Run insert statement  
        ...
        try
            {            
            var sqlStmt = snowflake.createStatement( {sqlText: ` INSERT INTO DATAHUB_STAGING.STG70_IPS_SM_BILLING_IPS_READ 
			(UNITID, COMPKEY, ROUTEID, IPS_READ_DATE, READING, FIELDNOTES, NOREADFLAG, READ_DATE, CURRENT_READ_DATE)
						SELECT m.UNITID, m.compkey, m.ROUTEID, to_date(R.READDTTM) AS ips_read_date, R.READING , R.FIELDNOTES, r.NOREADFLAG,
			rk.read_date, rk.current_read_date
			FROM DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_METERREADING R
			INNER JOIN (SELECT *
						FROM datahub_staging.STG60_IPS_SM_billing_readkey		
					   ) RK ON R.READINGKEY = RK.READINGKEY
			INNER JOIN (SELECT UNITID, COMPKEY, ROUTEID 
						FROM datahub_staging.STG39_IPS_SM_billing_all_meters
					   ) M 
			  ON M.COMPKEY = R.COMPKEY;	`} );
