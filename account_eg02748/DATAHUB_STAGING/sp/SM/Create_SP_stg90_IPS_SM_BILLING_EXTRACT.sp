//  Name:       sp_stg90_ips_sm_billing_extract
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - staging table 
//   stg90_ips_sm_billing_extract loaded from staging tables: 
//     stg80_ips_sm_billing_all_readings
//  by job sp_stg90_ips_sm_billing_extract
//
//      Create final billing extract  
//
//  Description:
//
...
create or replace procedure datahub_staging.sp_stg90_ips_sm_billing_extract()
    returns string
    language javascript
    execute as caller
    as
...
//  Run insert statement  
        ...
        try
            {            
            var sqlStmt = snowflake.createStatement( {sqlText: `  INSERT INTO DATAHUB_STAGING.STG90_IPS_SM_BILLING_EXTRACT 
			(ROUTE, READING_DATE, METER, REGISTER_NUMBER, READING, READ_TYPE, REASON_UNREAD, READER, NOTES, READING_TYPE)
			SELECT ROUTE_ID AS ROUTE,
			CASE WHEN UNREAD_NOTE='NRSMART' THEN read_date else actual_read_date end AS reading_date,
			 UNIT_ID AS METER,
			 1 AS REGISTER_NUMBER,
			 READING,
			 'R' AS READ_TYPE ,
			 UNREAD_NOTE AS REASON_UNREAD,
			 'DATALAKESR' AS READER,
			 null AS NOTES,
			 READING_TYPE
			FROM datahub_staging.STG80_IPS_SM_billing_all_readings;`} );
