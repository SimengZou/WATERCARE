CREATE OR REPLACE PROCEDURE "SP_STG_DIM_ROUTE"()
...
//  Get the last date that will be extracted  - for each source table

//update the latest time included in extract for ROUTE table
		update_datetime_sql_command = "UPDATE datahub_logging.etl_load_control_job ";
		update_datetime_sql_command += "SET new_etl_load_datetime=(SELECT max(ETL_LOAD_DATETIME) FROM DATAHUB_TARGET.IPS_METERMANAGEMENT_WATER_ROUTE ) ";
		update_datetime_sql_command += "WHERE stg_process_name = ''" + process_name + "'' AND table_name=''IPS_METERMANAGEMENT_WATER_ROUTE'' AND schema_name=''DATAHUB_TARGET'';";
	...
	if( debug === "True" ) 
            {
            debug_statement = "call datahub_logging.sp_etl_log_process_debug(''" + process_name + "'',''Date updated for incremental extract for ROUTE table.'');";
            snowflake.execute({sqlText: debug_statement}); 
            }	
...
//  Drop target table (if exists)
        ...
        snowflake.execute ({sqlText: "TRUNCATE TABLE DATAHUB_STAGING.STG_DIM_ROUTE"});
...
//  Run insert statement
        ...
        var sql_command = "INSERT INTO DATAHUB_STAGING.STG_DIM_ROUTE (  ";
		sql_command += "ROUTE_KEY, ";
		sql_command += "ROUTE_DESCRIPTION, ";
		sql_command += "ROUTE_ID,";
		sql_command += "LAST_READ_DATE, ";
		sql_command += "NEXT_READ_DATE, ";
		sql_command += "BILLING_ROUTE_FLAG, ";
		sql_command += "OUT_FOR_ACTIVITY_FLAG, ";
		sql_command += "READING_CYCLE, ";
		sql_command += "TIME_INTERVAL, ";
		sql_command += "TIME_INTERVAL_UNIT, ";
		sql_command += "ETL_SOURCE_DATETIME, ";
		sql_command += "ETL_RECORD_CREATED, ";
		sql_command += "ETL_RECORD_UPDATED)";
		sql_command += "SELECT ";
		sql_command += "ROUTE.ROUTEKEY AS Route_Key,";
		sql_command += "ROUTE.ROUTEDESC AS Route_Description,";
		sql_command += "ROUTE.ROUTEID AS Route_ID,";
		sql_command += "ROUTE.LASTDTTM AS Last_Read_Date,";
		sql_command += "ROUTE.NEXTDTTM AS Next_Read_Date,";
		sql_command += "ROUTE.BILLRTFLAG AS BILLING_ROUTE_FLAG,";
		sql_command += "ROUTE.OUTFORACT AS OUTFORACT,";
		sql_command += "ROUTE.READINGCYCLE AS READINGCYCLE,";
		sql_command += "ROUTE.TIMEINT AS TIMEINT,";
		sql_command += "ROUTE.TIMEUNIT AS TIMEUNIT,";
		sql_command += "ROUTE.etl_load_datetime AS  ETL_SOURCE_DATETIME, ";
		sql_command += "current_timestamp() AS ETL_RECORD_CREATED, ";
		sql_command += "current_timestamp() AS ETL_RECORD_UPDATED ";
		sql_command += "FROM DATAHUB_TARGET.IPS_METERMANAGEMENT_WATER_ROUTE ROUTE ";
		
		sql_command += "LEFT JOIN DATAHUB_LOGGING.ETL_LOAD_CONTROL_JOB ROUTE_CTRL ";
	    sql_command += " ON ROUTE_CTRL.STG_PROCESS_NAME=''SP_STG_DIM_ROUTE'' AND ROUTE_CTRL.TABLE_NAME=''IPS_METERMANAGEMENT_WATER_ROUTE'' AND ROUTE_CTRL.SCHEMA_NAME=''DATAHUB_TARGET'' ";
		
	    sql_command += "  WHERE  ROUTE.ETL_LOAD_DATETIME>ROUTE_CTRL.INC_ETL_LOAD_DATETIME ; ";
