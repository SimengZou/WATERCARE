CREATE OR REPLACE SECURE VIEW ${buildvar.env}_WBP_SHARE.DATAHUB_SHARED_WBP.VWS_PLANT_ROADS_BRIDGES
AS
SELECT
...
, XROADSNBRIDGES.INVLEVEL                         AS "Invert Level (M)"
, A1.WARRANTYEND           						  AS "Warranty End Date"
, A1.WARRANTYSTART         						  AS "Warranty Start Date"
, XROADSNBRIDGES.FUNCAREA                         AS "Functional Area"
, XROADSNBRIDGES.GRDLEVEL                         AS "Ground Level"
, XROADSNBRIDGES.LOADRATING                       AS "Load Rating (kN)"
, XROADSNBRIDGES.PHOTO3D                          AS "Photo/3D Model"
...
FROM
  ...
  LEFT OUTER JOIN "${buildvar.env}_WCC_DATAWAREHOUSE"."DATAHUB_TARGET".IPS_WSLASSETSCONFIGURED_XROADSNBRIDGES AS XROADSNBRIDGES ON A2.COMPKEY = XROADSNBRIDGES.COMPKEY 
  WHERE A2.OWN = 'WBP' and A4.COMPCODE = 'ROBR';
