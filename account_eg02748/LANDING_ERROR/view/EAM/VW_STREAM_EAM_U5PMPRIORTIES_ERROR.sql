CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5PMPRIORTIES_ERROR AS SELECT src, 'EAM_U5PMPRIORTIES' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_COMPLETE_DAYS), '0'), 38, 10) is null and 
                    src:PMP_COMPLETE_DAYS is not null) THEN
                    'PMP_COMPLETE_DAYS contains a non-numeric value : \'' || AS_VARCHAR(src:PMP_COMPLETE_DAYS) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_CYCLE_LENGTH), '0'), 38, 10) is null and 
                    src:PMP_CYCLE_LENGTH is not null) THEN
                    'PMP_CYCLE_LENGTH contains a non-numeric value : \'' || AS_VARCHAR(src:PMP_CYCLE_LENGTH) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_RELEASE_DAYS), '0'), 38, 10) is null and 
                    src:PMP_RELEASE_DAYS is not null) THEN
                    'PMP_RELEASE_DAYS contains a non-numeric value : \'' || AS_VARCHAR(src:PMP_RELEASE_DAYS) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_COMPLETE_DAYS), '0'), 38, 10) is null and 
                    src:PMP_COMPLETE_DAYS is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_CYCLE_LENGTH), '0'), 38, 10) is null and 
                    src:PMP_CYCLE_LENGTH is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PMP_RELEASE_DAYS), '0'), 38, 10) is null and 
                    src:PMP_RELEASE_DAYS is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:UPDATECOUNT), '0'), 38, 10) is null and 
