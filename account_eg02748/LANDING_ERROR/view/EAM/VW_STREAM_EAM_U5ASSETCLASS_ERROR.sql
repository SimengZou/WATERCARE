CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_EAM_U5ASSETCLASS_ERROR AS SELECT src, 'EAM_U5ASSETCLASS' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ASCL_IPSCOMPTYPE), '0'), 38, 10) is null and 
                    src:ASCL_IPSCOMPTYPE is not null) THEN
                    'ASCL_IPSCOMPTYPE contains a non-numeric value : \'' || AS_VARCHAR(src:ASCL_IPSCOMPTYPE) || '\'' WHEN 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_EAM_U5ASSETCLASS as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ASCL_IPSCOMPTYPE), '0'), 38, 10) is null and 
                    src:ASCL_IPSCOMPTYPE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CREATED), '1900-01-01')) is null and 
