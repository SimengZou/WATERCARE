CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PROJECT_PROJVLTN_ERROR AS SELECT src, 'IPS_CDR_PROJECT_PROJVLTN' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROJVLTNKEY), '0'), 38, 10) is null and 
                    src:PROJVLTNKEY is not null) THEN
                    'PROJVLTNKEY contains a non-numeric value : \'' || AS_VARCHAR(src:PROJVLTNKEY) || '\'' WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:PROJVLTNKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_PROJECT_PROJVLTN as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROJVLTNKEY), '0'), 38, 10) is null and 
                    src:PROJVLTNKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SUVLTNKEY), '0'), 38, 10) is null and 
