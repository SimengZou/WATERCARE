CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PLANNING_PLANHEARDETAILTYPE_ERROR AS SELECT src, 'IPS_CDR_PLANNING_PLANHEARDETAILTYPE' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APPLICATIONTYPEKEY), '0'), 38, 10) is null and 
                    src:APPLICATIONTYPEKEY is not null) THEN
                    'APPLICATIONTYPEKEY contains a non-numeric value : \'' || AS_VARCHAR(src:APPLICATIONTYPEKEY) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PLANHEARTYPE), '0'), 38, 10) is null and 
                    src:PLANHEARTYPE is not null) THEN
                    'PLANHEARTYPE contains a non-numeric value : \'' || AS_VARCHAR(src:PLANHEARTYPE) || '\'' WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:PLANHEARDTLTYPEKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_PLANNING_PLANHEARDETAILTYPE as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ACCESSID), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APPLICATIONTYPEKEY), '0'), 38, 10) is null and 
                    src:APPLICATIONTYPEKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:DISPLAY), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PLANHEARTYPE), '0'), 38, 10) is null and 
                    src:PLANHEARTYPE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:VARIATION_ID), '0'), 38, 10) is null and 
