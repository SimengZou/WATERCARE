CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWPS_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_WATER_COMPWPS' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NOPUMPS), '0'), 38, 10) is null and 
                    src:NOPUMPS is not null) THEN
                    'NOPUMPS contains a non-numeric value : \'' || AS_VARCHAR(src:NOPUMPS) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PUDISSIZE), '0'), 38, 10) is null and 
                    src:PUDISSIZE is not null) THEN
                    'PUDISSIZE contains a non-numeric value : \'' || AS_VARCHAR(src:PUDISSIZE) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PUMPCAP), '0'), 38, 10) is null and 
                    src:PUMPCAP is not null) THEN
                    'PUMPCAP contains a non-numeric value : \'' || AS_VARCHAR(src:PUMPCAP) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WETWLELEV), '0'), 38, 10) is null and 
                    src:WETWLELEV is not null) THEN
                    'WETWLELEV contains a non-numeric value : \'' || AS_VARCHAR(src:WETWLELEV) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WETWLVOL), '0'), 38, 10) is null and 
                    src:WETWLVOL is not null) THEN
                    'WETWLVOL contains a non-numeric value : \'' || AS_VARCHAR(src:WETWLVOL) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWPS as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PUDISSIZE), '0'), 38, 10) is null and 
                    src:PUDISSIZE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PUMPCAP), '0'), 38, 10) is null and 
                    src:PUMPCAP is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SEGKEY), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WETWLELEV), '0'), 38, 10) is null and 
                    src:WETWLELEV is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WETWLVOL), '0'), 38, 10) is null and 
                    src:WETWLVOL is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XCOORD), '0'), 38, 10) is null and 
