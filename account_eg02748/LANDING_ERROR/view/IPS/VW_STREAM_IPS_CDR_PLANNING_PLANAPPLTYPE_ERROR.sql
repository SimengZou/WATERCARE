CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PLANNING_PLANAPPLTYPE_ERROR AS SELECT src, 'IPS_CDR_PLANNING_PLANAPPLTYPE' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PLANAPPLTYPECATKEY), '0'), 38, 10) is null and 
                    src:PLANAPPLTYPECATKEY is not null) THEN
                    'PLANAPPLTYPECATKEY contains a non-numeric value : \'' || AS_VARCHAR(src:PLANAPPLTYPECATKEY) || '\'' WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:APPLANDEFNKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_PLANNING_PLANAPPLTYPE as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ACCID), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PLANAPPLTYPECATKEY), '0'), 38, 10) is null and 
                    src:PLANAPPLTYPECATKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PRIMARYSITETYPE), '0'), 38, 10) is null and 
