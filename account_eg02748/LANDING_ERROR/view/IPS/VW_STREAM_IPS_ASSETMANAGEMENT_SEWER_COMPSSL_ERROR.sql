CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSSL_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_SEWER_COMPSSL' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BLDGKEY), '0'), 38, 10) is null and 
                    src:BLDGKEY is not null) THEN
                    'BLDGKEY contains a non-numeric value : \'' || AS_VARCHAR(src:BLDGKEY) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NOTAPS), '0'), 38, 10) is null and 
                    src:NOTAPS is not null) THEN
                    'NOTAPS contains a non-numeric value : \'' || AS_VARCHAR(src:NOTAPS) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROPLNDPTH), '0'), 38, 10) is null and 
                    src:PROPLNDPTH is not null) THEN
                    'PROPLNDPTH contains a non-numeric value : \'' || AS_VARCHAR(src:PROPLNDPTH) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:TAPADDRKEY), '0'), 38, 10) is null and 
                    src:TAPADDRKEY is not null) THEN
                    'TAPADDRKEY contains a non-numeric value : \'' || AS_VARCHAR(src:TAPADDRKEY) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:TAPDIST), '0'), 38, 10) is null and 
                    src:TAPDIST is not null) THEN
                    'TAPDIST contains a non-numeric value : \'' || AS_VARCHAR(src:TAPDIST) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSSL as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROPLNDPTH), '0'), 38, 10) is null and 
                    src:PROPLNDPTH is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SEGKEY), '0'), 38, 10) is null and 
                    src:SEGKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SITEKEY), '0'), 38, 10) is null and 
                    src:SITEKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:TAPADDRKEY), '0'), 38, 10) is null and 
                    src:TAPADDRKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:TAPDIST), '0'), 38, 10) is null and 
                    src:TAPDIST is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:USGAREAKEY), '0'), 38, 10) is null and 
