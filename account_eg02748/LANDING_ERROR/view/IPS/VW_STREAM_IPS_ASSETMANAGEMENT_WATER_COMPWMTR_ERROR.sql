CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWMTR_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_WATER_COMPWMTR' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HIGHREG), '0'), 38, 10) is null and 
                    src:HIGHREG is not null) THEN
                    'HIGHREG contains a non-numeric value : \'' || AS_VARCHAR(src:HIGHREG) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWREG), '0'), 38, 10) is null and 
                    src:LOWREG is not null) THEN
                    'LOWREG contains a non-numeric value : \'' || AS_VARCHAR(src:LOWREG) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:METERSZ), '0'), 38, 10) is null and 
                    src:METERSZ is not null) THEN
                    'METERSZ contains a non-numeric value : \'' || AS_VARCHAR(src:METERSZ) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWMTR as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWREG), '0'), 38, 10) is null and 
                    src:LOWREG is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:MAINKEY), '0'), 38, 10) is null and 
                    src:MAINKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:METERSZ), '0'), 38, 10) is null and 
                    src:METERSZ is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MFGDATE), '1900-01-01')) is null and 
