CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PROJECT_PROJPROCESSSTATE_ERROR AS SELECT src, 'IPS_CDR_PROJECT_PROJPROCESSSTATE' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NEXTSTATEFRMLA), '0'), 38, 10) is null and 
                    src:NEXTSTATEFRMLA is not null) THEN
                    'NEXTSTATEFRMLA contains a non-numeric value : \'' || AS_VARCHAR(src:NEXTSTATEFRMLA) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NEXTSTATEKEY), '0'), 38, 10) is null and 
                    src:NEXTSTATEKEY is not null) THEN
                    'NEXTSTATEKEY contains a non-numeric value : \'' || AS_VARCHAR(src:NEXTSTATEKEY) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:STATDATETYPE), '0'), 38, 10) is null and 
                    src:STATDATETYPE is not null) THEN
                    'STATDATETYPE contains a non-numeric value : \'' || AS_VARCHAR(src:STATDATETYPE) || '\'' WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:APPROJPROCESSSTATEKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_PROJECT_PROJPROCESSSTATE as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NEXTSTATEFRMLA), '0'), 38, 10) is null and 
                    src:NEXTSTATEFRMLA is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:NEXTSTATEKEY), '0'), 38, 10) is null and 
                    src:NEXTSTATEKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:STATDATETYPE), '0'), 38, 10) is null and 
                    src:STATDATETYPE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:VARIATION_ID), '0'), 38, 10) is null and 
