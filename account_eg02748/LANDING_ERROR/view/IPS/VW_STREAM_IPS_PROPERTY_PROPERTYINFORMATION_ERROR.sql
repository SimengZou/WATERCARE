CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_PROPERTY_PROPERTYINFORMATION_ERROR AS SELECT src, 'IPS_PROPERTY_PROPERTYINFORMATION' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    src:ADDDTTM is not null) THEN
                    'ADDDTTM contains a non-timestamp value : \'' || AS_VARCHAR(src:ADDDTTM) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CENSUSTRACT), '0'), 38, 10) is null and 
                    src:CENSUSTRACT is not null) THEN
                    'CENSUSTRACT contains a non-numeric value : \'' || AS_VARCHAR(src:CENSUSTRACT) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HEALTHAREA), '0'), 38, 10) is null and 
                    src:HEALTHAREA is not null) THEN
                    'HEALTHAREA contains a non-numeric value : \'' || AS_VARCHAR(src:HEALTHAREA) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_PROPERTY_PROPERTYINFORMATION as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    src:ADDDTTM is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CENSUSTRACT), '0'), 38, 10) is null and 
                    src:CENSUSTRACT is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:EFFDATE), '1900-01-01')) is null and 
                    src:EFFDATE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:EXPDATE), '1900-01-01')) is null and 
                    src:EXPDATE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HEALTHAREA), '0'), 38, 10) is null and 
                    src:HEALTHAREA is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MODDTTM), '1900-01-01')) is null and 
