CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_WSLASSETSCONFIGURED_XELECTSTATIC_ERROR AS SELECT src, 'IPS_WSLASSETSCONFIGURED_XELECTSTATIC' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XELECTSTATICKEY), '0'), 38, 10) is null and 
                    src:XELECTSTATICKEY is not null) THEN
                    'XELECTSTATICKEY contains a non-numeric value : \'' || AS_VARCHAR(src:XELECTSTATICKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:XELECTSTATICKEY), '0'), 38, 10) is null and 
                    src:XELECTSTATICKEY is not null) or 
                (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:VARIATION_ID), '1900-01-01')) is null and 
