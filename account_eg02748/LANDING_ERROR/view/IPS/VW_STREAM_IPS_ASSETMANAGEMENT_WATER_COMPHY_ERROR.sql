CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPHY_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_WATER_COMPHY' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BARRELSIZE), '0'), 38, 10) is null and 
                    src:BARRELSIZE is not null) THEN
                    'BARRELSIZE contains a non-numeric value : \'' || AS_VARCHAR(src:BARRELSIZE) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FEEDERDIAM), '0'), 38, 10) is null and 
                    src:FEEDERDIAM is not null) THEN
                    'FEEDERDIAM contains a non-numeric value : \'' || AS_VARCHAR(src:FEEDERDIAM) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FEEDERLEN), '0'), 38, 10) is null and 
                    src:FEEDERLEN is not null) THEN
                    'FEEDERLEN contains a non-numeric value : \'' || AS_VARCHAR(src:FEEDERLEN) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ1), '0'), 38, 10) is null and 
                    src:OUTLSZ1 is not null) THEN
                    'OUTLSZ1 contains a non-numeric value : \'' || AS_VARCHAR(src:OUTLSZ1) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ2), '0'), 38, 10) is null and 
                    src:OUTLSZ2 is not null) THEN
                    'OUTLSZ2 contains a non-numeric value : \'' || AS_VARCHAR(src:OUTLSZ2) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ3), '0'), 38, 10) is null and 
                    src:OUTLSZ3 is not null) THEN
                    'OUTLSZ3 contains a non-numeric value : \'' || AS_VARCHAR(src:OUTLSZ3) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ4), '0'), 38, 10) is null and 
                    src:OUTLSZ4 is not null) THEN
                    'OUTLSZ4 contains a non-numeric value : \'' || AS_VARCHAR(src:OUTLSZ4) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:WVKEY), '0'), 38, 10) is null and 
                    src:WVKEY is not null) THEN
                    'WVKEY contains a non-numeric value : \'' || AS_VARCHAR(src:WVKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_WATER_COMPHY as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FEEDERDIAM), '0'), 38, 10) is null and 
                    src:FEEDERDIAM is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FEEDERLEN), '0'), 38, 10) is null and 
                    src:FEEDERLEN is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:GISSTATIC), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ1), '0'), 38, 10) is null and 
                    src:OUTLSZ1 is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ2), '0'), 38, 10) is null and 
                    src:OUTLSZ2 is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ3), '0'), 38, 10) is null and 
                    src:OUTLSZ3 is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OUTLSZ4), '0'), 38, 10) is null and 
                    src:OUTLSZ4 is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:POSITION), '0'), 38, 10) is null and 
