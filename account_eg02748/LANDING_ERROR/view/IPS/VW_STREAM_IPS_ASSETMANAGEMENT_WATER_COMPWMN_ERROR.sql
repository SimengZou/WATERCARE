CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWMN_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_WATER_COMPWMN' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CORRFACTOR), '0'), 38, 10) is null and 
                    src:CORRFACTOR is not null) THEN
                    'CORRFACTOR contains a non-numeric value : \'' || AS_VARCHAR(src:CORRFACTOR) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:FROSTDPTH), '0'), 38, 10) is null and 
                    src:FROSTDPTH is not null) THEN
                    'FROSTDPTH contains a non-numeric value : \'' || AS_VARCHAR(src:FROSTDPTH) || '\'' WHEN 
