CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWV_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_WATER_COMPWV' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HIGHPRES), '0'), 38, 10) is null and 
                    src:HIGHPRES is not null) THEN
                    'HIGHPRES contains a non-numeric value : \'' || AS_VARCHAR(src:HIGHPRES) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWPRES), '0'), 38, 10) is null and 
                    src:LOWPRES is not null) THEN
                    'LOWPRES contains a non-numeric value : \'' || AS_VARCHAR(src:LOWPRES) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:VALVESZ), '0'), 38, 10) is null and 
                    src:VALVESZ is not null) THEN
                    'VALVESZ contains a non-numeric value : \'' || AS_VARCHAR(src:VALVESZ) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWV as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:HIGHPRES), '0'), 38, 10) is null and 
                    src:HIGHPRES is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:INSTDATE), '1900-01-01')) is null and 
                    src:INSTDATE is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:INTKEY), '0'), 38, 10) is null and 
                    src:INTKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:LOWPRES), '0'), 38, 10) is null and 
                    src:LOWPRES is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:MAINKEY), '0'), 38, 10) is null and 
