CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_BUILDING_BLDGAPPL_ERROR AS SELECT src, 'IPS_CDR_BUILDING_BLDGAPPL' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APBLDGBATCHKEY), '0'), 38, 10) is null and 
                    src:APBLDGBATCHKEY is not null) THEN
                    'APBLDGBATCHKEY contains a non-numeric value : \'' || AS_VARCHAR(src:APBLDGBATCHKEY) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APMODELPROJKEY), '0'), 38, 10) is null and 
                    src:APMODELPROJKEY is not null) THEN
                    'APMODELPROJKEY contains a non-numeric value : \'' || AS_VARCHAR(src:APMODELPROJKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_BUILDING_BLDGAPPL as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ACTLVLTN), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APBLDGBATCHKEY), '0'), 38, 10) is null and 
                    src:APBLDGBATCHKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APBLDGDEFNKEY), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:APMODELPROJKEY), '0'), 38, 10) is null and 
                    src:APMODELPROJKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:BLDGAREA), '0'), 38, 10) is null and 
