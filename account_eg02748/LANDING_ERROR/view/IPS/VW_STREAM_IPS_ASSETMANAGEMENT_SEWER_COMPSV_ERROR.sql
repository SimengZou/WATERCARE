CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSV_ERROR AS SELECT src, 'IPS_ASSETMANAGEMENT_SEWER_COMPSV' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:INVERTELEV), '0'), 38, 10) is null and 
                    src:INVERTELEV is not null) THEN
                    'INVERTELEV contains a non-numeric value : \'' || AS_VARCHAR(src:INVERTELEV) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:OPERDPTH), '0'), 38, 10) is null and 
                    src:OPERDPTH is not null) THEN
                    'OPERDPTH contains a non-numeric value : \'' || AS_VARCHAR(src:OPERDPTH) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:RIMELEV), '0'), 38, 10) is null and 
                    src:RIMELEV is not null) THEN
                    'RIMELEV contains a non-numeric value : \'' || AS_VARCHAR(src:RIMELEV) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:VALVESIZE), '0'), 38, 10) is null and 
                    src:VALVESIZE is not null) THEN
                    'VALVESIZE contains a non-numeric value : \'' || AS_VARCHAR(src:VALVESIZE) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSV as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
