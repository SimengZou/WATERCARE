CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_BILLING_SENDCOPIESTO_ERROR AS SELECT src, 'IPS_BILLING_SENDCOPIESTO' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SENDCOPIESQUANTITY), '0'), 38, 10) is null and 
                    src:SENDCOPIESQUANTITY is not null) THEN
                    'SENDCOPIESQUANTITY contains a non-numeric value : \'' || AS_VARCHAR(src:SENDCOPIESQUANTITY) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SENDCOPIESTOKEY), '0'), 38, 10) is null and 
                    src:SENDCOPIESTOKEY is not null) THEN
                    'SENDCOPIESTOKEY contains a non-numeric value : \'' || AS_VARCHAR(src:SENDCOPIESTOKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SENDCOPIESQUANTITY), '0'), 38, 10) is null and 
                    src:SENDCOPIESQUANTITY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SENDCOPIESTOKEY), '0'), 38, 10) is null and 
                    src:SENDCOPIESTOKEY is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:TODATE), '1900-01-01')) is null and 
