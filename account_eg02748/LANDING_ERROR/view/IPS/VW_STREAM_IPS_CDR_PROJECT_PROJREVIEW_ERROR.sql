CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PROJECT_PROJREVIEW_ERROR AS SELECT src, 'IPS_CDR_PROJECT_PROJREVIEW' as TABLE_OBJECT, CASE WHEN 
            ...
            ROW_NUMBER() OVER (PARTITION BY 
                ...
                src:APPROJREVIEWKEY  ORDER BY 
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_PROJECT_PROJREVIEW as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ACTLTM), '0'), 38, 10) is null and 
