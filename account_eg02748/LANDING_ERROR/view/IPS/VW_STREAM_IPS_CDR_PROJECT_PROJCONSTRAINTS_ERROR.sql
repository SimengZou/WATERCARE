CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CDR_PROJECT_PROJCONSTRAINTS_ERROR AS SELECT src, 'IPS_CDR_PROJECT_PROJCONSTRAINTS' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROJCONSTRAINTSKEY), '0'), 38, 10) is null and 
                    src:PROJCONSTRAINTSKEY is not null) THEN
                    'PROJCONSTRAINTSKEY contains a non-numeric value : \'' || AS_VARCHAR(src:PROJCONSTRAINTSKEY) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:PROJCONSTRAINTSKEY), '0'), 38, 10) is null and 
                    src:PROJCONSTRAINTSKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:SETTING), '0'), 38, 10) is null and 
