CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_IPS_CASHIERING_CHARGE_ERROR AS SELECT src, 'IPS_CASHIERING_CHARGE' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:ADDDTTM), '1900-01-01')) is null and 
                    src:ADDDTTM is not null) THEN
                    'ADDDTTM contains a non-timestamp value : \'' || AS_VARCHAR(src:ADDDTTM) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CHARGEKEY), '0'), 38, 10) is null and 
                    src:CHARGEKEY is not null) THEN
                    'CHARGEKEY contains a non-numeric value : \'' || AS_VARCHAR(src:CHARGEKEY) || '\'' WHEN 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CHGDTTM), '1900-01-01')) is null and 
                    src:CHGDTTM is not null) THEN
                    'CHGDTTM contains a non-timestamp value : \'' || AS_VARCHAR(src:CHGDTTM) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CHGQTY), '0'), 38, 10) is null and 
                    src:CHGQTY is not null) THEN
                    'CHGQTY contains a non-numeric value : \'' || AS_VARCHAR(src:CHGQTY) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CONTID), '0'), 38, 10) is null and 
                    src:CONTID is not null) THEN
                    'CONTID contains a non-numeric value : \'' || AS_VARCHAR(src:CONTID) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:INTERNALCHARGE), '0'), 38, 10) is null and 
                    src:INTERNALCHARGE is not null) THEN
                    'INTERNALCHARGE contains a non-numeric value : \'' || AS_VARCHAR(src:INTERNALCHARGE) || '\'' WHEN 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CHARGEKEY), '0'), 38, 10) is null and 
                    src:CHARGEKEY is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:CHGDTTM), '1900-01-01')) is null and 
                    src:CHGDTTM is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CHGQTY), '0'), 38, 10) is null and 
                    src:CHGQTY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CHGSUBTOT), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:CONTID), '0'), 38, 10) is null and 
                    src:CONTID is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:EXTCHGKEY), '0'), 38, 10) is null and 
                    src:EXTCHGKEY is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:INTERNALCHARGE), '0'), 38, 10) is null and 
                    src:INTERNALCHARGE is not null) or 
                    (TRY_TO_TIMESTAMP(NVL(AS_VARCHAR(src:MODDTTM), '1900-01-01')) is null and 
