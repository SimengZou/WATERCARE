CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TFFAM830_ERROR AS SELECT src, 'LN_TFFAM830' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:abdf), '0'), 38, 10) is null and 
                    src:abdf is not null) THEN
                    'abdf contains a non-numeric value : \'' || AS_VARCHAR(src:abdf) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dtty), '0'), 38, 10) is null and 
                    src:dtty is not null) THEN
                    'dtty contains a non-numeric value : \'' || AS_VARCHAR(src:dtty) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ltdu), '0'), 38, 10) is null and 
                    src:ltdu is not null) THEN
                    'ltdu contains a non-numeric value : \'' || AS_VARCHAR(src:ltdu) || '\'' WHEN 
