CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPDM635_ERROR AS SELECT src, 'LN_TPPDM635' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfpp), '0'), 38, 10) is null and 
                    src:dfpp is not null) THEN
                    'dfpp contains a non-numeric value : \'' || AS_VARCHAR(src:dfpp) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfps), '0'), 38, 10) is null and 
                    src:dfps is not null) THEN
                    'dfps contains a non-numeric value : \'' || AS_VARCHAR(src:dfps) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfsb_ref_compnr), '0'), 38, 10) is null and 
                    src:dfsb_ref_compnr is not null) THEN
                    'dfsb_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:dfsb_ref_compnr) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPDM635 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:blbl), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfpp), '0'), 38, 10) is null and 
                    src:dfpp is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfps), '0'), 38, 10) is null and 
                    src:dfps is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfsb_ref_compnr), '0'), 38, 10) is null and 
                    src:dfsb_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dsbc), '0'), 38, 10) is null and 
