CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_WHINA112_ERROR AS SELECT src, 'LN_WHINA112' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:qskt), '0'), 38, 10) is null and 
                    src:qskt is not null) THEN
                    'qskt contains a non-numeric value : \'' || AS_VARCHAR(src:qskt) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:vaco), '0'), 38, 10) is null and 
                    src:vaco is not null) THEN
                    'vaco contains a non-numeric value : \'' || AS_VARCHAR(src:vaco) || '\'' WHEN 
