CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPDM040_ERROR AS SELECT src, 'LN_TPPDM040' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:acta), '0'), 38, 10) is null and 
                    src:acta is not null) THEN
                    'acta contains a non-numeric value : \'' || AS_VARCHAR(src:acta) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPDM040 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:acta), '0'), 38, 10) is null and 
                    src:acta is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:actp), '0'), 38, 10) is null and 
