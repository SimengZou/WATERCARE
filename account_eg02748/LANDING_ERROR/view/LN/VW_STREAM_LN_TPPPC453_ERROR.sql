CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPPC453_ERROR AS SELECT src, 'LN_TPPPC453' as TABLE_OBJECT, CASE WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPPC453 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:amac_1), '0'), 38, 10) is null and 
