CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPDM625_ERROR AS SELECT src, 'LN_TPPDM625' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfeq_ref_compnr), '0'), 38, 10) is null and 
                    src:dfeq_ref_compnr is not null) THEN
                    'dfeq_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:dfeq_ref_compnr) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrp), '0'), 38, 10) is null and 
                    src:dfrp is not null) THEN
                    'dfrp contains a non-numeric value : \'' || AS_VARCHAR(src:dfrp) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPDM625 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:avtp_ref_compnr), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfeq_ref_compnr), '0'), 38, 10) is null and 
                    src:dfeq_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrc), '0'), 38, 10) is null and 
                    src:dfrc is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrp), '0'), 38, 10) is null and 
                    src:dfrp is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:dfrs), '0'), 38, 10) is null and 
