CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TPPTC101_ERROR AS SELECT src, 'LN_TPPTC101' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:cprj_cspc_ref_compnr), '0'), 38, 10) is null and 
                    src:cprj_cspc_ref_compnr is not null) THEN
                    'cprj_cspc_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:cprj_cspc_ref_compnr) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:cprj_cspp_ref_compnr), '0'), 38, 10) is null and 
                    src:cprj_cspp_ref_compnr is not null) THEN
                    'cprj_cspp_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:cprj_cspp_ref_compnr) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:qant), '0'), 38, 10) is null and 
                    src:qant is not null) THEN
                    'qant contains a non-numeric value : \'' || AS_VARCHAR(src:qant) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_LN_TPPTC101 as strm)
                WHERE
                ROWNUMBER=1) where 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:compnr), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:cprj_cspc_ref_compnr), '0'), 38, 10) is null and 
                    src:cprj_cspc_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:cprj_cspp_ref_compnr), '0'), 38, 10) is null and 
                    src:cprj_cspp_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:cprj_ref_compnr), '0'), 38, 10) is null and 
                    src:cprj_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:qant), '0'), 38, 10) is null and 
                    src:qant is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:sequencenumber), '0'), 38, 10) is null and 
