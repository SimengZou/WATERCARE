CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TFACR001_ERROR AS SELECT src, 'LN_TFACR001' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:defs_ref_compnr), '0'), 38, 10) is null and 
                    src:defs_ref_compnr is not null) THEN
                    'defs_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:defs_ref_compnr) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ircd_ref_compnr), '0'), 38, 10) is null and 
                    src:ircd_ref_compnr is not null) THEN
                    'ircd_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:ircd_ref_compnr) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:defs_ref_compnr), '0'), 38, 10) is null and 
                    src:defs_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:ircd_ref_compnr), '0'), 38, 10) is null and 
                    src:ircd_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:sequencenumber), '0'), 38, 10) is null and 
