CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TIROU003_ERROR AS SELECT src, 'LN_TIROU003' as TABLE_OBJECT, CASE WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:mlmc), '0'), 38, 10) is null and 
                    src:mlmc is not null) THEN
                    'mlmc contains a non-numeric value : \'' || AS_VARCHAR(src:mlmc) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:mlwc), '0'), 38, 10) is null and 
                    src:mlwc is not null) THEN
                    'mlwc contains a non-numeric value : \'' || AS_VARCHAR(src:mlwc) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:mlmc), '0'), 38, 10) is null and 
                    src:mlmc is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:mlwc), '0'), 38, 10) is null and 
                    src:mlwc is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:sequencenumber), '0'), 38, 10) is null and 
