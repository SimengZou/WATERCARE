CREATE OR REPLACE VIEW LANDING_ERROR.VW_STREAM_LN_TSCTM135_ERROR AS SELECT src, 'LN_TSCTM135' as TABLE_OBJECT, CASE WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:alcl), '0'), 38, 10) is null and 
                    src:alcl is not null) THEN
                    'alcl contains a non-numeric value : \'' || AS_VARCHAR(src:alcl) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:alun), '0'), 38, 10) is null and 
                    src:alun is not null) THEN
                    'alun contains a non-numeric value : \'' || AS_VARCHAR(src:alun) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:bsfn), '0'), 38, 10) is null and 
                    src:bsfn is not null) THEN
                    'bsfn contains a non-numeric value : \'' || AS_VARCHAR(src:bsfn) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:bsls), '0'), 38, 10) is null and 
                    src:bsls is not null) THEN
                    'bsls contains a non-numeric value : \'' || AS_VARCHAR(src:bsls) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eacl), '0'), 38, 10) is null and 
                    src:eacl is not null) THEN
                    'eacl contains a non-numeric value : \'' || AS_VARCHAR(src:eacl) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eaun), '0'), 38, 10) is null and 
                    src:eaun is not null) THEN
                    'eaun contains a non-numeric value : \'' || AS_VARCHAR(src:eaun) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:nrcl), '0'), 38, 10) is null and 
                    src:nrcl is not null) THEN
                    'nrcl contains a non-numeric value : \'' || AS_VARCHAR(src:nrcl) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:nrun), '0'), 38, 10) is null and 
                    src:nrun is not null) THEN
                    'nrun contains a non-numeric value : \'' || AS_VARCHAR(src:nrun) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rctt), '0'), 38, 10) is null and 
                    src:rctt is not null) THEN
                    'rctt contains a non-numeric value : \'' || AS_VARCHAR(src:rctt) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rsin), '0'), 38, 10) is null and 
                    src:rsin is not null) THEN
                    'rsin contains a non-numeric value : \'' || AS_VARCHAR(src:rsin) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rstu_ref_compnr), '0'), 38, 10) is null and 
                    src:rstu_ref_compnr is not null) THEN
                    'rstu_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:rstu_ref_compnr) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:sequencenumber), '0'), 38, 10) is null and 
                    src:sequencenumber is not null) THEN
                    'sequencenumber contains a non-numeric value : \'' || AS_VARCHAR(src:sequencenumber) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:slfn), '0'), 38, 10) is null and 
                    src:slfn is not null) THEN
                    'slfn contains a non-numeric value : \'' || AS_VARCHAR(src:slfn) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:slst), '0'), 38, 10) is null and 
                    src:slst is not null) THEN
                    'slst contains a non-numeric value : \'' || AS_VARCHAR(src:slst) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spcl), '0'), 38, 10) is null and 
                    src:spcl is not null) THEN
                    'spcl contains a non-numeric value : \'' || AS_VARCHAR(src:spcl) || '\'' WHEN 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spun), '0'), 38, 10) is null and 
                    src:spun is not null) THEN
                    'spun contains a non-numeric value : \'' || AS_VARCHAR(src:spun) || '\'' WHEN 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:srcl_ref_compnr), '0'), 38, 10) is null and 
                    src:srcl_ref_compnr is not null) THEN
                    'srcl_ref_compnr contains a non-numeric value : \'' || AS_VARCHAR(src:srcl_ref_compnr) || '\'' WHEN 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:bsfn), '0'), 38, 10) is null and 
                    src:bsfn is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:bsls), '0'), 38, 10) is null and 
                    src:bsls is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:camt_1), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:eaun), '0'), 38, 10) is null and 
                    src:eaun is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:nrbt), '0'), 38, 10) is null and 
                    src:nrbt is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:nrcl), '0'), 38, 10) is null and 
                    src:nrcl is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:nrun), '0'), 38, 10) is null and 
                    src:nrun is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rctt), '0'), 38, 10) is null and 
                    src:rctt is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rsin), '0'), 38, 10) is null and 
                    src:rsin is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:rstu_ref_compnr), '0'), 38, 10) is null and 
                    src:rstu_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:sequencenumber), '0'), 38, 10) is null and 
                    src:sequencenumber is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:slfn), '0'), 38, 10) is null and 
                    src:slfn is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:slst), '0'), 38, 10) is null and 
                    src:slst is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spcl), '0'), 38, 10) is null and 
                    src:spcl is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spco_1), '0'), 38, 10) is null and 
                    ...
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:spun), '0'), 38, 10) is null and 
                    src:spun is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:srcl_ref_compnr), '0'), 38, 10) is null and 
                    src:srcl_ref_compnr is not null) or 
                    (TRY_TO_NUMERIC(NVL(AS_VARCHAR(src:term), '0'), 38, 10) is null and 
