CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_IPS_CDR_AP AS SELECT
                        src:ADDBY::varchar AS ADDBY, 
                        src:ADDDTTM::datetime AS ADDDTTM, 
                        src:APKEY::integer AS APKEY, 
                        src:BILLACCTAPPLBASEDFLAG::varchar AS BILLACCTAPPLBASEDFLAG, 
                        src:BILLACCTKEY::integer AS BILLACCTKEY, 
                        src:CDRKEY::integer AS CDRKEY, 
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_IPS_CDR_AP as strm)
                WHERE
                ROWNUMBER=1) where 
                    ...
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:CDRKEY), '0'), 38, 10) is not null and 
