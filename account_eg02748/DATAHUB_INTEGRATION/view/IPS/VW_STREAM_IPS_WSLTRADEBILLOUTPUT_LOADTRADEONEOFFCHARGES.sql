CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLTRADEBILLOUTPUT_LOADTRADEONEOFFCHARGES AS SELECT
            ...
            src:VARIATION_ID desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                ROWNUMBER=1) where 
                    ...
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:LOADTRADEONEOFFCHARGESKEY), '0'), 38, 10) is not null and 
