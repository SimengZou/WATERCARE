CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_DEPM_DIM_AMP_VERSION AS SELECT
                        ...
                        src:StartYear::varchar AS StartYear, 
                        src:Timestamp::datetime AS Timestamp, 
                        src:WorkflowEnabled::varchar AS WorkflowEnabled, 
            ...
            src:Timestamp desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_DEPM_DIM_AMP_VERSION as strm)
                WHERE
                ROWNUMBER=1) where 
