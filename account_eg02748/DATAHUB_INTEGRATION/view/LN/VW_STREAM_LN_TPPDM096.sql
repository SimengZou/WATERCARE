CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_LN_TPPDM096 AS SELECT
                        src:cobs::varchar AS cobs, 
                        src:cobs_coep_ref_compnr::integer AS cobs_coep_ref_compnr, 
                        src:cobs_ref_compnr::integer AS cobs_ref_compnr, 
                        src:coel::varchar AS coel, 
                        src:coep::varchar AS coep, 
            ...
            src:sequencenumber desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                ...
                WHERE
                ROWNUMBER=1) where 
                    TRY_TO_NUMERIC(nvl(AS_VARCHAR(src:cobs_coep_ref_compnr), '0'), 38, 10) is not null and 
