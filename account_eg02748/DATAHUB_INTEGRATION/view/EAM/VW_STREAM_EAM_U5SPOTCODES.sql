CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_EAM_U5SPOTCODES AS SELECT
                        ...
                        src:SPC_CODE::varchar AS SPC_CODE, 
                        src:SPC_DESC::varchar AS SPC_DESC, 
                        src:SPC_NOTUSED::varchar AS SPC_NOTUSED, 
                        src:SPC_TYPE::varchar AS SPC_TYPE, 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_EAM_U5SPOTCODES as strm)
                WHERE
                ROWNUMBER=1) where 
