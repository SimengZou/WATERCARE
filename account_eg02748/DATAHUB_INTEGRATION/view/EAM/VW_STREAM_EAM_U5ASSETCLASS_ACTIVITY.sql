CREATE OR REPLACE VIEW DATAHUB_INTEGRATION.VW_STREAM_EAM_U5ASSETCLASS_ACTIVITY AS SELECT
                        src:ACAA_ASSETCLASS::varchar AS ACAA_ASSETCLASS, 
                        src:ACAA_ASSETCLASS_DESC::varchar AS ACAA_ASSETCLASS_DESC, 
                        src:ACAA_NOTUSED::varchar AS ACAA_NOTUSED, 
                        src:ACAA_TASKTYPE::varchar AS ACAA_TASKTYPE, 
                        src:ACAA_TASKTYPE_DESC::varchar AS ACAA_TASKTYPE_DESC, 
            ...
            src:LASTSAVED desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
                FROM DATAHUB_INTEGRATION.STREAM_EAM_U5ASSETCLASS_ACTIVITY as strm)
                WHERE
                ROWNUMBER=1) where 
