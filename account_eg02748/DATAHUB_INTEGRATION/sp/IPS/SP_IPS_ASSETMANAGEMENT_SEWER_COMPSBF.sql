create or replace procedure DATAHUB_INTEGRATION.SP_IPS_ASSETMANAGEMENT_SEWER_COMPSBF()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSBF_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_ASSETMANAGEMENT_SEWER_COMPSBF as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
            strm.ADDRQUAL, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_ASSETMANAGEMENT_SEWER_COMPSBF as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.COMPKEY=src.COMPKEY) OR (target.COMPKEY IS NULL AND src.COMPKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.BFLOWSZ=src.BFLOWSZ, 
                    target.BFLOWSZUOM=src.BFLOWSZUOM, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_ASSETMANAGEMENT_SEWER_COMPSBF as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ADDRKEY, ADDRQUAL, AREA, ASBLT, BFLOWSZ, BFLOWSZUOM, BGTNO, BLDGFLOOR, BLDGROOM, BUDGETKEY, COMPKEY, COMPLEXKEY, DATALAKE_DELETED, DISTRICT, EXPBY, EXPDATE, GISSTATIC, INSTDATE, LOC, MAINKEY, MAPNO, MFGKEY, MODBY, MODDTTM, MODELNO, OWN, POSITION, PRCLKEY, RESP, SEGKEY, SERNO, SERVSTAT, SITEKEY, SLKEY, SUBAREA, UNITDESC, UNITID, UNITTYPE, USGAREAKEY, VARIATION_ID, XCOORD, YCOORD, ZCOORD, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
