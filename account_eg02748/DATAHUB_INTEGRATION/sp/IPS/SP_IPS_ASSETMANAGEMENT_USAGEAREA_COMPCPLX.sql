create or replace procedure DATAHUB_INTEGRATION.SP_IPS_ASSETMANAGEMENT_USAGEAREA_COMPCPLX()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPCPLX_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_ASSETMANAGEMENT_USAGEAREA_COMPCPLX as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
            strm.ADDRQUAL, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPCPLX as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_ASSETMANAGEMENT_USAGEAREA_COMPCPLX as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ADDRKEY, ADDRQUAL, AREA, AREASZ, AREAUM, ASBLT, BGTNO, BLDGFLOOR, BLDGROOM, BUDGETKEY, COMPKEY, COMPLEXKEY, DATALAKE_DELETED, DISTRICT, EXPBY, EXPDATE, GISSTATIC, INSTDATE, LOC, MAPNO, MODBY, MODDTTM, OWN, POSITION, PRCLKEY, SEGKEY, SERVSTAT, SITEKEY, SUBAREA, UNITDESC, UNITID, UNITTYPE, USGAREAKEY, VARIATION_ID, XCOORD, YCOORD, ZCOORD, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
