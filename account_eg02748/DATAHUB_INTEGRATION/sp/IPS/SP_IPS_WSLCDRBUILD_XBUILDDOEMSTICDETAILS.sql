create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLCDRBUILD_XBUILDDOEMSTICDETAILS()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLCDRBUILD_XBUILDDOEMSTICDETAILS_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLCDRBUILD_XBUILDDOEMSTICDETAILS as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLCDRBUILD_XBUILDDOEMSTICDETAILS as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLCDRBUILD_XBUILDDOEMSTICDETAILS as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDITIONALDETAILS, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ADDITIONALDETAILS, APBLDGAPPLDTLKEY, BILLADDRESS, BILLMETHOD, BILLOWNERORAPPLICANT, BUILDCONSENTNO, CT, DELETED, DEVCONTPAID, DP, FLOORAREALT65, INSTALLIN10DAYS, LOT, MODBY, MODDTTM, SITECONNECTEDTOWASTEWATER, SITECONNECTEDTOWATER, VARIATION_ID, XBUILDDOEMSTICDETAILSKEY, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
