create or replace procedure DATAHUB_INTEGRATION.SP_IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
            strm.ADDRQUAL, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_ASSETMANAGEMENT_USAGEAREA_COMPAREA as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.COMPKEY=src.COMPKEY) OR (target.COMPKEY IS NULL AND src.COMPKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.AREADESC=src.AREADESC, 
                    target.AREALEN=src.AREALEN, 
                    target.AREASZ=src.AREASZ, 
                    target.AREAUM=src.AREAUM, 
                    target.AREAWID=src.AREAWID, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_ASSETMANAGEMENT_USAGEAREA_COMPAREA as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ADDRKEY, ADDRQUAL, AREA, AREADESC, AREALEN, AREASZ, AREAUM, AREAWID, ASBLT, BGTNO, BLDGFLOOR, BLDGROOM, BUDGETKEY, COMPKEY, COMPLEXKEY, DATALAKE_DELETED, DISTRICT, EXPBY, EXPDATE, GISSTATIC, INSTDATE, INTKEY, LOC, MAPNO, MODBY, MODDTTM, OWN, POSITION, PRCLKEY, SEGKEY, SERVSTAT, SITEKEY, SUBAREA, SURF, UNITDESC, UNITID, UNITTYPE, USGAREAKEY, VARIATION_ID, WARREXP, XCOORD, YCOORD, ZCOORD, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
