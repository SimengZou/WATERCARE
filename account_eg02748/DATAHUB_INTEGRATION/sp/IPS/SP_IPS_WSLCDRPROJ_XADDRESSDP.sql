create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLCDRPROJ_XADDRESSDP()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLCDRPROJ_XADDRESSDP_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLCDRPROJ_XADDRESSDP as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLCDRPROJ_XADDRESSDP as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLCDRPROJ_XADDRESSDP as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.APPROJAPPLDTLKEY, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, APPROJAPPLDTLKEY, CITY, DATALAKE_DELETED, FLAT, HOUSENUMBER, MODBY, MODDTTM, POSTALCODE, STREETNAME, STREETTYPE, SUBURB, VARIATION_ID, XADDRESSDPKEY, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
