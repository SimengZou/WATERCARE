create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSOTHER_XOFFICEEQUIPMENT()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSOTHER_XOFFICEEQUIPMENT_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSOTHER_XOFFICEEQUIPMENT as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XOFFICEEQUIPMENTKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSOTHER_XOFFICEEQUIPMENT as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XOFFICEEQUIPMENTKEY=src.XOFFICEEQUIPMENTKEY) OR (target.XOFFICEEQUIPMENTKEY IS NULL AND src.XOFFICEEQUIPMENTKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XOFFICEEQUIPMENTKEY=src.XOFFICEEQUIPMENTKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSOTHER_XOFFICEEQUIPMENT as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
        ...
        when not matched   then insert ( ACCELDEPREC, ACCELDEPRECDESC, ACQTYPE, ADDBY, ADDDTTM, BUSINESSAREA, COMPKEY, CONFINEDSPACE, COSTCENTRE, CRITICALITY, DATALAKE_DELETED, DISPOSALDESC, DRAWINGNO, DRAWINGNOREF, FORDISPOSAL, LINKEDASSET, MAKE, MODBY, MODDTTM, MODELNO, PROJECTNO, PROJECTWISE, RELDATE, RELDEPTH, RELDIA, RELGEOSP, RELMATL, RELOWN, RELSTATUS, SAFETYCRITICAL, VARIATION_ID, WARRANTYEND, WARRANTYSTART, XOFFICEEQUIPMENTKEY, ZONECATCHMENT, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
