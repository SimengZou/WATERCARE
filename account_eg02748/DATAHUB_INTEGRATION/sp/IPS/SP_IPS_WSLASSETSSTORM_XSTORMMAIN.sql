create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSSTORM_XSTORMMAIN()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSSTORM_XSTORMMAIN_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSSTORM_XSTORMMAIN as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XSTORMMAINKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSSTORM_XSTORMMAIN as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XSTORMMAINKEY=src.XSTORMMAINKEY) OR (target.XSTORMMAINKEY IS NULL AND src.XSTORMMAINKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XSTORMMAINKEY=src.XSTORMMAINKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSSTORM_XSTORMMAIN as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
