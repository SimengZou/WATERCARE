create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSWATER_XWATERSERVICELINE()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSWATER_XWATERSERVICELINE_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSWATER_XWATERSERVICELINE as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XWATERSERVICELINEKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSWATER_XWATERSERVICELINE as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XWATERSERVICELINEKEY=src.XWATERSERVICELINEKEY) OR (target.XWATERSERVICELINEKEY IS NULL AND src.XWATERSERVICELINEKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XWATERSERVICELINEKEY=src.XWATERSERVICELINEKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSWATER_XWATERSERVICELINE as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
        ...
        when not matched   then insert ( ACCELDEPREC, ACCELDEPRECDESC, ACQTYPE, ADDBY, ADDDTTM, BUSINESSAREA, COMPKEY, CONFINEDSPACE, CONSTMETHOD, COSTCENTRE, CRITICALITY, DATALAKE_DELETED, DESIGNRESILIENCE, DIAMEXT, DIAMINT, DISPOSALDESC, DRAWINGNO, DRAWINGNOREF, EXTCOAT, FORDISPOSAL, FUNCAREA, GRDLEVEL, INTLINING, INVLEVEL, LINKEDASSET, LOCALITY, MAKE, MEDIATYPE, MODBY, MODDTTM, MODELNO, PHOTO3D, PRESSURERATING, PROJECTNO, PROJECTWISE, QUAKEDESIGN, RELDATE, RELDEPTH, RELDIA, RELGEOSP, RELMATL, RELOWN, RELSTATUS, SAFETYCRITICAL, STIFFRATING, VARIATION_ID, WARRANTYEND, WARRANTYSTART, XWATERSERVICELINEKEY, ZONECATCHMENT, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
