create or replace procedure DATAHUB_INTEGRATION.SP_IPS_ASSETMANAGEMENT_WATER_COMPWMR()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWMR_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_ASSETMANAGEMENT_WATER_COMPWMR as target using (SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
            strm.ADDRQUAL, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_ASSETMANAGEMENT_WATER_COMPWMR as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_ASSETMANAGEMENT_WATER_COMPWMR as target using (
                SELECT * FROM (SELECT 
            strm.ADDBY, 
            strm.ADDDTTM, 
            strm.ADDRKEY, 
        ...
        when not matched   then insert ( ADDBY, ADDDTTM, ADDRKEY, ADDRQUAL, AREA, ASBLT, BGTNO, BUDGETKEY, COMPKEY, DATALAKE_DELETED, DISTRICT, EXPBY, EXPDATE, GISSTATIC, INSTDATE, LOC, MAPNO, MFGDATE, MFGKEY, MODBY, MODDTTM, MODELNO, NODIALS, OWN, POSITION, PRCLKEY, PURCDATE, SERNO, SERVSTAT, SUBAREA, UNITDESC, UNITID, UNITTYPE, VARIATION_ID, XCOORD, YCOORD, ZCOORD, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
