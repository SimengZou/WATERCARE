create or replace procedure DATAHUB_INTEGRATION.SP_IPS_CDR_BUILDING_BLDGINSP()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_CDR_BUILDING_BLDGINSP_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_CDR_BUILDING_BLDGINSP as target using (SELECT * FROM (SELECT 
            strm.ACTVD, 
            strm.ACTVDBY, 
            strm.ACTVDDTTM, 
            strm.ADDBY, 
            ...
            strm.APBLDGINSPKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_CDR_BUILDING_BLDGINSP as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.APBLDGINSPKEY=src.APBLDGINSPKEY) OR (target.APBLDGINSPKEY IS NULL AND src.APBLDGINSPKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.APBLDGINSPTYPEKEY=src.APBLDGINSPTYPEKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_CDR_BUILDING_BLDGINSP as target using (
                SELECT * FROM (SELECT 
            strm.ACTVD, 
            strm.ACTVDBY, 
            strm.ACTVDDTTM, 
        ...
        when not matched   then insert ( ACTVD, ACTVDBY, ACTVDDTTM, ADDBY, ADDDTTM, APBLDGINSPKEY, APBLDGINSPTYPEKEY, APBLDGKEY, ASSIGNED, ASSIGNTO, ASSIGNTOPROVIDER, CALLBY, CALLDTTM, CMPTRGEN, CNTCTINFO, CNTCTPERSON, COMPDTTM, DATALAKE_DELETED, INSPBY, INSPBYPROVIDER, INSPGEN, INSPHOURS, INSTRIPKEY, ISPARTIAL, LOC, MOBILE, MOBILEDTTM, MOBILEEMP, MODBY, MODDTTM, ODOMSTART, ODOMSTOP, ORD, PREFERENCE, REFNO, RELEVANTINSP, REQUESTEDBY, RESULT, RESULTBY, RESULTBYPROVIDER, RESULTDTTM, SCHED, SCHEDBY, SCHEDDTTM, SPECIALINSTR, STARTDTTM, STAT, TYPENO, VARIATION_ID, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
