create or replace procedure DATAHUB_INTEGRATION.SP_IPS_WSLASSETSCONFIGURED_XCIVIL()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_IPS_WSLASSETSCONFIGURED_XCIVIL_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  DATAHUB_TARGET.IPS_WSLASSETSCONFIGURED_XCIVIL as target using (SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            ...
            strm.XCIVILKEY ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_IPS_WSLASSETSCONFIGURED_XCIVIL as  strm
                    )   
            WHERE ROWNUMBER=1  
            
                    ) as src on
            ((target.XCIVILKEY=src.XCIVILKEY) OR (target.XCIVILKEY IS NULL AND src.XCIVILKEY IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.XCIVILKEY=src.XCIVILKEY, 
    ...
    snowflake.execute( {sqlText: `merge into DATAHUB_TARGET_HISTORY.IPS_DELETED_WSLASSETSCONFIGURED_XCIVIL as target using (
                SELECT * FROM (SELECT 
            strm.ACCELDEPREC, 
            strm.ACCELDEPRECDESC, 
            strm.ACQTYPE, 
        ...
        when not matched   then insert ( ACCELDEPREC, ACCELDEPRECDESC, ACQTYPE, ADDBY, ADDDTTM, BUSINESSAREA, COMPKEY, CONFINEDSPACE, COSTCENTRE, CRITICALITY, DATALAKE_DELETED, DESIGNRESILIENCE, DIAMEXT, DIAMINT, DISPOSALDESC, DRAWINGNO, DRAWINGNOREF, EXTCOAT, FORDISPOSAL, GRDLEVEL, HAZARDRATING, INVLEVEL, LINKEDASSET, LOADRATING, LOCALITY, MAKE, MODBY, MODDTTM, MODELNO, PHOTO3D, PROJECTNO, PROJECTWISE, QUAKEDESIGN, RELDATE, RELDEPTH, RELDIA, RELGEOSP, RELMATL, RELOWN, RELSTATUS, SAFETYCRITICAL, SUBTYPEFEATURES, VARIATION_ID, WARRANTYEND, WARRANTYSTART, XCIVILKEY, ZONECATCHMENT, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
