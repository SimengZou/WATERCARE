create or replace procedure DATAHUB_INTEGRATION.SP_LN_TFGLD204()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TFGLD204_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TFGLD204 as target using (SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TFGLD204 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TFGLD204_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
            strm.ccur_ref_compnr, 
        ...
        when not matched   then insert ( bpid, ccur, ccur_ref_compnr, compnr, cono, deleted, dimx, dtyp, duac, duac_kw, fobh_1, fobh_2, fobh_3, ftob, leac, nob1, nob2, nobh_1, nobh_2, nobh_3, ntob, qob1, qob2, sequencenumber, timestamp, username, year, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
