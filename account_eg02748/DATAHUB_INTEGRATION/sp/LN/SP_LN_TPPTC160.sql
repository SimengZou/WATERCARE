create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPTC160()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPTC160_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPTC160 as target using (SELECT * FROM (SELECT 
            strm.amoc, 
            strm.amos, 
            strm.btdt, 
            strm.cact, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPTC160 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPTC160_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amoc, 
            strm.amos, 
            strm.btdt, 
        ...
        when not matched   then insert ( amoc, amos, btdt, cact, ccco, ccco_ref_compnr, cico, cocu, cocu_ref_compnr, compnr, cona, cpla, cprj, cprj_cpla_cact_ref_compnr, cprj_cpla_ref_compnr, cprj_cspa_ref_compnr, cprj_cstl_ref_compnr, cprj_ref_compnr, cspa, cstl, cuni, cuni_ref_compnr, deleted, dfpc, dfpc_kw, pric, pris, quan, rtcc_1, rtcc_2, rtcc_3, rtcs_1, rtcs_2, rtcs_3, rtfc_1, rtfc_2, rtfc_3, rtfs_1, rtfs_2, rtfs_3, sacu, sacu_ref_compnr, sequencenumber, sern, stat, stat_kw, timestamp, trts, trts_kw, tsrl, tsrl_kw, txta, txta_ref_compnr, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
