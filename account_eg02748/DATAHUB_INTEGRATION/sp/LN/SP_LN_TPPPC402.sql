create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPPC402()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPPC402_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPPC402 as target using (SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
            strm.amac_4, 
            ...
            strm.mpto,
            strm.ccco,
            strm.cott ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPPC402 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.ccco=src.ccco) OR (target.ccco IS NULL AND src.ccco IS NULL)) AND
            ((target.cott=src.cott) OR (target.cott IS NULL AND src.cott IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.cott=src.cott, 
                    target.cott_kw=src.cott_kw, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPPC402_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
