create or replace procedure DATAHUB_INTEGRATION.SP_LN_WHINA115()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_WHINA115_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_WHINA115 as target using (SELECT * FROM (SELECT 
            strm.amnt_1, 
            strm.amnt_2, 
            strm.amnt_3, 
            strm.compnr, 
            ...
            strm.inwp_kw, 
            strm.item, 
            strm.item_cwar_trdt_seqn_inwp_sern_ref_compnr, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_WHINA115 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.cpcp=src.cpcp) OR (target.cpcp IS NULL AND src.cpcp IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.item_cwar_trdt_seqn_inwp_sern_ref_compnr=src.item_cwar_trdt_seqn_inwp_sern_ref_compnr, 
        ...
        when not matched and (src.ETL_DELETED=FALSE OR src.ETL_DELETED IS NULL)  then insert ( 
                    ...
                    inwp_kw, 
                    item, 
                    item_cwar_trdt_seqn_inwp_sern_ref_compnr, 
                    item_ref_compnr, 
                    seqn, 
                    sequencenumber, 
                    sern, 
            ...
            values (
                    ...
                    src.inwp_kw, 
                    src.item, 
                    src.item_cwar_trdt_seqn_inwp_sern_ref_compnr, 
                    src.item_ref_compnr, 
                    src.seqn, 
                    src.sequencenumber, 
                    src.sern, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_WHINA115_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amnt_1, 
            strm.amnt_2, 
            strm.amnt_3, 
        ...
        when not matched   then insert ( amnt_1, amnt_2, amnt_3, compnr, cpcp, cpcp_ref_compnr, cwar, cwar_ref_compnr, deleted, hour, inwp, inwp_kw, item, item_cwar_trdt_seqn_inwp_sern_ref_compnr, item_ref_compnr, seqn, sequencenumber, sern, timestamp, trdt, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
