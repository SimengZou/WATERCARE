create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPTC157()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPTC157_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPTC157 as target using (SELECT * FROM (SELECT 
            strm.amoc, 
            strm.bsqn, 
            strm.btdt, 
            strm.cact, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPTC157 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPTC157_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amoc, 
            strm.bsqn, 
            strm.btdt, 
        ...
        when not matched   then insert ( amoc, bsqn, btdt, cact, ccco, ccco_ref_compnr, ccsb, cocu, cocu_ref_compnr, compnr, cpla, cprj, cprj_cpla_cact_ref_compnr, cprj_cspa_ref_compnr, cprj_cstl_ref_compnr, cprj_ref_compnr, cspa, cstl, csub, deleted, dfpr, exep, exep_kw, fidt, otbp, otbp_ref_compnr, prby, prby_kw, prdt, pric, quan, rtcc_1, rtcc_2, rtcc_3, rtfc_1, rtfc_2, rtfc_3, sequencenumber, sern, shft, timestamp, tsrl, tsrl_kw, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
