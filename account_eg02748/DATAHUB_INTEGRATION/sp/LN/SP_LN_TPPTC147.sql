create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPTC147()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPTC147_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPTC147 as target using (SELECT * FROM (SELECT 
            strm.amoc, 
            strm.bsqn, 
            strm.btdt, 
            strm.cact, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPTC147 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.sern=src.sern) OR (target.sern IS NULL AND src.sern IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.eqan=src.eqan, 
                    target.eqty=src.eqty, 
                    target.eqty_kw=src.eqty_kw, 
                    target.exep=src.exep, 
                    target.exep_kw=src.exep_kw, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPTC147_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amoc, 
            strm.bsqn, 
            strm.btdt, 
        ...
        when not matched   then insert ( amoc, bsqn, btdt, cact, ccco, ccco_ref_compnr, cceq, cequ, cocu, cocu_ref_compnr, compnr, cpla, cprj, cprj_cpla_cact_ref_compnr, cprj_cspa_ref_compnr, cprj_cstl_ref_compnr, cprj_ref_compnr, cspa, cstl, deleted, dfrt, eqan, eqty, eqty_kw, exep, exep_kw, exeq, exeq_kw, fidt, otbp, otbp_ref_compnr, prby, prby_kw, prdt, pric, qutm, rtcc_1, rtcc_2, rtcc_3, rtfc_1, rtfc_2, rtfc_3, sequencenumber, sern, shft, timestamp, tsrl, tsrl_kw, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
