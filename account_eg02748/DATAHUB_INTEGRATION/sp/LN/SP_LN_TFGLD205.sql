create or replace procedure DATAHUB_INTEGRATION.SP_LN_TFGLD205()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TFGLD205_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TFGLD205 as target using (SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TFGLD205 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.bpid=src.bpid) OR (target.bpid IS NULL AND src.bpid IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.fcah_dtwc=src.fcah_dtwc, 
                    target.fcah_rfrc=src.fcah_rfrc, 
                    target.fcam=src.fcam, 
                    target.fdah_1=src.fdah_1, 
                    target.fdah_2=src.fdah_2, 
                    target.fdah_3=src.fdah_3, 
                    target.fdah_dtwc=src.fdah_dtwc, 
                    target.fdah_rfrc=src.fdah_rfrc, 
                    ...
                    target.ncah_dtwc=src.ncah_dtwc, 
                    target.ncah_rfrc=src.ncah_rfrc, 
                    target.ncam=src.ncam, 
                    target.ndah_1=src.ndah_1, 
                    target.ndah_2=src.ndah_2, 
                    target.ndah_3=src.ndah_3, 
                    target.ndah_dtwc=src.ndah_dtwc, 
                    target.ndah_rfrc=src.ndah_rfrc, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TFGLD205_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.bpid, 
            strm.ccur, 
            strm.ccur_ref_compnr, 
