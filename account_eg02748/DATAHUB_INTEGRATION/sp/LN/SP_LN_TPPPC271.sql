create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPPC271()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPPC271_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPPC271 as target using (SELECT * FROM (SELECT 
            strm.amoc, 
            strm.amos, 
            strm.blbl, 
            strm.blbl_kw, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPPC271 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.sern=src.sern) OR (target.sern IS NULL AND src.sern IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.tetf=src.tetf, 
                    target.tetf_kw=src.tetf_kw, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPPC271_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amoc, 
            strm.amos, 
            strm.blbl, 
        ...
        when not matched   then insert ( amoc, amos, blbl, blbl_kw, cact, ccco, ccco_ref_compnr, cdf_sref, cdf_susr, cdoc, cdoc_ref_compnr, cfpo, cfpo_kw, cohd_1, cohd_2, cohd_3, compnr, cpla, cprj, cprj_cpla_cact_ref_compnr, cprj_cspa_ref_compnr, cprj_cstl_ref_compnr, cprj_ref_compnr, cptc, cptc_ref_compnr, cptc_year_peri_ref_compnr, cptf, cptf_fyea_fper_ref_compnr, cptf_ref_compnr, cspa, cstl, csub, cuni, cuni_ref_compnr, curc, curc_ref_compnr, curs, curs_ref_compnr, deleted, desc, entu, fcrt, fcrt_kw, fdoc, flin, fper, fsrl, ftln, ftty, fyea, guid, ifbp, ifbp_ref_compnr, loco, ltdt, ncmp, orno, otbp, otbp_ref_compnr, peri, pono, potf, potf_kw, pric, pris, quan, rdas, rdat, rgdt, rper, rtcc_1, rtcc_2, rtcc_3, rtcs_1, rtcs_2, rtcs_3, rtfc_1, rtfc_2, rtfc_3, rtfs_1, rtfs_2, rtfs_3, rtyp, rtyp_ref_compnr, ryea, sequencenumber, sern, srnb, srni, sttl, sttl_kw, tetf, tetf_kw, timestamp, txta, txta_ref_compnr, username, year, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
