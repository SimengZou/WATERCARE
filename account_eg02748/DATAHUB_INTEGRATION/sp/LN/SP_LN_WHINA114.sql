create or replace procedure DATAHUB_INTEGRATION.SP_LN_WHINA114()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_WHINA114_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_WHINA114 as target using (SELECT * FROM (SELECT 
            strm.atse, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_WHINA114 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_WHINA114_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.atse, 
            strm.atse_ref_compnr, 
            strm.bfbp, 
        ...
        when not matched   then insert ( atse, atse_ref_compnr, bfbp, clot, compnr, cprj, cprj_ref_compnr, ctdt, cwar, cwar_ref_compnr, deleted, dlin, inwp, inwp_kw, item, item_ref_compnr, itid, itse, koor, koor_kw, lgdt, ocmp, ocmp_ref_compnr, orno, ownr, owns, owns_kw, phtr, phtr_kw, pono, qstk, reje, reje_kw, seqn, sequencenumber, serl, sern, srnb, tagn, timestamp, trdt, username, wvgr, wvgr_ref_compnr, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
