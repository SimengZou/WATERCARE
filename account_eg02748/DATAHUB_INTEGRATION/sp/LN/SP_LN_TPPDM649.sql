create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPDM649()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPDM649_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPDM649 as target using (SELECT * FROM (SELECT 
            strm.cnln, 
            strm.compnr, 
            strm.cono, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPDM649 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.cres=src.cres) OR (target.cres IS NULL AND src.cres IS NULL)) AND
            ((target.seqn=src.seqn) OR (target.seqn IS NULL AND src.seqn IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.vldf=src.vldf, 
                    target.vldt=src.vldt, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPDM649_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.cnln, 
            strm.compnr, 
            strm.cono, 
        ...
        when not matched   then insert ( cnln, compnr, cono, cono_cnln_ref_compnr, cono_ref_compnr, cprj, cprj_ref_compnr, cres, cres_ref_compnr, deleted, emno, emno_ref_compnr, seqn, sequencenumber, timestamp, txta, txta_ref_compnr, username, vldf, vldt, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
