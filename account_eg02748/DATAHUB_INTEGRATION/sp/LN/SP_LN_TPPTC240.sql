create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPTC240()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPTC240_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPTC240 as target using (SELECT * FROM (SELECT 
            strm.acln, 
            strm.amoc, 
            strm.amos, 
            strm.btdt, 
        ...
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPTC240 as  strm
                    )   
            WHERE ROWNUMBER=1  
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPTC240_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.acln, 
            strm.amoc, 
            strm.amos, 
        ...
        when not matched   then insert ( acln, amoc, amos, btdt, cact, ccal, ccco, ccco_ref_compnr, cequ, clas, clas_ref_compnr, cocu, cocu_ref_compnr, compnr, cona, copy, copy_kw, cpla, cprj, cprj_cpla_cact_ref_compnr, cprj_cpla_ref_compnr, cprj_cstl_ref_compnr, cprj_ref_compnr, cspa, cstl, cuti, cuti_ref_compnr, deleted, dfrc, dfrc_kw, dura, eqan, eqty, eqty_kw, espp, exeq, exeq_kw, lcta, lino, orno, otbp, otbp_ref_compnr, qutm, ratc, rats, rfin, rsta, rtcc_1, rtcc_2, rtcc_3, rtcs_1, rtcs_2, rtcs_3, rtfc_1, rtfc_2, rtfc_3, rtfs_1, rtfs_2, rtfs_3, sacu, sacu_ref_compnr, sequencenumber, sern, slnk, slnk_kw, snsb, stat, stat_kw, timestamp, tmud, trts, trts_kw, tsrl, tsrl_kw, txta, txta_ref_compnr, username, 
                    ETL_SEQUENCE_NUMBER,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
