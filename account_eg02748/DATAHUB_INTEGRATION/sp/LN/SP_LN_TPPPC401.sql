create or replace procedure DATAHUB_INTEGRATION.SP_LN_TPPPC401()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_LN_TPPPC401_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_LN.LN_TPPPC401 as target using (SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
            strm.amac_4, 
            ...
            strm.cprj,
            strm.mpto,
            strm.cotp ORDER BY strm.ETL_SEQUENCE_NUMBER desc,IFNULL(TRY_TO_TIMESTAMP(replace(right(replace(lower(etl_load_metadatafilename),'.json'),23),'_','-'), 'yyyy-mm-dd-HH-MI-SS-FF') ,etl_load_datetime) desc) as ROWNUMBER
        FROM DATAHUB_INTEGRATION.VW_STREAM_LN_TPPPC401 as  strm
                    )   
            WHERE ROWNUMBER=1  
            ...
            ((target.cotp=src.cotp) OR (target.cotp IS NULL AND src.cotp IS NULL)) 
                ...
                when matched then update set
                    ...
                    target.quac=src.quac, 
                    target.quap=src.quap, 
                    target.qubg=src.qubg, 
                    target.quep=src.quep, 
                    target.quex=src.quex, 
                    target.qupe=src.qupe, 
                    target.qupm=src.qupm, 
                    target.qupp=src.qupp, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_LN.LN_TPPPC401_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.amac_1, 
            strm.amac_2, 
            strm.amac_3, 
