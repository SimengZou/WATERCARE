create or replace procedure DATAHUB_INTEGRATION.SP_EAM_U5PUBLICHOLIDAYS()
...
try
    {
        var sql_command = `
                            ...
                            SELECT * FROM LANDING_ERROR.VW_STREAM_EAM_U5PUBLICHOLIDAYS_ERROR`;
    var sqlStmt = snowflake.createStatement( {sqlText:  sql_command} );
    var rs = sqlStmt.execute();
        
        var sql_command = ` 
                            merge into  TARGET_EAM.EAM_U5PUBLICHOLIDAYS as target using (SELECT * FROM (SELECT 
            strm.AUTO, 
            strm.CREATED, 
            strm.CREATEDBY, 
            strm.HLY_CONCODE, 
    ...
    snowflake.execute( {sqlText: `merge into TARGET_HISTORY_EAM.EAM_U5PUBLICHOLIDAYS_DELETED as target using (
                SELECT * FROM (SELECT 
            strm.AUTO, 
            strm.CREATED, 
            strm.CREATEDBY, 
        ...
        when not matched   then insert ( AUTO, CREATED, CREATEDBY, HLY_CONCODE, HLY_DATE, HLY_ID, HLY_NAME, HLY_ORG, HLY_YEAR, LASTSAVED, UPDATECOUNT, UPDATED, UPDATEDBY, _DELETED, 
                    ETL_LASTSAVED,                       
                    etl_load_datetime,
                    etl_load_metadatafilename,
                    etl_is_deleted,
