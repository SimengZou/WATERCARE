//  Name:       sp_ips_sm_billing_extract_history
//
//  Purpose:    Load meter readings for Smart Meters from Data Lake to IPS - permanent table 
//   ips_sm_billing_extract_history loaded from staging tables: 
//  STG90_IPS_SM_BILLING_EXTRACT 
//  by job sp_ips_sm_billing_extract_history
//
//  Keep extracted data for 60 days in history table 
//
//  Description:
//
//  Stored procedure for IPS billing for smart meters from datalake
//
//  Parameter: DELETE_DAYS datatype: delete records older then DELETE_DAYS
//
...
//
//  4.  Populate target table
//
//  5.  Commit transaction and complete process run
//


create or replace procedure datahub_extract.sp_ips_sm_billing_extract_history(DELETE_DAYS FLOAT)
    returns string
    language javascript
    execute as caller
    as
...
//  Delete old records as per delete_days 
        ...
        snowflake.execute ({sqlText: `DELETE FROM DATAHUB_EXTRACT.IPS_SM_BILLING_EXTRACT_HISTORY
...
//  Run insert statement  
        ...
        try
            ...
            var sqlStmt = snowflake.createStatement( {sqlText: `INSERT INTO DATAHUB_EXTRACT.IPS_SM_BILLING_EXTRACT_HISTORY 
(ROUTE, READING_DATE, METER, REGISTER_NUMBER, READING, READ_TYPE, REASON_UNREAD, READER, NOTES, READING_TYPE, ETL_LOAD_DATETIME) 
...
convert_timezone('UTC', 'Pacific/Auckland', CURRENT_TIMESTAMP()) AS etl_load_datetime
