                ...
                create or replace procedure DATAHUB_INTEGRATION.EAM_TARGET_MERGE_R5VIOLATIONS()
              ...
              //  Start execution - log start
                ...
                snowflake.execute( {sqlText: "begin transaction;"} );
                  try
                      ...
                      var sql_command = `      
                                merge into  "DATAHUB_TARGET"."EAM_R5VIOLATIONS" as target using (
                    ...
                    SELECT * FROM (SELECT
                     ...
                     FROM DATAHUB_INTEGRATION.VW_STREAM_EAM_R5VIOLATIONS as  strm
                     )   
                WHERE ROWNUMBER=1  
                    ...
                    when matched then update set
                                target.VIO_USER=src.VIO_USER,
                                target.VIO_PASSWORD=src.VIO_PASSWORD,
                                target.VIO_DATE=src.VIO_DATE,
                                target.VIO_OSUSER=src.VIO_OSUSER,
                                target.VIO_OSMACHINE=src.VIO_OSMACHINE,
                                target.VIO_UPDATECOUNT=src.VIO_UPDATECOUNT,
                                target.VIO_LASTSAVED=src.VIO_LASTSAVED,
...
`
  merge into  "DATAHUB_TARGET_HISTORY"."EAM_DELETED_R5VIOLATIONS" as target using (
